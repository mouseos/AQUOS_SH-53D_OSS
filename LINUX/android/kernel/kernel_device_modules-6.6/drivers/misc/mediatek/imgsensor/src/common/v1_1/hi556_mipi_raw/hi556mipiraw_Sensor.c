...
{ 2592, 1944,    0,    0, 2592, 1944, 2592, 1944,     0,    0, 2592, 1944,    0,    0, 2592, 1944},
	// video
{ 2592, 1944,    0,    0, 2592, 1458, 2592, 1458,     0,    0, 2592, 1458,    0,    0, 2592, 1458},
       // high speed video
...
//fihtdc, for get otp, start
int otp_read = 0, module_info_err = 0, awb_err = 0, lsc_err = 0;
unsigned char module_info_data[HI556_MODULE_INFO_SIZE];
unsigned char awb_data[HI556_AWB_SIZE_WITH_CHECKSUM];
unsigned char lsc_data[HI556_LSC_SIZE_WITH_CHECKSUM];
unsigned char chip_id[HI556_AWB_SIZE_WITH_CHECKSUM];

static int OTP_Init_HI556(void)
{
	//Fast sleep on
...
static int OTP_Deinit_HI556(void)
{
	//Stand by on
	write_cmos_sensor_8(0x0a00, 0x00);
	//delay 10ms
	mdelay(10);
	//Disaply mode
	write_cmos_sensor_8(0x003e, 0x01);
	//Stand by off
	write_cmos_sensor_8(0x0a00, 0x01);
	return 0;
}
...
//int otp_read = 0;
unsigned char all_data[HI556_DATA_SIZE];

static void read_otp_all(void)
{
    int i = 0;
    //unsigned char start_flag;
    kal_uint32 start_Offset = HI556_START_OFFSET;

    //start_flag = read_cmos_sensor_otp(HI556_MODULE_INFO_FLAG);

	write_cmos_sensor_8(HI556_R_OTP_ADDR_H, start_Offset >> 8);
	write_cmos_sensor_8(HI556_R_OTP_ADDR_L, start_Offset & 0xFF);
	write_cmos_sensor_8(HI556_R_OTP_CMD, 0x01);
	pr_info("[read_otp_all][rex] read otp start!!\n");
	for(i = 0; i < HI556_DATA_SIZE; i++) {
		all_data[i] = read_cmos_sensor(HI556_R_OTP_RDATA);

	}
	pr_info("[read_otp_all][rex] read otp end!!\n");
}
void read_hi556_otp_data(void)
{
	if(otp_read==0){
		OTP_Init_HI556();
		//get all otp data
		...
		OTP_Deinit_HI556();
		otp_read = 1;
		pr_debug("[read_hi556_otp_data] read otp!!\n");
	} else {
		pr_debug("[read_hi556_otp_data] already read!!\n");
	}
}

unsigned int HI556_read_region(unsigned int addr,
                unsigned char *data, unsigned int size)
{
	int ret = 0;

	memcpy(data, all_data, HI556_DATA_SIZE);

	return ret;
}
EXPORT_SYMBOL(HI556_read_region);
...
kal_uint16 addr_data_pair_init_hi556[] = {
	...
	0x006c, 0x0000,
	0x005e, 0x0200,
	0x000e, 0x0200,/*mirror off,flip on*/
	0x0e0a, 0x0001,
	0x004a, 0x0100,
	0x004c, 0x0000,
	0x000c, 0x0022,
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x000e, 0x0200);/*mirror off,flip on*/
	...
	write_cmos_sensor(0x004e, 0x0700);
#endif
}
...
kal_uint16 addr_data_pair_preview_hi556[] = {
	...
	0x0030, 0x1111,
	0x0032, 0x1111,
	0x0006, 0x0822, //frame line length
	0x0a22, 0x0000,
	0x0a12, 0x0a20, //x output size
	0x0a14, 0x0798, //y output size
	0x003e, 0x0000,
	0x0074, 0x0820, //coarse integration time
	0x0070, 0x0400,
	0x0804, 0x0200,
	0x0a04, 0x014a,
	0x090c, 0x0fdc,
...
////////////////////////////////////////////////
	...
	write_cmos_sensor(0x0f32, 0x7067);
	write_cmos_sensor(0x004a, 0x0100);
	write_cmos_sensor(0x004c, 0x0000); //perframe enable
	write_cmos_sensor(0x000c, 0x0022);
	write_cmos_sensor(0x0008, 0x0b00);
	write_cmos_sensor(0x005a, 0x0202);
	write_cmos_sensor(0x0012, 0x000e);
...
#if MULTI_WRITE
//2592*1944
kal_uint16 addr_data_pair_normal_video_fps_hi556[] = {
	0x0a00, 0x0000,
	0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7067,
...
////////////////////////////////////////////////
#if MULTI_WRITE
	hi556_table_write_cmos_sensor(
		addr_data_pair_normal_video_fps_hi556,
		sizeof(addr_data_pair_normal_video_fps_hi556) /
		sizeof(kal_uint16));
#else
	write_cmos_sensor(0x0a00, 0x0000);
	write_cmos_sensor(0x0b0a, 0x8252);
	...
	write_cmos_sensor(0x004c, 0x0000);
	write_cmos_sensor(0x000c, 0x0022);
	write_cmos_sensor(0x0008, 0x0b00); //perframe enable
	write_cmos_sensor(0x005a, 0x0202);
	write_cmos_sensor(0x0012, 0x000e);
	write_cmos_sensor(0x0018, 0x0a31);
	write_cmos_sensor(0x0022, 0x0008);
...
//Image size        : 640x480
//MCLK              : 24MHz
//MIPI speed(Mbps)  : 2200Mbps x 2Lane
//Frame Length	  	: 520
//Line Length	  	: 2816
//Max Fps	  		: 120.19fps
...
static kal_uint32 open(void)
{
	...
	LOG_INF("[%s]: PLATFORM:MT6761,MIPI 24LANE\n", __func__);
	...
	struct SET_SENSOR_PATTERN_SOLID_COLOR *pdata)
{
	...
	pr_debug("hi556-> set_test_pattern enum: %d\n", modes);

	if (modes == 1 && (pdata != NULL)) { //Solid Color
	    write_cmos_sensor(0x0a04, 0x0143);
		write_cmos_sensor_8(0x0201, modes); //1:solid color from hi556 datasheet
		pr_debug("hi556->Solid color : R=0x%x,Gr=0x%x,B=0x%x,Gb=0x%x",
		...
		pr_debug("hi556-> write : R=0x%x,Gr=0x%x,B=0x%x,Gb=0x%x",Color_R, Color_Gr, Color_B, Color_Gb);
		write_cmos_sensor_8(0x0206, (Color_R >> 8) & 0x3);
		write_cmos_sensor_8(0x0207, Color_R & 0xFF);
		write_cmos_sensor_8(0x0208, (Color_Gr >> 8) & 0x3);
		write_cmos_sensor_8(0x0209, Color_Gr & 0xFF);
		write_cmos_sensor_8(0x020A, (Color_B >> 8) & 0x3);
		write_cmos_sensor_8(0x020B, Color_B & 0xFF);
		write_cmos_sensor_8(0x020C, (Color_Gb >> 8) & 0x3);
		write_cmos_sensor_8(0x020D, Color_Gb & 0xFF);
	} else if (modes == 2) { //Color bars
		pr_debug("hi556-> Color bars");
		write_cmos_sensor(0x0a04, 0x0143);
		write_cmos_sensor_8(0x0201, modes); //2:color bars from hi556 datasheet*/
	} else if ( modes == 5) { //Black color
	    write_cmos_sensor(0x0a04, 0x0143);
		write_cmos_sensor_8(0x0201, 0x01); //1:solid color from hi556 datasheet
		pr_debug("hi556-> BLACK pdata : R=0x%x,Gr=0x%x,B=0x%x,Gb=0x%x",
			pdata->COLOR_R, pdata->COLOR_Gr, pdata->COLOR_B, pdata->COLOR_Gb);
		Color_R = 0;
		Color_Gr = 0;
		Color_B = 0;
		Color_Gb = 0;
		pr_debug("hi556-> Force set BLACK pattern: R=0x%x,Gr=0x%x,B=0x%x,Gb=0x%x",Color_R, Color_Gr, Color_B, Color_Gb);
		write_cmos_sensor_8(0x0206, Color_R);
		write_cmos_sensor_8(0x0207, Color_R);
		write_cmos_sensor_8(0x0208, Color_Gr);
		write_cmos_sensor_8(0x0209, Color_Gr);
		write_cmos_sensor_8(0x020A, Color_B);
		write_cmos_sensor_8(0x020B, Color_B);
		write_cmos_sensor_8(0x020C, Color_Gb);
		write_cmos_sensor_8(0x020D, Color_Gb);
	} else {
		pr_debug("hi556-> No pattern");
		write_cmos_sensor(0x0a04, 0x0142);
		write_cmos_sensor_8(0x0201, 0x0000); /*No pattern*/
	}
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
	    *(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 1300000;
	break;
	...
	case SENSOR_FEATURE_GET_FRAME_CTRL_INFO_BY_SCENARIO:
		...
		*(feature_data + 2) = imgsensor_info.margin;
		pr_debug("SENSOR_FEATURE_GET_FRAME_CTRL_INFO_BY_SCENARIO margin:%d\n", imgsensor_info.margin);
		break;
	...
	case SENSOR_FEATURE_GET_PERIOD_BY_SCENARIO:
		...
		pr_debug("SENSOR_FEATURE_GET_PERIOD_BY_SCENARIO scenarioId:%d peroid:%d\n", (UINT32)*feature_data, *(MUINT32 *)(uintptr_t)(*(feature_data + 1)));
		break;
	...
	break;
	/*misty add for[FAQ21105] image sensor driver template change for mt6761*/
