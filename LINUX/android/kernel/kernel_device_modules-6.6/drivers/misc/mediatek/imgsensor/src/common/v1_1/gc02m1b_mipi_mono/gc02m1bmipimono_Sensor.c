...
#define MULTI_WRITE 0 //gc02m1 sensor not support sr signal, multi_write i2c using new methon afer kerenl-5.10. Detail please renfer mtk case : ALPS09410607
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.min_gain = (1024/16),   /* 1x gain ,reference /isp_6s/sensor_mgr/aaa_sensor_mgr.cpp MRESULT AAASensorMgr::getSensorDeviceInfo2*/
	.max_gain = (12288/16), /* 16x gain ,reference /isp_6s/sensor_mgr/aaa_sensor_mgr.cpp MRESULT AAASensorMgr::getSensorDeviceInfo2*/
	.min_gain_iso = 100,
	.margin = 16,
	.min_shutter = 4,
	.gain_step = 1,
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	switch (image_mirror) {
	case IMAGE_NORMAL:
		write_cmos_sensor(0x017, 0x00);
		break;
	case IMAGE_H_MIRROR:
		write_cmos_sensor(0x017, 0x01);
		break;
	case IMAGE_V_MIRROR:
		write_cmos_sensor(0x017, 0x02);
		break;
	case IMAGE_HV_MIRROR:
		write_cmos_sensor(0x017, 0x03);
		break;
	default:
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PERIOD_BY_SCENARIO:
		...
		LOG_INF("SENSOR_FEATURE_GET_PERIOD_BY_SCENARIO scenarioId:%d peroid:%d\n", (UINT32)*feature_data, *(MUINT32 *)(uintptr_t)(*(feature_data + 1)));
		break;
        //[FIH][Alex] E: sensor porting	
...
/*misty add for[FAQ21105] image sensor driver template change for mt6761*/
	case SENSOR_FEATURE_GET_PIXEL_RATE:
	{
		...
		LOG_INF("SENSOR_FEATURE_GET_PIXEL_RATE scenarioId:%d rate:%d\n", (UINT32)*feature_data, rate);
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = rate;
	}
	break;
	default:
