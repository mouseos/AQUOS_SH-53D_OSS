...
static imgsensor_info_struct imgsensor_info = {
	...
	.min_gain_iso = 100,
	.gain_step = 2,                  // Samsung:2, Follow MTK document Sensor_Porting_Introduction_-_5G.pdf
	.gain_type = 2,
	...
	.sensor_output_dataformat =  SENSOR_OUTPUT_FORMAT_RAW_Gr,//IMAGE_V_MIRROR->SENSOR_OUTPUT_FORMAT_RAW_B, /*fih set as mirror flip */
	.mclk = 24,
...
static imgsensor_struct imgsensor = {
	.mirror = IMAGE_NORMAL,//IMAGE_V_MIRROR,			/* mirrorflip information *//*fih set as mirror flip*/
...
static void read_otp_chip_id(void)
{
	...
	check_byte = read_cmos_sensor(0x0A01);
	if((check_byte & 0x01)==1){
	    for(i = 0; i < 8; i++) {
	        chip_id[i] = read_cmos_sensor(0xA04+i);
	        pr_err("[S5K4H7]chip_id[%d] 0x%02X\n", i, chip_id[i]);
            }
        }
}
*/
...
static void write_shutter(kal_uint16 shutter)
{
    ...
    if (pre_frame_flag == 0) {
		pre_frame_flag = 1;
		LOG_INF("debug first pre_frame_flag:%d\n", pre_frame_flag);
	}
...
static void preview_setting(void)
{
	...
	write_cmos_sensor_8(0x3930, 0x80);      ////M_DPHYCTL[23]:1  Cap peaking Tr/Tf Control for SX4 board
...
static void hs_video_setting(void)
{
	...
	write_cmos_sensor_8(0x0901, 0x44);		// remember to setup MTK's (SENSOR_FEATURE_GET_BINNING_TYPE)
	write_cmos_sensor_8(0x0381, 0x01);
	write_cmos_sensor_8(0x0383, 0x01);
	write_cmos_sensor_8(0x0385, 0x01);
...
static void slim_video_setting(void)
{
	...
	write_cmos_sensor_8(0x0900, 0x01);
	write_cmos_sensor_8(0x0901, 0x22);		// remember to setup MTK's (SENSOR_FEATURE_GET_BINNING_TYPE)
	write_cmos_sensor_8(0x0381, 0x01);
	write_cmos_sensor_8(0x0383, 0x01);
	write_cmos_sensor_8(0x0385, 0x01);
...
#if 0
	itemp = 0;
	itemp = read_cmos_sensor(0x3932);
	itemp &= ~0x0C;
	//0x3932->M_DPHYCTL[7:0]  //M_DPHYCTL[3:2] TX Impedance Control
	//write_cmos_sensor_8(0x3932, itemp | 0x04);//55
	//write_cmos_sensor_8(0x3932, itemp | 0x08);//45
	write_cmos_sensor_8(0x3932, itemp | 0x0C);//50

	//0x3930 -> M_DPHYCTL[23:16]  //0x392F -> M_DPHYCTL[31:24]  //M_DPHYCTL[24:23]  Tr/Tf Control 
	itemp = 0;
	itemp1 = 0;
	itemp = read_cmos_sensor(0x3930);
	itemp1 = read_cmos_sensor(0x392F);
	itemp &= ~0x80; //bit23
	itemp1 &= ~0x01; //bit24

	write_cmos_sensor_8(0x3930, itemp);//1step up bit23:0
	write_cmos_sensor_8(0x392F, itemp1);//1step up bit24:0

	//write_cmos_sensor_8(0x3930, itemp | 0x80);//1step up bit23:1
	//write_cmos_sensor_8(0x392F, itemp1);//1step up bit24:0

	//write_cmos_sensor_8(0x3930, itemp);//1step up bit23:0
	//write_cmos_sensor_8(0x392F, itemp1 | 0x01);//1step up bit24:1

	//write_cmos_sensor_8(0x3930, itemp | 0x80);//2step up bit23:1
	//write_cmos_sensor_8(0x392F, itemp1 | 0x01);//2step up bit24:1
#endif
//test for hw MIPI--
...
{
	pr_debug("s5k4h7-> set_test_pattern enum: %d\n", modes);

	if (modes == 5 && (pdata != NULL)) { //Black Color
		write_cmos_sensor_8(0x0601, 0x01); //1:solid color from S5K4H7YX datasheet
...
//fih++mirror flip check
#if 0
	kal_uint8 itemp;
	itemp = read_cmos_sensor(0x0101);
		pr_err("[Misty]CHECK***bbbb****image_mirror itemp= %d*******\n", itemp);
		pr_err("[Misty]////*******image_mirror = %d*******\n", imgsensor.mirror);

	if((imgsensor.mirror==IMAGE_HV_MIRROR) && (itemp !=3)){
		write_cmos_sensor_8(0x0100, 0x01);
		write_cmos_sensor_8(0x0101, 0x03);
		itemp=88;
		itemp = read_cmos_sensor(0x0101);
		pr_err("[Misty]CHECK***aaaa****image_mirror itemp= %d*******\n", itemp);
	}

	else if((imgsensor.mirror==IMAGE_V_MIRROR) && (itemp !=2)){
		write_cmos_sensor_8(0x0100, 0x01);
		write_cmos_sensor_8(0x0101, 0x02);
		itemp=77;
		itemp = read_cmos_sensor(0x0101);
...
//fih--
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			*feature_return_para_32 = 2;	// write_cmos_sensor_8(0x0901, 0x22);
			break;

		case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
			*feature_return_para_32 = 4;	//write_cmos_sensor_8(0x0901, 0x44);
			break;
