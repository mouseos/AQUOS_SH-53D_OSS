...
static __s32 mtk_ts_quiet_volt_to_temp(__u32 dwVolt)
{
	...
	mtkts_quiet_dprintk("TRes:%d, g_RAP_pull_up_R:%d\n", TRes, g_RAP_pull_up_R);
...
#if IS_ENABLED(CONFIG_DEVICE_MODULES_MEDIATEK_MT6577_AUXADC)
	if (IS_ERR_OR_NULL(thermistor_ch6)) {
		mtkts_quiet_printk("invalid thermistor_ch6:0x%px\n", thermistor_ch6);
		return ret;
	}
...
static int mtkts_quiet_change_mode(struct thermal_zone_device *thermal,
...
static struct thermal_zone_device_ops mtkts_quiet_dev_ops = {
	...
	.change_mode = mtkts_quiet_change_mode,
...
#else
			mtkts_quiet_printk(
					"[mtkts_quiet_write] Not support overwrite PUP_R argument, g_RAP_pull_up_R is always %d\n", g_RAP_pull_up_R);
#endif
...
static int mtkts_quiet_register_thermal(void)
{
	...
	/* trips : trip 0~1 */
	thz_dev = mtk_thermal_zone_device_register("mtktsquiet", trips, num_trip,
...
static const struct proc_ops mtkts_quiet_fops = {
	.proc_open = mtkts_quiet_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtkts_quiet_write,
...
static const struct proc_ops mtkts_quiet_param_fops = {
	.proc_open = mtkts_quiet_param_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtkts_quiet_param_write,
	.proc_release = single_release,
};
