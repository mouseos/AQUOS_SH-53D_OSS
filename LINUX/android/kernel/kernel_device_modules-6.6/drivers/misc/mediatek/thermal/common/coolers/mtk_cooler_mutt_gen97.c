				...
				unsigned long state)
{
	...
	if (clmutt_data.disabled == 1) { //REQ: disable modem thermal mitigation
		mtk_cooler_mutt_dprintk_always(
			"[%s] %s: modem mitigation disabled, force state %lu to 0\n",
...
static void mtk_cl_mutt_set_mutt_limit(enum mutt_type type)
{
	...
	/* DO NOT compare with other type (NR/LTE) cooler level */
	...
	/* DO NOT check common cur_level, check each type's target_level instead */
	/* if (target_lv != clmutt_data.cur_level) { */
	if (target_lv != clmutt_data.cooler_param[type].target_level) {
		/* update lv to MD */
		ret = clmutt_send_tmc_cmd(
			clmutt_level_selection(target_lv, type));
...
static int clmutt_disabled_proc_read(struct seq_file *m, void *v)
{
	seq_printf(m, "disabled: %d\n", clmutt_data.disabled);

	return 0;
}

static ssize_t clmutt_disabled_proc_write(struct file *filp,
	const char __user *buffer, size_t count, loff_t *data)
{
	char desc[PROC_BUFFER_LEN] = {0};
	int len = 0, disabled;
	...
	if (kstrtoint(desc, 10, &disabled) == 0) {
		mtk_cooler_mutt_dprintk_always(
			"[%s] disabled:%d\n", __func__, disabled);

		mutex_lock(&clmutt_data.lock);
		clmutt_data.disabled = disabled;
		mutex_unlock(&clmutt_data.lock);
