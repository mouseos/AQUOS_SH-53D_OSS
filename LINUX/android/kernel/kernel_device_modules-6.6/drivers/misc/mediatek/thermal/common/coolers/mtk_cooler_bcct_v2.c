...
//	enable_power_path(!state);
	if (state == 1) {
		chrlmt_set_limit(&cl_bcct_chrlmt_handle, 0, (cl_bcct_cur_limit >= 65535)?(-1):cl_bcct_cur_limit, 0);
		mtk_cooler_bcct_dprintk("%s enable stop charger\n", __func__);
	} else {
		chrlmt_set_limit(&cl_bcct_chrlmt_handle, -1, (cl_bcct_cur_limit >= 65535)?(-1):cl_bcct_cur_limit, -1);
