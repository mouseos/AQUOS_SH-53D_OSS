...
#define MAX_NUM_INSTANCE_MTK_COOLER_BCCT  13
...
static void chrlmt_set_limit_handler(struct work_struct *work)
{
	...
	if (chrlmt_bat_chr_curr_limit != -1)
		...
		prop.intval = UNLIMIT_CURRENT_MASK;
...
static int mtk_cl_bcct_chgstop_get_max_state(
struct thermal_cooling_device *cdev, unsigned long *state)
{
	*state = 1;
	mtk_cooler_bcct_dprintk("%s %s %lu\n", __func__, cdev->type, *state);
...
static int mtk_cl_bcct_chgstop_get_cur_state(
struct thermal_cooling_device *cdev, unsigned long *state)
{
	MTK_CL_BCCT_GET_CURR_STATE(*state, *((unsigned long *)cdev->devdata));
	mtk_cooler_bcct_dprintk("%s %s %lu\n", __func__, cdev->type, *state);
...
static int mtk_cl_bcct_chgstop_set_cur_state(
struct thermal_cooling_device *cdev, unsigned long state)
{
	mtk_cooler_bcct_dprintk("%s %s %lu\n", __func__, cdev->type, state);
	MTK_CL_BCCT_SET_CURR_STATE(state, *((unsigned long *)cdev->devdata));
//	enable_power_path(!state);
	if (state == 1) {
		chrlmt_set_limit(&cl_bcct_chrlmt_handle, 0, (cl_bcct_cur_limit >= 65535)?(-1):cl_bcct_cur_limit);
		mtk_cooler_bcct_dprintk("%s enable stop charger\n", __func__);
	} else {
		chrlmt_set_limit(&cl_bcct_chrlmt_handle, -1, (cl_bcct_cur_limit >= 65535)?(-1):cl_bcct_cur_limit);
		mtk_cooler_bcct_dprintk("%s disable stop charger\n", __func__);
	}
	return 0;
}

/* bind chgstop callbacks to chgstop device */
static struct thermal_cooling_device_ops mtk_cl_bcct_chgstop_ops = {
	.get_max_state = mtk_cl_bcct_chgstop_get_max_state,
	.get_cur_state = mtk_cl_bcct_chgstop_get_cur_state,
	.set_cur_state = mtk_cl_bcct_chgstop_set_cur_state,
};
...
static int mtk_cooler_bcct_register_ltf(void)
{
...
#elif (MAX_NUM_INSTANCE_MTK_COOLER_BCCT == 13)
	MTK_CL_BCCT_SET_LIMIT(3000, cl_bcct_state[0]);
	MTK_CL_BCCT_SET_LIMIT(1500, cl_bcct_state[1]);
	MTK_CL_BCCT_SET_LIMIT(1400, cl_bcct_state[2]);
	MTK_CL_BCCT_SET_LIMIT(1200, cl_bcct_state[3]);
	MTK_CL_BCCT_SET_LIMIT(1100, cl_bcct_state[4]);
	MTK_CL_BCCT_SET_LIMIT(1000, cl_bcct_state[5]);
	MTK_CL_BCCT_SET_LIMIT( 900, cl_bcct_state[6]);
	MTK_CL_BCCT_SET_LIMIT( 700, cl_bcct_state[7]);
	MTK_CL_BCCT_SET_LIMIT( 600, cl_bcct_state[8]);
	MTK_CL_BCCT_SET_LIMIT( 500, cl_bcct_state[9]);
	MTK_CL_BCCT_SET_LIMIT( 400, cl_bcct_state[10]);
	MTK_CL_BCCT_SET_LIMIT( 300, cl_bcct_state[11]);
	MTK_CL_BCCT_SET_LIMIT(   1, cl_bcct_state[12]);
#endif
	...
	cl_bcct_chgstop_dev = mtk_thermal_cooling_device_register("mtk-cl-bcct-chgstop",
						(void *)&cl_bcct_chgstop_state,
						&mtk_cl_bcct_chgstop_ops);
...
static void mtk_cooler_bcct_unregister_ltf(void)
{
	...
	if (cl_bcct_chgstop_dev) {
		mtk_thermal_cooling_device_unregister(cl_bcct_chgstop_dev);
		cl_bcct_chgstop_dev = NULL;
		cl_bcct_chgstop_state = 0;
	}
...
struct file *filp, const char __user *buf, size_t len, loff_t *data)
{
	/* int ret = 0; */
	char tmp[128] = { 0 };
	int klog_on, limit0, limit1, limit2, limit3, limit4, limit5, limit6, limit7, limit8, limit9, limit10, limit11, limit12;
	...
	MTK_CL_BCCT_SET_LIMIT(-1, cl_bcct_state[10]);
	MTK_CL_BCCT_SET_LIMIT(-1, cl_bcct_state[11]);
	MTK_CL_BCCT_SET_LIMIT(-1, cl_bcct_state[12]);

	if (sscanf(
		tmp, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d", &klog_on, &limit0, &limit1, &limit2, &limit3, &limit4, &limit5, &limit6, &limit7, &limit8, &limit9, &limit10, &limit11, &limit12) >= 1) {
		if (klog_on == 0 || klog_on == 1)
			cl_bcct_klog_on = klog_on;
		...
		if (limit10 >= -1)
			MTK_CL_BCCT_SET_LIMIT(limit10, cl_bcct_state[10]);
		if (limit11 >= -1)
			MTK_CL_BCCT_SET_LIMIT(limit11, cl_bcct_state[11]);
		if (limit12 >= -1)
			MTK_CL_BCCT_SET_LIMIT(limit12, cl_bcct_state[12]);
