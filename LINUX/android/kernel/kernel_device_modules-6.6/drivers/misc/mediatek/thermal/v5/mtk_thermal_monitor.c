...
int mtk_thermal_get_tz_idx(char *type)
{
	...
	else if (strncmp(type, "mtktschargerntc", 15) == 0)
		return MTK_THERMAL_SENSOR_CHARGER_NTC;
	else if (strncmp(type, "mtktscharger", 12) == 0)
		return MTK_THERMAL_SENSOR_CHARGER;
	else if (strncmp(type, "mtktscamera", 11) == 0)
		return MTK_THERMAL_SENSOR_CAMERA;
	else if (strncmp(type, "mtktsquiet", 10) == 0)
		return MTK_THERMAL_SENSOR_QUIET;

	return -1;
}
EXPORT_SYMBOL(mtk_thermal_get_tz_idx);
...
static int __init thermal_monitor_init(void)
{
		...
		tsallts_init();
		//FIH-Thermal-Add+[
		mtkts_chargerntc_init();
		mtkts_camera_init();
		mtkts_quiet_init();
		mtk_cooler_tethering_init();
		mtk_cooler_ushutdown_init();
		//FIH-Thermal-Add+]
		return 0;
}
...
static void __exit thermal_monitor_exit(void)
{
	...
	tsallts_exit();
	//FIH-Thermal-Add+[
	mtkts_chargerntc_exit();
	mtkts_camera_exit();
	mtkts_quiet_exit();
	mtk_cooler_tethering_exit();
	mtk_cooler_ushutdown_exit();
	//FIH-Thermal-Add+]
