...
#else
		mtkts_bts_printk(
			"[%s] Not support overwrite PUP_R argument, g_RAP_pull_up_R is always %d\n)", __func__, g_RAP_pull_up_R);
#endif
...
static int mtkts_bts_probe(struct platform_device *pdev)
{
	...
	mtkts_bts_printk("statr to read fih_thermal");
	...
	else
	{
		...
		mtkts_bts_printk("%s: read fih_thermal->hw_id = %s\n", __func__, hw_id);
	}


	ret = of_property_read_u32(pdev->dev.of_node, "fih_thermal,pull_up_r",&pull_up_r);
	if(ret)
		mtkts_bts_printk("%s: read pull_up_r fail\n", __func__);
	else
	{
		mtkts_bts_printk("%s: read fih_thermal->pull_up_r = %d, overwrite g_RAP_pull_up_R\n", __func__, pull_up_r);
