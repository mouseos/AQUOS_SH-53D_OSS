...
static long start_gpio_reset_guard_timer(void)
{
	...
	if (!ret)
		...
		//FIH-Sean-format specifies type 'int' but the argument has type 'long int'
...
int ese_reset_gpio_setup(struct p61_spi_platform_data *platform_data)
{
	...
	if (gpio_is_valid(platform_data->rst_gpio)) {
		...
		if (ret < 0) {
			pr_err("reset gpio 0x%x request failed\n", platform_data->rst_gpio);
			return ret;
		}
		...
		if (ret < 0) {
			pr_err("Failed to set the direction of reset gpio=0x%x\n",
						 platform_data->rst_gpio);
			goto fail_gpio;
		}
		pr_debug("Exit : %s\n", __func__);
