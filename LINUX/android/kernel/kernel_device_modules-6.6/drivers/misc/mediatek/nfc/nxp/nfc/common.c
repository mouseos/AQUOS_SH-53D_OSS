		      ...
		      char *devname, char *classname)
{
	...
	//FIH-Sean-too many arguments to function call
	...
	//FIH-Sean-format string is not a string literal (potentially insecure)
...
static int nfc_gpio_info(struct nfc_dev *nfc_dev, unsigned long arg)
{
	...
	if (ret < 0) {
		//FIH-Sean-more '%' conversions than data arguments
...
static int nfc_ioctl_power_states(struct nfc_dev *nfc_dev, unsigned long arg)
{
	...
	if (arg == NFC_POWER_OFF) {
		...
		set_valid_gpio(nfc_gpio->dwl_req, 0);
		nfc_dev->nfc_state = NFC_STATE_NCI;
/* SHARP EXTENDED ADD-Start CUST-ID:0063 */
  } else if (arg == 100) {
    /* sh power on */
    pr_info("%s VEN High\n", __func__);
    gpio_set_ven(nfc_dev, 1);
    ...
    gpio_set_ven(nfc_dev, 0);
    pr_info("%s VEN Low\n", __func__);
/* SHARP EXTENDED ADD-End CUST-ID:0063 */
	} else {
		pr_err("%s: bad arg %lu\n", __func__, arg);
		ret = -ENOIOCTLCMD;
	}
