...
#include "tpd.h"
//#include "met_ftrace_touch.h"//hc rm
...
void tpd_sitronix_enable_em_log(int enable)
{
	if (enable)
		tpd_em_log = 1;
	else
...
EXPORT_SYMBOL(tpd_sitronix_enable_em_log);
...
int tpd_sitronix_log_init(void)
{
...
EXPORT_SYMBOL_GPL(tpd_sitronix_log_init);

void tpd_sitronix_log_exit(void)
{
	misc_deregister(&tpd_debug_log_dev);
}
EXPORT_SYMBOL_GPL(tpd_sitronix_log_exit);
