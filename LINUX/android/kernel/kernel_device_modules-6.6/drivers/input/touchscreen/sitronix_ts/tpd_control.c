...
struct pinctrl_state *eint_as_int, *eint_output0,
        ...
        *en_output0, *en_output1;
const struct of_device_id touch_sitronix_of_match[] = {
	{ .compatible = "sitronix,touch" },
	{},
};
...
void tpd1_get_dts_info(void)
{
	...
	node1 = of_find_matching_node(node1, touch_sitronix_of_match);
	if (node1) {
...
void tpd_sitronix_gpio_power_enable(int level)
{
	mutex_lock(&tpd1_set_gpio_mutex);
	tpd1_DEBUG("%s enable pin level = %d\n", __func__, level);
	if (level)
		pinctrl_select_state(pinctrl1, en_output1);
	else
		pinctrl_select_state(pinctrl1, en_output0);
	mutex_unlock(&tpd1_set_gpio_mutex);
}

EXPORT_SYMBOL(tpd_sitronix_gpio_power_enable);
...
int tpd1_get_gpio_info(struct platform_device *pdev)
{
	...
	if (tpd1_dts_data.tpd1_use_ext_gpio == false) {
		...
		en_output0 =
			pinctrl_lookup_state(pinctrl1, "state_en_output0");
		if (IS_ERR(en_output0)) {
			ret = PTR_ERR(en_output0);
			tpd1_DEBUG("Cannot find pinctrl state_en_output0!\n");
			return ret;
		}
		en_output1 =
			pinctrl_lookup_state(pinctrl1, "state_en_output1");
		if (IS_ERR(en_output1)) {
			ret = PTR_ERR(en_output1);
			tpd1_DEBUG("Cannot find pinctrl state_en_output1!\n");
			return ret;
		}
	}
	tpd1_DEBUG("[tpd1%d] mt_tpd1_pinctrl----------\n", pdev->id);
...
static int __init tpd_sitronix_device_init(void);
static void __exit tpd_sitronix_device_exit(void);
static int tpd_sitronix_probe(struct platform_device *pdev);
static int tpd_sitronix_remove(struct platform_device *pdev);
...
static struct platform_driver tpd_sitronix_driver = {
	.remove = tpd_sitronix_remove,
	.shutdown = NULL,
	.probe = tpd_sitronix_probe,
	.driver = {
			.name = CTP_NAME,
			.pm = &tpd1_pm_ops,
			.owner = THIS_MODULE,
			.of_match_table = touch_sitronix_of_match,
	},
};
...
*/
	...
	tpd1->dev->name = CTP_NAME;
	tpd1->dev->phys = "ilitek/input0";
...
{
	tpd1_DEBUG("MediaTek sitronix touch panel driver init\n");
	if (platform_driver_register(&tpd_sitronix_driver) != 0){
...
static int __init tpd_sitronix_device_init(void)
{
	...
	sitronix_touch_driver_init();
	//tpd1_log_init();
	tpd1_init_workqueue = create_singlethread_workqueue(CTP_NAME);
...
static void __exit tpd_sitronix_device_exit(void)
{
	...
	platform_driver_unregister(&tpd_sitronix_driver);
	tpd1_DEBUG("Touch exit done\n");
}

late_initcall(tpd_sitronix_device_init);
module_exit(tpd_sitronix_device_exit);
