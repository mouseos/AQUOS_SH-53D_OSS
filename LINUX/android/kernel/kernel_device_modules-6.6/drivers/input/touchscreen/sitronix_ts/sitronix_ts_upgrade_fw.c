...
static int st_v2_read_bytes(st_u16 addr, unsigned char *rxbuf, int len)
{
	...
	tmp_buf = kmalloc(len * sizeof(unsigned char), GFP_KERNEL);
	ret = st_v2_len_check(addr, 1, tmp_buf, rxbuf, len);
	kfree(tmp_buf);
...
static int st_v2_write_bytes(st_u16 addr, unsigned char *txbuf, int len)
{
	...
	tmp_buf = kmalloc(len * sizeof(unsigned char), GFP_KERNEL);
	ret = st_v2_len_check(addr, 0, txbuf, tmp_buf, len);
	kfree(tmp_buf);
...
int stx_clr_crash_msg(void)
{
	...
	if (buf[0]== 0x0 && buf[1]== 0x0) {
		STX_ERROR("%s before buf0-1 = 0x0,0x0, I2C bus get problem!", __func__);
		return -1;
	}
...
static int st_v2_isp_off(void)
{
	...
	//stx_clr_crash_msg();
	if (stx_clr_crash_msg() == -1) {
		STX_ERROR("stx_clr_crash_msg error");
		return -1;
	}
...
static int st_v2_flash_write_page(unsigned char *Buf, unsigned short PageNumber)
{
	...
	for (i = 0; i < ST_FLASH_PAGE_SIZE / ST_ISP_BLOCK_SIZE; i++)
	{
		...
		if (st_v2_cmd_write(PacketData, 8, 8) < 0)
			return -1;
	}
	return 0;
}
...
static int stx_try_i2c_addr(void)
{
	int ret = 0;
	int i = 0;
	int length;
	unsigned char buffer[8];
	unsigned char ADDR_LIST[] = {0x55,0x38,0x70,0x60};
	stx_gpts.host_if->default_i2c_flag = 0;

	ret = stx_i2c_read_bytes(0x1, buffer, 2);
	if (ret < 0){
		STX_ERROR("i2c read status reg error (%d),client->addr = 0x%x\n", ret,stx_gpts.client->addr);
		stx_gpts.host_if->default_i2c_flag = 1;
		stx_gpts.host_if->default_i2c = stx_gpts.client->addr;
		
		length = sizeof(ADDR_LIST)/sizeof(ADDR_LIST[0]);
//		printk("length = %d \n",length);
		for(i = 0;i < length;i++)
		{
			stx_gpts.client->addr = ADDR_LIST[i];
			ret = stx_i2c_read_bytes(0x1, buffer, 2);
			if (ret < 0){
				STX_ERROR("i2c read status reg error (%d),client->addr = 0x%x\n", ret,stx_gpts.client->addr);
			}else if(((buffer[0]&0x0F) == 0x06) && (st_check_chipid() == 0))
			{
				STX_INFO("Check BT ChipId Success,it's sitronix");
				break;
			}
		}
		if(i == length)
		{
			stx_gpts.host_if->default_i2c_flag = 0;
			stx_gpts.client->addr = stx_gpts.host_if->default_i2c;
			STX_INFO("i2c transfer error");
			ret = -1;
		}
		
	}else{
		STX_INFO("status reg = 0x%x\n", buffer[0]);
	}

	return ret;
}

static int stx_restore_i2c_addr(void)
{
	if(stx_gpts.host_if->default_i2c_flag == 1)
	{
		stx_gpts.host_if->default_i2c_flag = 0;
		stx_gpts.client->addr = stx_gpts.host_if->default_i2c;
	}
...
static int st_select_ispid(void)
{
	...
	if((fw_check[0] == 0x50) &&(fw_check[1] == 0x49) &&(fw_check[2] == 0x44) && (len > 0))
	{
		...
		STX_ERROR("ispid fail 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x",\
			fw_check[0],fw_check[1],fw_check[2],fw_check[3],fw_check[6],fw_check[7],fw_check[8],fw_check[9],fw_check[10]);
		return -1;
	}
	return -1;
}
...
static int st_select_fw_by_id(void)
{
	...
	else if (status != 0x6)
	{
		...
		stx_i2c_write_bytes(buf + 4, 2);
		st_msleep(1);
		//STX_DEBUG("customer ids: %x %x %x %x ,buf %x %x %x %x ",id[0],id[1],id[2],id[3],buf[0],buf[1],buf[2],buf[3]);
		...
		for (i = 0; i < idlen; i++)
		{
			if (id[0] == id_buf[i * 4] && id[1] == id_buf[i * 4 + 1] && id[2] == id_buf[i * 4 + 2] && id[3] == id_buf[i * 4 + 3])
			{
				isFindID = 1;
//				st_replace_fw_by_id(i);
	...
	else
	{
		...
		else
		{
			...
			for (i = 0; i < idlen; i++)
			{
				STX_DEBUG("customer ids: %x %x %x %x", fw_check[0 + id_off[i]], fw_check[1 + id_off[i]], fw_check[2 + id_off[i]], fw_check[3 + id_off[i]]);
				if (fw_check[0 + id_off[i]] == id_buf[i * 4] && fw_check[1 + id_off[i]] == id_buf[i * 4 + 1] && fw_check[2 + id_off[i]] == id_buf[i * 4 + 2] && fw_check[3 + id_off[i]] == id_buf[i * 4 + 3])
				{
					isFindID = 1;
//					st_replace_fw_by_id(i);
					return i;
			...
			if (0 == isFindID)
			{
				...
				for (i = 0; i < idlen; i++)
				{
					...
					if (fw_check[0 + id_off[i]] == id_buf[i * 4] && fw_check[1 + id_off[i]] == id_buf[i * 4 + 1] && fw_check[2 + id_off[i]] == id_buf[i * 4 + 2] && fw_check[3 + id_off[i]] == id_buf[i * 4 + 3])
					{
						STX_INFO("Find ID in page 0 ,customer ids: %x %x %x %x ", id_buf[i * 4], id_buf[i * 4 + 1], id_buf[i * 4 + 2], id_buf[i * 4 + 3]);
						isFindID = 1;
//						st_replace_fw_by_id(i);
...
static int st_get_device_status_reg(unsigned char *device_status)
{
	int ret = 0;
	...
	{
		STX_INFO("status reg = 0x%x", buffer[0]);
	}
	*device_status = buffer[0];

	return 0;
}

#ifdef STX_TRIMDATA_FUNC
...
#define		AFE_CFIG_TAB_OFFSET_OFFSET		14
#define		TRIM_PHASE_DATA_OFFSET		0X1A000
#define		FREQ_COUNT_MAX		12
#define		PHASE_LENGTH_MAX		12

struct trim_phase_t{
	unsigned char freqValue;
	unsigned char oriMuPhase;
	unsigned char oriScPhase;
	unsigned char oriScTxPhase;
	char MuPhaseOffset;
	char ScPhaseOffset;
	char ScTxPhaseOffset;
};

static UINT32 RemainderCal(UINT32 poly, UINT32 dat, UINT8 polysize, UINT8 datasize)
{
...
static unsigned short Crc16Cal(unsigned char* Buff, int size)
{
	const unsigned char POLYSIZE = 16, DATASIZE = 8;
	unsigned long crc;
	int ind;
	crc = Buff[0];
	for(ind = 1; ind<size; ind++){
		crc = (crc<<DATASIZE) | Buff[ind];
		crc = RemainderCal(CRC16_POLY, crc, POLYSIZE, (unsigned char)(POLYSIZE+DATASIZE));// 16 bit CRC
	}
	crc = crc<<DATASIZE; // CRC16, the data should be shifted left 16bits. Shift 8 bit first
	crc = RemainderCal(CRC16_POLY, crc, POLYSIZE, (unsigned char)(POLYSIZE+DATASIZE));
...
static int stx_updata_fw_table_in_isp(unsigned char *cfg)
{
	unsigned short AfeCfigTabOffset = 0;
	unsigned char buf[100] = {0};
	unsigned char trimTag[] = {'T','R','M','2'};
	unsigned char freqCnt = 0;
	int i,j;
	unsigned char group = 0;
	unsigned short cfgChecksum = 0;
	unsigned short trimChecksum = 0;
	int cfgLen = 0;
	int trimLen = 0;

	struct trim_phase_t trimData[FREQ_COUNT_MAX];

//	cfgLen = length;
	group = sizeof(trimData[0]);

	AfeCfigTabOffset = (cfg[AFE_CFIG_TAB_OFFSET_OFFSET] << 8) + cfg[AFE_CFIG_TAB_OFFSET_OFFSET + 1];
	STX_INFO("the num of one group = %d,AfeCfigTabOffset = 0x%x",group,AfeCfigTabOffset);

//	if(cfgLen != 0x83E)
//	{
//		STX_DEBUG("Trim CFG cfgLen = 0x%x",cfgLen);
	cfgLen = 0x83E;
//	}

	if(st_v2_flash_read_offset(buf, TRIM_PHASE_DATA_OFFSET, sizeof(buf)) < 0)
	{
		STX_ERROR("read trimData fail ");
		return -1;
	}
	STX_INFO("buf[0 ~ 6] = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x",buf[0],buf[1],buf[2],\
			buf[3],buf[4],buf[5],buf[6]);
	if((buf[0] == trimTag[0])&&(buf[1] == trimTag[1])&&(buf[2] == trimTag[2])&&(buf[3] == trimTag[3]))
	{
		freqCnt = buf[4];
		trimChecksum = 0;
		trimLen = 5 + group*freqCnt;
		ChecksumCalculation(&trimChecksum, buf, trimLen);

		STX_INFO("freqCnt = %d,trimLen = %d\n",freqCnt,trimLen);

		if((trimChecksum & 0xFF) != buf[trimLen])
		{
			STX_ERROR("trimChecksum Error, Cancel trim data");
			return -1;
		}
		for(i = 0;i < freqCnt;i++)
		{
			STX_INFO("buf[5 ~ 11] = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x",buf[i*group + 5],buf[i*group + 6],buf[i*group + 7],\
			buf[i*group + 8],buf[i*group + 9],buf[i*group + 10],buf[i*group + 11]);

			trimData[i].freqValue = buf[i*group + 5];
			trimData[i].oriMuPhase = buf[i*group + 6];
			trimData[i].oriScPhase = buf[i*group + 7];
			trimData[i].oriScTxPhase = buf[i*group + 8];

			if(buf[i*group + 9] & 0x80)
				trimData[i].MuPhaseOffset = buf[i*group + 9] - 0x100;
			else
				trimData[i].MuPhaseOffset = buf[i*group + 9];

			if(buf[i*group + 10] & 0x80)
				trimData[i].ScPhaseOffset = buf[i*group + 10] - 0x100;
			else
				trimData[i].ScPhaseOffset = buf[i*group + 10];

			if(buf[i*group + 11] & 0x80)
				trimData[i].ScTxPhaseOffset = buf[i*group + 11] - 0x100;
			else
				trimData[i].ScTxPhaseOffset = buf[i*group + 11];

			for(j =0;j < FREQ_COUNT_MAX;j++)
			{
				if(trimData[i].freqValue == cfg[AfeCfigTabOffset + 1 + PHASE_LENGTH_MAX*j])
				{
					if(trimData[i].oriMuPhase != 0xFF)
						cfg[AfeCfigTabOffset + 2 + PHASE_LENGTH_MAX*j] = trimData[i].oriMuPhase + trimData[i].MuPhaseOffset;

					if(trimData[i].oriScPhase != 0xFF)
						cfg[AfeCfigTabOffset + 3 + PHASE_LENGTH_MAX*j] = trimData[i].oriScPhase + trimData[i].ScPhaseOffset;

					if(trimData[i].oriScTxPhase != 0xFF)
						cfg[AfeCfigTabOffset + 4 + PHASE_LENGTH_MAX*j] = trimData[i].oriScTxPhase + trimData[i].ScTxPhaseOffset;
				}
			}
		}
		cfgChecksum = Crc16Cal(cfg, (cfgLen - 2));

		cfg[cfgLen - 2] = (cfgChecksum & 0xFF00) >> 8;
		cfg[cfgLen - 1] = cfgChecksum & 0xFF;
	}

	STX_INFO("Exit the trim func");
	return 0;
}
#endif
...
int st_upgrade_fw(void)
{
	...
	rt = st_get_device_status_reg(&status);
	if(rt < 0)
		STX_ERROR("get status error in upgrade");
	else if(((status & 0x0F) == 0x6)||((status & 0xF0) != 0x0))
	{
		powerfulWrite = 1;
		STX_INFO("powerfullWrite set %d",powerfulWrite);
	}
	STX_INFO("upgrade status 0x%x",status);
...
#ifdef STX_TRIMDATA_FUNC
	stx_updata_fw_table_in_isp(cfg_buf);
#endif

	if (powerfulWrite == 0 && (fwSize != 0 || cfgSize != 0))
	{
#ifdef ST_UPGRADE_VERSION_CTRL
		CfgTabOffset = (cfg_buf[CFG_TAB_VER_OFF] << 8) + cfg_buf[CFG_TAB_VER_OFF + 1];

		customer_ver = cfg_buf[CfgTabOffset];
		if (customer_ver <= stx_gpts.ts_dev_info.fw_version[0])
		{
			STX_ERROR("The dump_file version:%d is lower than the TP:%d ,cancel upgrade",customer_ver,stx_gpts.ts_dev_info.fw_version[0]);
			goto ST_ISP_OFF;
		}
#endif
		//check fw and cfg
...
ST_ISP_OFF:
	rt = st_v2_isp_off();
	if(rt == -1) {
		STX_ERROR("st_v2_isp_off error");
		return -1;
	}
ST_IRQ_ON:
	st_irq_on();

	stx_restore_i2c_addr();
...
static int stx_upgrade_fw_direct(unsigned char *pfw_buf, unsigned char *pcfg_buf)
{
	int rt = 0;
	int fwSize = 0;
	int cfgSize = 0;
...
//		rt = -1;
	...
	fwSize = ST_FW_LEN;
	cfgSize = ST_CFG_LEN;
//	fwSize = sizeof(pfw_buf);
//	cfgSize = sizeof(pcfg_buf);
	STX_INFO("fwSize 0x%X,cfgsize 0x%X", fwSize, cfgSize);

	if (fwSize != 0)
	{
		fwInfoOff = st_get_fw_info_offset(fwSize, pfw_buf);
		//		fwInfoOff = st_get_fw_info_offset(fwSize,pfw_buf);
		if (fwInfoOff < 0)
		{
			fwSize = 0;
		...
		fwInfoLen = pfw_buf[fwInfoOff] * 0x100 + pfw_buf[fwInfoOff + 1] + 2 + 3;
		STX_INFO("fwInfoOff 0x%X , fwInfoLen 0x%x", fwInfoOff, fwInfoLen);
	}
...
//	cfgSize = min(cfgSize, ST_CFG_LEN);
	if (cfgSize != 0)
	{
		if (pcfg_buf[0] != 0x43 || pcfg_buf[1] != 0x46 || pcfg_buf[2] != 0x54 || pcfg_buf[3] != 0x31)
		{
			STX_ERROR("pcfg_buf error (invalid CFG)");
			goto ST_ISP_OFF;
		}
	}
...
#ifdef STX_TRIMDATA_FUNC
	stx_updata_fw_table_in_isp(pcfg_buf);
#endif
...
//			rt = -1;
		...
		if (fwSize != 0)
		{
			if (0 == st_compare_array(fw_check, pfw_buf + fwInfoOff, fwInfoLen))
			{
				STX_INFO("fw compare :same");
				fwSize = 0;
			}
		...
		{
			st_v2_flash_read_offset(fw_check, ST_CFG_OFFSET, ST_CFG_LEN);
			if (0 == st_compare_array(fw_check, pcfg_buf, cfgSize))
			{
				STX_INFO("cfg compare :same");
				cfgSize = 0;
			}
	...
	if (cfgSize != 0)
		st_v2_flash_write(pcfg_buf, ST_CFG_OFFSET, cfgSize);

	if (fwSize != 0)
	{
		//write page 1 ~ N
		st_v2_flash_write(pfw_buf + ST_FLASH_PAGE_SIZE, ST_FLASH_PAGE_SIZE, fwSize - ST_FLASH_PAGE_SIZE);
		//write page 0
		st_v2_flash_write(pfw_buf, 0, ST_FLASH_PAGE_SIZE);
	}
...
//	int cfg_size;
	...
	if (fw != NULL)
	{
		...
		STX_INFO("%s: pCfg_Buf [0] = %02X, [1] = %02X, [2] = %02X, [3] = %02X", __func__, pCfg_Buf[0], pCfg_Buf[1], pCfg_Buf[2], pCfg_Buf[3]);
	}
	else
	{
		STX_ERROR("%s: error fw = NULL", __func__);
...
//	fw_size = ST_FW_LEN;
//	cfg_size = ST_CFG_LEN;
	stx_upgrade_fw_direct(pFw_Buf, pCfg_Buf);
...
int st_upgrade_fw_handler(void *unused)
{
    ...
    if(status < 0)
        STX_ERROR("%s: upgrade firmware fail\n", __func__);
    else
        STX_ERROR("%s: upgrade firmware ok\n", __func__);
//	st_print_version(&stx_gpts);
