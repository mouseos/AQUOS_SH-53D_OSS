...
#define STX_ADDR_LENGTH         (1)

static s32 _do_i2c_read(struct i2c_client *client, struct i2c_msg * msgs, u16 addr, u8 * buffer, s32 len)
{
    s32 ret = -1;
    s32 pos = 0;
    s32 data_length = len;
    ...
    data = (u8 *) kmalloc(IIC_MAX_TRANSFER_SIZE < (len + STX_ADDR_LENGTH) ? IIC_MAX_TRANSFER_SIZE : (len + STX_ADDR_LENGTH), GFP_KERNEL);
    if (data == NULL) {
        return -1;
    }
    msgs[1].buf = data;
    msgs[0].len = STX_ADDR_LENGTH;
    ...
    while (pos != data_length) {
        ...
        if (ret != 2) {
            STX_ERROR("I2c Transfer error! (%d)", ret);
            kfree(data);
            return -1;
        }
...
static s32 _do_i2c_write(struct i2c_client *client, struct i2c_msg * msg, u16 addr, u8 * buffer, s32 len)
{
    s32 ret = -1;
    s32 pos = 0;
    s32 data_length = len - STX_ADDR_LENGTH;
    s32 transfer_length = 0;
    u8 *data = NULL;
    u16 address = addr;
    ...
    while (pos != data_length) {
        if ((data_length - pos) > (IIC_MAX_TRANSFER_SIZE - STX_ADDR_LENGTH)) {
            transfer_length = IIC_MAX_TRANSFER_SIZE - STX_ADDR_LENGTH;
        } else {
            transfer_length = data_length - pos;
        }
        msg->buf[0] = address & 0xFF;
        msg->len = transfer_length + STX_ADDR_LENGTH;
        memcpy(&msg->buf[STX_ADDR_LENGTH], &buffer[pos], transfer_length);
...
#ifdef CONFIG_MTK_I2C_EXTENSION
        ret = _do_i2c_write(client, &msg, buf[0], &buf[1], len);
#else
        ret = i2c_transfer(client->adapter, &msg, 1);
#endif
	if (ret < 0)
...
{
#ifdef CONFIG_MTK_I2C_EXTENSION
	stx_i2c_read(stx_gpts.client, rxbuf, len, addr);
	return 0;
#else
	int ret = 0;
	st_u8 txbuf = addr;
	...
	if (ret < 0)
	{
		STX_ERROR("stx_i2c_read_bytes write 0x%x error (%d)", addr, ret);
		goto unlock;
	}
	udelay(50);
	ret = i2c_master_recv(stx_gpts.client, rxbuf, len);
...
{
#ifdef CONFIG_MTK_I2C_EXTENSION
	stx_i2c_write(stx_gpts.client, txbuf, len);
	return 0;
#else
	int ret = 0;
	if (txbuf == NULL)
	...
	if (ret < 0)
	{
		STX_ERROR("stx_i2c_write_bytes write 0x%x error (%d)", *txbuf, ret);
		mutex_unlock(&i2c_rw_mutex);
		return ret;
	}
	mutex_unlock(&i2c_rw_mutex);
...
static void STChecksumCalculation(unsigned short *pChecksum, unsigned char *pInData, unsigned long Len)
{
	unsigned long i;
...
static bool stx_GetH2DReady(void)
{
	...
	do
	{
		...
		if (ret <= 0)
		{
			STX_ERROR("stx_GetH2DReady: retry(%d) read ready.", retry++);
		}
		else
		{
			if (tmp == 0x01)
	...
	{
		STX_ERROR("stx_ReadIOCommand: Host to device command checksum error.");
		bRet = false;
	}
	else
	{
...
int stx_cmdio_read(int type, int address, unsigned char *buf, int len)
{
	...
	do
	{
		...
		if (inPacket.CmdID == 0x82 && inPacket.CmdData[0] == type)
		{
			...
			{
				/* drop packet */
				STX_ERROR("Invalid Cheksum Expect(0x%02x) Get(0x%02X)", vchksum, inPacket.CmdData[inPacket.ValidDataSize - 1]);
			}
			retry = 0;
		}
		else
		{
			/* drop packet */
			STX_ERROR("Unexpect CmdID (0x%02x) or Type (0x%02X)", inPacket.CmdID, inPacket.CmdData[0]);
			retry++;
			if (retry > 10)
				return -1;
		}
