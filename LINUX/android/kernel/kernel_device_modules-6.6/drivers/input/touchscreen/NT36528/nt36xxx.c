...
#define PINCTRL_STATE_SPI_DEFAULT   "nvt_spi_mode"
#define PINCTRL_STATE_SPI_DISABLE   "nvt_spi_disable"
static struct pinctrl *nvt_pinctrl;
static struct pinctrl_state *nvt_spi_mode_default;
static struct pinctrl_state *nvt_spi_disable;
extern void novatek_touch_vendor_read(char *);
...
extern void novatek_touch_tpfwver_read(char *fw_ver);
extern int novatek_selftest_result_read(void);
...
/*******************************************************
  touchscreen /proc/AllHWList/tp_fw_ver read touch fw version.
*******************************************************/
void novatek_touch_tpfwver_read(char *fw_ver)
{
	NVT_ERR("%s enter\n",__func__);
	memset(fw_ver, 0, sizeof(*fw_ver));
	if(!strcmp(lcm_name, "djn"))
		snprintf(fw_ver, 128, "Novatek_DJN-V%02X\n", ts->fw_ver);
	else
		snprintf(fw_ver, 128, "Novatek_MDT-V%02X\n", ts->fw_ver);
}

void novatek_touch_vendor_read(char *id)
{
	NVT_ERR("%s enter\n",__func__);
	snprintf(id, 128, "NT36528\n");
}
...
char lcm_name[50] = {0};
static const char LCM_TAG_NAME[] = "kernel.lcm=";
...
void lcm_prop(void)
{
	...
	if (!of_chosen)
	{
		NVT_LOG("%s doesn't find chosen node\n", __func__);
		return;
	}
	...
	if (!bootargs)
	{
		NVT_LOG("%s doesn't find bootargs node\n", __func__);
		return;
	}

	ptr = strstr(bootargs, LCM_TAG_NAME);
	if (!ptr)
	{
		NVT_LOG("%s doesn't find kernel.lcm node\n", __func__);
		return;
	}
	...
	display_token = ptr = cmd;
	ptr = strsep(&display_token, "=");
	NVT_LOG("%s: str = (%s)\n", __func__, display_token);

    strncpy(lcm_name, display_token, strlen(display_token));
}
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	NVT_LOG("CONFIG_TOUCHSCREEN_NVT_GKI_RAWDATA_MODULE_DRIVER support\n");
#endif
	...
	nvt_pinctrl = devm_pinctrl_get(
		ts->client->controller->dev.parent);
	if (IS_ERR_OR_NULL(nvt_pinctrl)) {
	...
	/* default spi mode */
	nvt_spi_mode_default = pinctrl_lookup_state(
				nvt_pinctrl, PINCTRL_STATE_SPI_DEFAULT);
	if (IS_ERR_OR_NULL(nvt_spi_mode_default)) {
		ret = PTR_ERR(nvt_spi_mode_default);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",
				PINCTRL_STATE_SPI_DEFAULT, ret);
		nvt_spi_mode_default = NULL;
		goto err_pinctrl_failed;
	}

	nvt_spi_disable = pinctrl_lookup_state(
				nvt_pinctrl, PINCTRL_STATE_SPI_DISABLE);
	if (IS_ERR_OR_NULL(nvt_spi_disable)) {
		ret = PTR_ERR(nvt_spi_disable);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",
				PINCTRL_STATE_SPI_DISABLE, ret);
		nvt_spi_disable = NULL;
		goto err_pinctrl_failed;
	}

	ret = pinctrl_select_state(nvt_pinctrl,
					nvt_spi_mode_default);
    ...
    touch_cb.touch_tpfwver_read = novatek_touch_tpfwver_read;
	touch_cb.touch_selftest_result = novatek_selftest_result_read;
	touch_cb.touch_vendor_read = novatek_touch_vendor_read;
...
{
	uint8_t charge_cmd = ts->charge_flag ? CHARGE_ON : CHARGE_OFF;
	int ret = 0;
	...
	ret = pinctrl_select_state(nvt_pinctrl,
					nvt_spi_mode_default);
	if(ret)
		NVT_LOG("failed to pinctrl select nvt_spi_mode_default\n");
	...
	if (nvt_update_firmware(BOOT_UPDATE_FIRMWARE_NAME)) {
		...
		nvt_change_mode(charge_cmd);
		NVT_LOG("Set charge mode to %d", ts->charge_flag);
	}
#endif
