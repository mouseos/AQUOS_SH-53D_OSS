...
static int32_t update_firmware_request(char *filename)
{
	uint8_t retry = 0;
	int32_t ret = 0;
	char firmware_name[50] = {0};

	if (NULL == filename) {
		return -ENOENT;
	}
	if(strstr(filename,MP_UPDATE_FIRMWARE_NAME)) {
		if(!strcmp(lcm_name, "djn")) // main source
			strcpy(firmware_name, MP_UPDATE_FIRMWARE_NAME);
		else
			strcpy(firmware_name, "novatek_ts_mdt_sx4_mp.bin");
	} else {
		if(!strcmp(lcm_name, "djn")) // main source
			strcpy(firmware_name, BOOT_UPDATE_FIRMWARE_NAME);
		else
			strcpy(firmware_name, "novatek_ts_mdt_sx4_fw.bin");
	}

	while (1) {
		NVT_LOG("filename is %s\n", firmware_name);

		ret = request_firmware(&fw_entry, firmware_name, &ts->client->dev);
		if (ret) {
