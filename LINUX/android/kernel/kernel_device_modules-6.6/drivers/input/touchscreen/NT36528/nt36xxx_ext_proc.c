...
int nvt_charge_mode_enable(struct notifier_block *nb, unsigned long event, void *v) {
	int32_t ret = 0;
	uint8_t charge_cmd = event ? CHARGE_ON : CHARGE_OFF;

	if(ts != NULL) {
		mutex_lock(&ts->lock);

		if (ts->charge_flag != event) {
			ts->charge_flag = event;
			NVT_LOG("Set charge flag to %lu", event);

			ret = nvt_check_fw_reset_state(RESET_STATE_REK);
			if (ret) {
				NVT_ERR("Failed to set charge mode, fw is not ready");
				goto end;
			}

			nvt_change_mode(charge_cmd);
			NVT_LOG("Set charge mode to %s", event ? "CHARGE_ON" : "CHARGE_OFF");
		}

end:
		mutex_unlock(&ts->lock);
	} else {
		NVT_LOG("NVT not found.\n");
	}
	return ret;
}

struct notifier_block touch_notifier = {
	.notifier_call = nvt_charge_mode_enable,
};

EXPORT_SYMBOL(touch_notifier);
