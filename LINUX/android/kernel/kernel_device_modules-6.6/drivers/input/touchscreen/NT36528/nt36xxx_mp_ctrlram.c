...
#define DOZE_FW_RAWDATA_CSV_FILE "/data/local/tmp/DozeFWRawdataTest.csv"
#define DOZE_FW_CC_CSV_FILE "/data/local/tmp/DozeFWCCTest.csv"
#define DOZE_NOISE_TEST_CSV_FILE "/data/local/tmp/DozeNoiseTest.csv"
...
static uint8_t *RecordResult_Doze_FW_Rawdata = NULL;
static uint8_t *RecordResult_Doze_FW_CC = NULL;
...
static int32_t TestResult_Doze_FW_Rawdata = 0;
static int32_t TestResult_Doze_FW_CC = 0;
...
static int32_t *RawData_Doze = NULL;
static int32_t *RawData_Doze_Diff_Min = NULL;
static int32_t *RawData_Doze_Diff_Max = NULL;
static int32_t *RawData_Doze_FW_Rawdata = NULL;
static int32_t *RawData_Doze_FW_CC = NULL;
...
static int nvt_mp_buffer_init(void)
{
	...
	RecordResult_Doze_FW_Rawdata = (uint8_t *)kzalloc(RecordResult_BufSize, GFP_KERNEL);
	if (!RecordResult_Doze_FW_Rawdata) {
		NVT_ERR("kzalloc for RecordResult_Doze_FW_Rawdata failed!\n");
		return -ENOMEM;
	}

	RecordResult_Doze_FW_CC = (uint8_t *)kzalloc(RecordResult_BufSize, GFP_KERNEL);
	if (!RecordResult_Doze_FW_CC) {
		NVT_ERR("kzalloc for RecordResult_Doze_FW_CC failed!\n");
		return -ENOMEM;
	}
	...
	RawData_Doze = (int32_t *)kzalloc(RawData_BufSize, GFP_KERNEL);
	if (!RawData_Doze) {
		NVT_ERR("kzalloc for RawData_Doze failed!\n");
		return -ENOMEM;
	}
	...
	RawData_Doze_FW_Rawdata = (int32_t *)kzalloc(RawData_BufSize, GFP_KERNEL);
	if (!RawData_Doze_FW_Rawdata) {
		NVT_ERR("kzalloc for RawData_Doze_FW_Rawdata failed!\n");
		return -ENOMEM;
	}

	RawData_Doze_FW_CC = (int32_t *)kzalloc(RawData_BufSize, GFP_KERNEL);
	if (!RawData_Doze_FW_CC) {
		NVT_ERR("kzalloc for RawData_Doze_FW_CC failed!\n");
		return -ENOMEM;
	}
...
static void nvt_mp_buffer_deinit(void)
{
	...
	if (RecordResult_Doze_FW_Rawdata) {
		kfree(RecordResult_Doze_FW_Rawdata);
		RecordResult_Doze_FW_Rawdata = NULL;
	}

	if (RecordResult_Doze_FW_CC) {
		kfree(RecordResult_Doze_FW_CC);
		RecordResult_Doze_FW_CC = NULL;
	}

	if (RecordResult_Doze_FW_DiffMax) {
		kfree(RecordResult_Doze_FW_DiffMax);
		RecordResult_Doze_FW_DiffMax = NULL;
	}

	if (RecordResult_Doze_FW_DiffMin) {
		kfree(RecordResult_Doze_FW_DiffMin);
		RecordResult_Doze_FW_DiffMin = NULL;
	}
	...
	if (RawData_Doze_Diff_Min) {
		kfree(RawData_Doze_Diff_Min);
		RawData_Doze_Diff_Min = NULL;
	}

	if (RawData_Doze_Diff_Max) {
		kfree(RawData_Doze_Diff_Max);
		RawData_Doze_Diff_Max = NULL;
	}

	if (RawData_Doze_FW_Rawdata) {
		kfree(RawData_Doze_FW_Rawdata);
		RawData_Doze_FW_Rawdata = NULL;
	}

	if (RawData_Doze_FW_CC) {
		kfree(RawData_Doze_FW_CC);
		RawData_Doze_FW_CC = NULL;
	}
...
#ifdef CONFIG_TOUCHSCREEN_NVT_GKI_RAWDATA_MODULE_DRIVER
	loff_t pos = 0;
#endif
#if TOUCH_KEY_NUM > 0
	int32_t k = 0;
...
#ifndef CONFIG_TOUCHSCREEN_NVT_GKI_RAWDATA_MODULE_DRIVER
	printk("%s:CONFIG_TOUCHSCREEN_NVT_GKI_RAWDATA_MODULE_DRIVER support, skip rawdata write to files\n", __func__);
	if(true)
		return 0;
#endif
...
static int32_t nvt_read_fw_doze_noise(uint8_t *xdata_buf)
{
	uint32_t x = 0;
	uint32_t y = 0;
	int32_t iArrayIndex = 0;
	int32_t frame_num = 0;
#if NVT_SAVE_TEST_DATA_IN_FILE
	uint32_t rawdata_doze_diff_min_offset = 0;
#endif /* #if NVT_SAVE_TEST_DATA_IN_FILE */
	...
	nvt_set_page(ts->mmap->DOZE_NOISE_ADDR);
	xdata_buf[0] = ts->mmap->DOZE_NOISE_ADDR & 0x7f;
	CTP_SPI_READ(ts->client, xdata_buf, Doze_X_Channel * IC_Y_CFG_SIZE * 2 + 1);
	for (y = 0; y < IC_Y_CFG_SIZE; y++) {
		iArrayIndex = y * Doze_X_Channel;
		for (x = 0; x < Doze_X_Channel; x++) {
			RawData_Doze_Diff_Max[iArrayIndex + x] = (int8_t)(xdata_buf[iArrayIndex + x * IC_Y_CFG_SIZE * 2 + 1 + 1]) * 4;
			RawData_Doze_Diff_Min[iArrayIndex + x] = (int8_t)(xdata_buf[iArrayIndex + x * IC_Y_CFG_SIZE * 2 + 1]) * 4;
		}
	}
...
#if NVT_SAVE_TEST_DATA_IN_FILE
	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(RawData_Doze_Diff_Max, Doze_X_Channel, Y_Channel, DOZE_NOISE_TEST_CSV_FILE, 0) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	rawdata_doze_diff_min_offset = Y_Channel * Doze_X_Channel * 7 + Y_Channel * 2;
#else /* #if NVT_SAVE_TEST_DATA_IN_FILE */
	nvt_print_data_array(RawData_Doze_Diff_Max, Doze_X_Channel, Y_Channel);
#endif /* #if NVT_SAVE_TEST_DATA_IN_FILE */

	printk("%s:RawData_Doze_Diff_Min:\n", __func__);
#if NVT_SAVE_TEST_DATA_IN_FILE
	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(RawData_Doze_Diff_Min, Doze_X_Channel, Y_Channel, DOZE_NOISE_TEST_CSV_FILE, rawdata_doze_diff_min_offset) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
#else /* #if NVT_SAVE_TEST_DATA_IN_FILE */
	nvt_print_data_array(RawData_Doze_Diff_Min, Doze_X_Channel, Y_Channel);
#endif /* #if NVT_SAVE_TEST_DATA_IN_FILE */
...
static int32_t nvt_read_fw_doze_rawdata(uint8_t *xdata_buf)
{
	uint32_t x = 0;
	uint32_t y = 0;
	int32_t iArrayIndex = 0;
	...
	nvt_set_page(ts->mmap->DOZE_RAWDATA_ADDR);
	xdata_buf[0] = ts->mmap->DOZE_RAWDATA_ADDR & 0x7f;
	CTP_SPI_READ(ts->client, xdata_buf, Doze_X_Channel * IC_Y_CFG_SIZE * 2 + 1);
	for (y = 0; y < IC_Y_CFG_SIZE; y++) {
		iArrayIndex = y * Doze_X_Channel;
		for (x = 0; x < Doze_X_Channel; x++) {
			RawData_Doze_FW_Rawdata[iArrayIndex + x] = (int16_t)(xdata_buf[iArrayIndex + x * IC_Y_CFG_SIZE * 2 + 1 + 1] << 8) + xdata_buf[iArrayIndex + x * IC_Y_CFG_SIZE * 2+ 1];
		}
	}

	printk("%s:RawData_Doze_FW_Rawdata:\n", __func__);
#if NVT_SAVE_TEST_DATA_IN_FILE
	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(RawData_Doze_FW_Rawdata, Doze_X_Channel, Y_Channel, DOZE_FW_RAWDATA_CSV_FILE, 0) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
#else /* #if NVT_SAVE_TEST_DATA_IN_FILE */
	nvt_print_data_array(RawData_Doze_FW_Rawdata, Doze_X_Channel, Y_Channel);
#endif /* #if NVT_SAVE_TEST_DATA_IN_FILE */
...
static int32_t nvt_read_fw_doze_CC(uint8_t *xdata_buf)
{
	uint32_t x = 0;
	uint32_t y = 0;
	int32_t iArrayIndex = 0;
	...
	nvt_set_page(ts->mmap->DOZE_CC_ADDR);
	xdata_buf[0] = ts->mmap->DOZE_CC_ADDR & 0x7f;
	CTP_SPI_READ(ts->client, xdata_buf, Doze_X_Channel * IC_Y_CFG_SIZE * 2 + 1);
	for (y = 0; y < IC_Y_CFG_SIZE; y++) {
		iArrayIndex = y * Doze_X_Channel;
		for (x = 0; x < Doze_X_Channel; x++) {
			RawData_Doze_FW_CC[iArrayIndex + x] = (int16_t)(xdata_buf[iArrayIndex + x * IC_Y_CFG_SIZE * 2 + 1 + 1] << 8) + xdata_buf[iArrayIndex + x * IC_Y_CFG_SIZE * 2 + 1];
		}
	}

	printk("%s:RawData_Doze_FW_CC:\n", __func__);
#if NVT_SAVE_TEST_DATA_IN_FILE
	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(RawData_Doze_FW_CC, Doze_X_Channel, Y_Channel, DOZE_FW_CC_CSV_FILE, 0) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
#else /* #if NVT_SAVE_TEST_DATA_IN_FILE */
	nvt_print_data_array(RawData_Doze_FW_CC, Doze_X_Channel, Y_Channel);
#endif /* #if NVT_SAVE_TEST_DATA_IN_FILE */
...
int novatek_selftest_result_read(void)
{
	int32_t ResultSelfTest = 1;
	if ((TestResult_Short == 0) && (TestResult_Open == 0) &&
		...
		(TestResult_Pen_Noise == 0) && (TestResult_Doze_Noise == 0) &&
        (TestResult_Doze_FW_Rawdata == 0) && (TestResult_Doze_FW_CC == 0)) {
		ResultSelfTest = 0;
	}
...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
	...
	if (ts->pen_support) {
		if ((TestResult_Short == 0) && (TestResult_Open == 0) &&
			(TestResult_FW_Rawdata == 0) && (TestResult_FW_CC == 0) &&
			(TestResult_Noise == 0) && (TestResult_Doze_Noise == 0) &&
            (TestResult_Doze_FW_Rawdata == 0) && (TestResult_Doze_FW_CC == 0) &&
	...
	if(!strcmp(lcm_name, "djn"))
		nvt_mp_seq_printf(m, "FW Version: %d, Novatek DJN V%02X, NVT PID: 0x%04X\n", fw_ver, ts->fw_ver, nvt_pid);
	else
		nvt_mp_seq_printf(m, "FW Version: %d, Novatek MDT V%02X, NVT PID: 0x%04X\n", fw_ver, ts->fw_ver, nvt_pid);
	nvt_mp_seq_printf(m, "\n");
	...
	if ((TestResult_Doze_Noise == 0) || (TestResult_Doze_Noise == 1)) {
		print_selftest_result(m, TestResult_Doze_FW_DiffMax, RecordResult_Doze_FW_DiffMax, RawData_Doze_Diff_Max, Doze_X_Channel, Y_Channel);
	} else { // TestResult_Noise is -1
		nvt_mp_seq_printf(m, " FAIL!\n");
		...
		if (TestResult_Doze_FW_DiffMin == -1) {
			...
			print_selftest_result(m, TestResult_Doze_FW_DiffMin, RecordResult_Doze_FW_DiffMin, RawData_Doze_Diff_Min, Doze_X_Channel, Y_Channel);
		}
	}
#else /* #if NVT_SAVE_TEST_DATA_IN_FILE */
	if (TestResult_Doze_Noise == 0)
		nvt_mp_seq_printf(m, " PASS.\n");
	else if (TestResult_Doze_Noise == 1)
		nvt_mp_seq_printf(m, " ERROR! Read Data FAIL!\n");
	else
		nvt_mp_seq_printf(m, " FAIL!\n");
	nvt_mp_seq_printf(m, "Doze FW Diff Max");
	...
	print_selftest_result(m, TestResult_Doze_FW_Rawdata, RecordResult_Doze_FW_Rawdata, RawData_Doze_FW_Rawdata, Doze_X_Channel, Y_Channel);

	nvt_mp_seq_printf(m, "Doze FW CC Test");
	print_selftest_result(m, TestResult_Doze_FW_CC, RecordResult_Doze_FW_CC, RawData_Doze_FW_CC, Doze_X_Channel, Y_Channel);
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
    ...
    TestResult_Doze_FW_Rawdata = 0;
    TestResult_Doze_FW_CC = 0;
	TestResult_Short = 0;
	TestResult_Open = 0;
	...
	if (nvt_read_fw_doze_noise((uint8_t *)RawData_Doze) != 0) {
		TestResult_Doze_Noise = 1;
		TestResult_Doze_FW_DiffMax = 1;
		TestResult_Doze_FW_DiffMin = 1;
		goto failed_out;
	} else {
		TestResult_Doze_FW_DiffMax = RawDataTest_SinglePoint_Sub(RawData_Doze_Diff_Max, RecordResult_Doze_FW_DiffMax, Doze_X_Channel, Y_Channel,
											PS_Config_Lmt_Doze_FW_Diff_P, PS_Config_Lmt_Doze_FW_Diff_N);
		TestResult_Doze_FW_DiffMin = RawDataTest_SinglePoint_Sub(RawData_Doze_Diff_Min, RecordResult_Doze_FW_DiffMin, Doze_X_Channel, Y_Channel,
											PS_Config_Lmt_Doze_FW_Diff_P, PS_Config_Lmt_Doze_FW_Diff_N);

		if ((TestResult_Doze_FW_DiffMax == -1) || (TestResult_Doze_FW_DiffMin == -1))
			TestResult_Doze_Noise = -1;
		else
			TestResult_Doze_Noise = 0;
	}

	if (nvt_read_fw_doze_rawdata((uint8_t *)RawData_Doze) != 0) {
		TestResult_Doze_FW_Rawdata = 1;
		goto failed_out;
	} else {
		TestResult_Doze_FW_Rawdata = RawDataTest_SinglePoint_Sub(RawData_Doze_FW_Rawdata, RecordResult_Doze_FW_Rawdata, Doze_X_Channel, Y_Channel,
												PS_Config_Lmt_Doze_FW_Rawdata_P, PS_Config_Lmt_Doze_FW_Rawdata_N);
	}

	nvt_change_mode(NORMAL_MODE);

	if (nvt_read_fw_doze_CC((uint8_t *)RawData_Doze) != 0) {
		TestResult_Doze_FW_CC = 1;
		goto failed_out;
	} else {
		TestResult_Doze_FW_CC = RawDataTest_SinglePoint_Sub(RawData_Doze_FW_CC, RecordResult_Doze_FW_CC, Doze_X_Channel, Y_Channel,
											PS_Config_Lmt_Doze_FW_CC_P, PS_Config_Lmt_Doze_FW_CC_N);
	}
...
int32_t nvt_mp_parse_dt(struct device_node *root, const char *node_compatible)
{
	...
	if (nvt_mp_parse_array(np, "PS_Config_Lmt_Short_Rawdata_P", PS_Config_Lmt_Short_Rawdata_P,
			X_Channel, Y_Channel)) {
		NVT_ERR("PS_Config_Lmt_Short_Rawdata_P parse fail!\n");
		return -1;
	}

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_Short_Rawdata_N", PS_Config_Lmt_Short_Rawdata_N,
			X_Channel, Y_Channel)) {
		NVT_ERR("PS_Config_Lmt_Short_Rawdata_N parse fail!\n");
		return -1;
	}

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_Open_Rawdata_P", PS_Config_Lmt_Open_Rawdata_P,
			X_Channel, Y_Channel)) {
		NVT_ERR("PS_Config_Lmt_Open_Rawdata_P parse fail!\n");
		return -1;
	}

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_Open_Rawdata_N", PS_Config_Lmt_Open_Rawdata_N,
			X_Channel, Y_Channel)) {
		NVT_ERR("PS_Config_Lmt_Open_Rawdata_N parse fail!\n");
		return -1;
	}

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_FW_Rawdata_P", PS_Config_Lmt_FW_Rawdata_P,
			X_Channel, Y_Channel)) {
		NVT_ERR("PS_Config_Lmt_FW_Rawdata_P parse fail!\n");
		return -1;
	}

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_FW_Rawdata_N", PS_Config_Lmt_FW_Rawdata_N,
			X_Channel, Y_Channel)) {
		NVT_ERR("PS_Config_Lmt_FW_Rawdata_N parse fail!\n");
		return -1;
	}

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_FW_CC_P", PS_Config_Lmt_FW_CC_P,
			X_Channel, Y_Channel)) {
		NVT_ERR("PS_Config_Lmt_FW_CC_P parse fail!\n");
		return -1;
	}

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_FW_CC_N", PS_Config_Lmt_FW_CC_N,
			X_Channel, Y_Channel)) {
		NVT_ERR("PS_Config_Lmt_FW_CC_N parse fail!\n");
		return -1;
	}

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_FW_Diff_P", PS_Config_Lmt_FW_Diff_P,
			X_Channel, Y_Channel)) {
		NVT_ERR("PS_Config_Lmt_FW_Diff_P parse fail!\n");
		return -1;
	}

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_FW_Diff_N", PS_Config_Lmt_FW_Diff_N,
			X_Channel, Y_Channel)) {
		NVT_ERR("PS_Config_Lmt_FW_Diff_N parse fail!\n");
		return -1;
	}

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_Doze_FW_Rawdata_P", PS_Config_Lmt_Doze_FW_Rawdata_P,
			Doze_X_Channel, Y_Channel))
		return -1;

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_Doze_FW_Rawdata_N", PS_Config_Lmt_Doze_FW_Rawdata_N,
			Doze_X_Channel, Y_Channel))
		return -1;

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_Doze_FW_CC_P", PS_Config_Lmt_Doze_FW_CC_P,
			Doze_X_Channel, Y_Channel))
		return -1;

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_Doze_FW_CC_N", PS_Config_Lmt_Doze_FW_CC_N,
			Doze_X_Channel, Y_Channel))
		return -1;

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_Doze_FW_Diff_P", PS_Config_Lmt_Doze_FW_Diff_P,
			Doze_X_Channel, Y_Channel))
		return -1;

	if (nvt_mp_parse_array(np, "PS_Config_Lmt_Doze_FW_Diff_N", PS_Config_Lmt_Doze_FW_Diff_N,
			Doze_X_Channel, Y_Channel))
		return -1;
	...
	if (nvt_mp_parse_u32(np, "PS_Config_Diff_Test_Frame", &PS_Config_Diff_Test_Frame)) {
		NVT_ERR("PS_Config_Diff_Test_Frame parse fail!\n");
		return -1;
	}
