...
#define INTERVAL_READ_REG                   5  /* unit:ms */
#define TIMEOUT_READ_REG                    25 /* unit:ms */
...
DEFINE_MUTEX(touch_lock);
int touch_init = 0;
EXPORT_SYMBOL(touch_lock);
EXPORT_SYMBOL(touch_init);

//FIH touch -- start
extern struct fih_touch_cb touch_cb;
extern void touch_tpfwver_read_fts(char *);
extern void fts_vendor_read(char *);
//FIH touch -- end
...
void fts_vendor_read(char *id)
{
    pr_err("%s enter\n",__func__);
    snprintf(id, 128, "FT5446U\n");
}

static void fts_gpio_configure(struct fts_ts_data *ts_data, int enable)
{
    tpd_gpio_power_enable(enable);
    msleep(5);
    tpd_gpio_output(ts_data->pdata->reset_gpio, enable);
}
...
static int fts_ts_probe(struct i2c_client *client)
{
	...
	char fwname[FILE_NAME_LENGTH] = FTS_CONF_NAME;
...
//FIH touch -- start
    touch_cb.touch_tpfwver_read = touch_tpfwver_read_fts;
    touch_cb.touch_vendor_read = fts_vendor_read;
//FIH touch -- end
	fw_status = request_firmware(&fw, fwname, &fts_data->input_dev->dev);
	if(!fw_status)
		FTS_INFO("firmware get(%s) success", fwname);
	touch_init = 1;
	mutex_unlock(&touch_lock);
...
void fts_ts_remove(struct i2c_client *client)
{
	if (fw != NULL) {
        release_firmware(fw);
        fw = NULL;
...
*  Return:
*****************************************************************************/
void ilitek_touch_driver_init(void)
{
    FTS_FUNC_ENTER();
...
void ilitek_touch_driver_exit(void)
{
    FTS_FUNC_ENTER();
...
EXPORT_SYMBOL_GPL(ilitek_touch_driver_init);
EXPORT_SYMBOL_GPL(ilitek_touch_driver_exit);
