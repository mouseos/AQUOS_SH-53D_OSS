...
extern void touch_selftest_fts(void);
extern int touch_selftest_result_read_fts(void);
...
#ifdef CONFIG_TOUCHSCREEN_FTS_GKI_MODULE_DRIVER
    loff_t pos;
#endif
    //mm_segment_t old_fs;
...
{
    printk(KERN_INFO "LCD_ID_DET1 = %d, LCD_ID_DET2 = %d", gpio_get_value(LCD_ID_DET1), gpio_get_value(LCD_ID_DET2));
    if((gpio_get_value(LCD_ID_DET1) == 0) && (gpio_get_value(LCD_ID_DET2) == 1))
    {
        printk(KERN_INFO "LCM CHIP = FL7703N");
        return 1;
    } else {
        printk(KERN_INFO "LCM CHIP = Unknown");
        return -1;
    }
}
...
int touch_selftest_result_read_fts(void)
{
    int result = 0;
...
int fts_test_init(struct fts_ts_data *ts_data)
{
    ...
    touch_cb.touch_selftest = touch_selftest_fts;
    touch_cb.touch_selftest_result = touch_selftest_result_read_fts;
    FTS_TEST_FUNC_EXIT();
