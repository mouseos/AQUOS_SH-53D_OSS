...
void tpd_gpio_power_enable(int level)
{
	mutex_lock(&tpd_set_gpio_mutex);
	TPD_DEBUG("%s enable pin level = %d\n", __func__, level);
	if (level)
...
EXPORT_SYMBOL(tpd_gpio_power_enable);
...
int tpd_get_gpio_info(struct platform_device *pdev)
{
	...
	if (tpd_dts_data.tpd_use_ext_gpio == false) {
		...
		if (IS_ERR(en_output0)) {
			...
			TPD_DMESG("Cannot find pinctrl state_en_output0!\n");
			return ret;
		}
		en_output1 =
		...
		if (IS_ERR(en_output1)) {
			...
			TPD_DMESG("Cannot find pinctrl state_en_output1!\n");
			return ret;
		}
	}
	TPD_DEBUG("[tpd%d] mt_tpd_pinctrl----------\n", pdev->id);
...
static const struct platform_device_id tpd_fts_ids[] = {
	{.name = TPD_DEVICE},
	{}
};
MODULE_DEVICE_TABLE(platform, tpd_fts_ids);
