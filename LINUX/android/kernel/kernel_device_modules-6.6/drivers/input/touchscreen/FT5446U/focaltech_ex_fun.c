    ...
    struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    struct fts_ts_data *ts_data = (struct fts_ts_data *)pde_data(file_inode(filp));
...
{
    struct fts_ts_data *ts_data = (struct fts_ts_data *)pde_data(inode);
...
void touch_tpfwver_read_fts(char *fih_touch)
{
    struct fts_ts_data *ts_data = fts_data;
