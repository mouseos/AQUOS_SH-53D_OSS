		...
		struct mmc_command *cmd)
{
	...
	if (events & MSDC_INT_ACMDRDY) {
		...
		if (events & MSDC_INT_ACMDCRCERR) {
			...
			#if IS_ENABLED(CONFIG_FIH)
			printk("FIHBATTLOG::112\n");  /* [SX3-2087] SD_ERROR_MISC_CMD_CRC_ERROR */
			#endif
		} else if (events & MSDC_INT_ACMDTMO) {
			cmd->error = -ETIMEDOUT;
			host->error |= REQ_STOP_TMO;
			#if IS_ENABLED(CONFIG_FIH)
			printk("FIHBATTLOG::109\n");  /* [SX3-2087] SD_ERROR_MISC_CMD_TIMEOUT */
			#endif
		}
		host->need_tune = true;
		dev_err(host->dev,
		...
		struct mmc_request *mrq, struct mmc_command *cmd)
{
	...
	if (ret) {
		...
		#if IS_ENABLED(CONFIG_FIH)
		printk("FIHBATTLOG::102\n");  /* [SX3-2087] SD_ERROR_WRITE_CMD_TIMEOUT */
		#endif
		return false;
	}
...
static int msdc_ops_switch_volt(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (!IS_ERR(mmc->supply.vqmmc)) {
		if (ios->signal_voltage != MMC_SIGNAL_VOLTAGE_330 &&
			...
			#if IS_ENABLED(CONFIG_FIH)
			printk("FIHBATTLOG::108\n");  /* [SX3-2087] ]SD_ERROR_MISC_UNKNOWN */
			#endif
			return -EINVAL;
		}
	...
	if (host->mrq) {
		...
		if (host->cmd) {
			...
			if (host->mrq->data->flags & MMC_DATA_WRITE) {
				printk("FIHBATTLOG::103\n");  /* [SX3-2087] SD_ERROR_WRITE_DATA_TIMEOUT */
			} else {
				printk("FIHBATTLOG::96\n");  /* [SX3-2087] SD_ERROR_READ_DATA_TIMEOUT */
			}
			#endif
	...
	case MMC_POWER_UP:
		if (!IS_ERR(mmc->supply.vmmc)) {
			...
			if (ret) {
				...
				#if IS_ENABLED(CONFIG_FIH)
				printk("FIHBATTLOG::93\n");  /* [SX3-2087] SD_ERROR_UNKNOWN_OTHER_ERROR */
				#endif
				pr_err("%s:%d: Failed to set vmmc power!\n", __func__, __LINE__);  //jiahao
				return;
			} else
...
#if IS_ENABLED(CONFIG_FIH_TRAY)
static u8 msdc_card_inserted = 0;
int msdc_get_tray(void)
{
	if (msdc_card_inserted) return 1;  /* INSERT */
	return 0;  /* REMOVE */
}
EXPORT_SYMBOL_GPL(msdc_get_tray);
#endif
...
static int msdc_get_cd(struct mmc_host *mmc)
{
	...
	if (mmc->trigger_card_event) {
		if (host->card_inserted) {
			printk("FIHBATTLOG::83\n");  /* [SX3-2087] SD_DETECTED */
		} else {
			printk("FIHBATTLOG::85\n");  /* [SX3-2087] SD_PHY_REMOVED */
		}
	}
#endif

#if IS_ENABLED(CONFIG_FIH_TRAY)
	msdc_card_inserted = host->card_inserted;
#endif
...
int sdcard_hw_reset(struct mmc_card *card)
{
	...
	if (!(host->card_inserted)) {
		...
		ret = -1;
		#if IS_ENABLED(CONFIG_FIH)
		printk("FIHBATTLOG::114\n");  /* [SX3-2087] SD_ERROR_MISC_OTHER_ERROR */
		#endif
		return ret;
	}
...
int msdc_data_timeout_cont_chk(struct msdc_host *host)
{
	if ((host->id == MSDC_SD) &&
		...
		msdc_set_bad_card_and_remove(host);
		#if IS_ENABLED(CONFIG_FIH)
		printk("FIHBATTLOG::110\n");  /* [SX3-2087] SD_ERROR_MISC_DATA_TIMEOUT */
		#endif
		return 1;
	}
...
static int msdc_detect_bad_sd(struct msdc_host *host, u32 condition)
{
	...
	if (++(bad_sd_detecter[condition]) >= bad_sd_tolerance[condition]) {
		...
		ret = -1;
		#if IS_ENABLED(CONFIG_FIH)
		printk("FIHBATTLOG::84\n");  /* [SX3-2087] SD_DETECT_FAILED */
		#endif
	}
...
static int msdc_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	if (ret == -EIO) {
		...
		#if IS_ENABLED(CONFIG_FIH)
		printk("FIHBATTLOG::87\n");  /* [SX3-2087] SD_ERROR_UNKNOWN_UNKNOWN */
		#endif
		return ret;
	}
