...
void sent_info_to_mtk_pd(struct chg_alg_device *alg_dev,u32 event,int info){
	//Prevent kernel panic
	if (!alg_dev || !alg_dev->ops || !alg_dev->ops->sent_jeita_limit) {
		pr_err("[%s] Invalid function pointer!\n", __func__);
		return;
	}
	switch (event) {
	case jeita_bat_cur_limit:
		alg_dev->ops->sent_jeita_limit(info);
		break;
	default:
		break;
	}
...
EXPORT_SYMBOL(sent_info_to_mtk_pd);
