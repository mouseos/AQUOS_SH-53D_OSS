...
extern void send_to_mt6360(u32 event,int value);
/* SX4V-284 JEITA support start */
int jeita_current_limit = 3000000;
...
static int _pd_is_algo_ready(struct chg_alg_device *alg)
{
	...
	case PD_HW_FAIL:
		...
		break;
	case PD_HW_READY:
		/* SX3V-127 , SX4V-293 srart */
		pd_err("[%s] PD_HW_READY, PD not start send_to_mt6360(PD_START,false)\n", __func__);
		send_to_mt6360(PD_START,false);
		/* SX3V-127 , SX4V-293 end */
	...
	case PD_POSTCC:
		/* SX3V-127 , SX4V-293 srart */
		pd_err("[%s] PD_RUN/PD_TUNING/PD_POSTCC , PD start send_to_mt6360(PD_START,true)\n", __func__);
		send_to_mt6360(PD_START,true);
		/* SX3V-127 , SX4V-293 end */
...
int mtk_pd_input_current_protection(struct chg_alg_device *alg, int vbus)
{
	...
	pd_err("%s run: vbus: %d, ibus_limit: %d", __func__, vbus, pd->input_current_limit1);
	return 0;
}
...
static int pd_sc_set_charger(struct chg_alg_device *alg)
{
	...
	/* SX4V-284 JEITA support end */
	pd_err("[%s] %d pd->charging_current1 = %d , pd->input_current1 = %d\n", __func__,__LINE__,pd->charging_current1,pd->input_current1);
	pd->charging_current1 = pd_jeita_compare(pd->charging_current1);
	pd_err("[%s] %d fih set pd->charging_current1 = %d \n", __func__,__LINE__,pd->charging_current1);
	/* SX4V-284 JEITA support end */
...
static int pd_dcs_set_charger(struct chg_alg_device *alg)
{
	...
	if (pd->state == PD_RUN) {
		...
		pd_err("[%s] %d pd_hal_set_input_current = %d \n", __func__,__LINE__,pd->charging_current2);
		pd_hal_set_charging_current(alg,
			CHG2, pd->charging_current2);
	...
	pd_hal_set_input_current(alg,
		CHG1, pd->input_current1);
	pd_err("[%s] %d pd_hal_set_input_current = %d \n", __func__,__LINE__,pd->input_current1);
	pd_hal_set_cv(alg,
		CHG1, pd->cv);
...
static int _pd_stop_algo(struct chg_alg_device *alg)
{
	...
	case PD_RUN:
		...
		pd_hal_set_input_current(alg,
			CHG1, PD_FAIL_CURRENT);
		pd_err("[%s] %d pd_hal_set_charging_current pd_hal_set_input_current = %d \n", __func__,__LINE__,PD_FAIL_CURRENT);
		pd_hal_set_cv(alg,
			CHG1, pd->cv);
		pd->state = PD_HW_READY;
		if (alg->config == DUAL_CHARGERS_IN_SERIES) {
...
/* SX4V-284 JEITA support start */
void __pd_sent_jeita_limit(int cur_limit){

	jeita_current_limit = cur_limit ;
	pr_err("[%s] get jeita limit = %d , PD jeita limit = %d  \n", __func__,cur_limit,jeita_current_limit);
}


int pd_jeita_compare(int pd_limit){
	pr_err("[%s] pd_limit = %d , jeita_current_limit = %d \n",__func__,pd_limit,jeita_current_limit);
	if(pd_limit>jeita_current_limit){
		pr_err("[%s] return jeita_current_limit \n",__func__);
		return jeita_current_limit;
	}else{
		pr_err("[%s] return pd_limit \n",__func__);
		return pd_limit;
	}
}
...
static struct chg_alg_ops pd_alg_ops = {
	...
	.sent_jeita_limit = __pd_sent_jeita_limit,
};
