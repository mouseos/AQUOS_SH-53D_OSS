...
extern int full_capacity_now;
	...
	#define battery_full_capacity 4860
#elif defined(CONFIG_FIH_SX3)
	#define battery_full_capacity 3620
#endif
/* FIH function declaration */
static void battery_deterioration_calculate(void);

static struct mtk_charger *finfo;

static struct customized_charging_status_source sharp_charging_en;
...
/**** [SX3-696] global variants: START ****/
#define	TYPEC_OTP_ALERT_THRESHOLD		55000
#define	TYPEC_OTP_CANCEL_THRESHOLD		45000

#define	TYPEC_OTP_TEMP_MIN				-40000
#define	TYPEC_OTP_TEMP_MAX				125000

#define CC_RECOVERY_TIME_INTERVAL_SEC	60

static __s32 get_ts_temp(struct device *dev);
int set_tcpc_to_charger(struct tcpc_device *tcpc);
int fih_disable_cc(struct mtk_charger *pinfo, bool disable);

struct S_cc_safety *gcs;
...
/******** [SX3-696][REQ][Charger][SW-PMIC-0043]Disable charger when USB TypeC connector becomes too hot ---- START ********/
	...
	chr_err("[TYPEC_OTP] boot_mode: %d\n", boot_mode);

	TTemperature = get_ts_temp(dev);
	if (gcs->cc_safety_level_test_enable) {
		if (gcs->cc_safety_level_test_val >= TYPEC_OTP_TEMP_MIN && gcs->cc_safety_level_test_val <= TYPEC_OTP_TEMP_MAX) {
			chr_err("[TYPEC_OTP] TypeC temperature is changed forcedly from %d to %d\n", TTemperature, gcs->cc_safety_level_test_val);
			TTemperature = gcs->cc_safety_level_test_val;
		} else
			chr_err("[TYPEC_OTP] Test temperature is over spec (%d ~ %d). Keep original temperature...\n", TYPEC_OTP_TEMP_MIN, TYPEC_OTP_TEMP_MAX);
	}

	if (TTemperature >= TYPEC_OTP_ALERT_THRESHOLD) {
		/* 8 = KERNEL_POWER_OFF_CHARGING_BOOT */
		/* 9 = LOW_POWER_OFF_CHARGING_BOOT */
		if (boot_mode == 8 || boot_mode == 9) {
			chr_err("[TYPEC_OTP] In power-off charging mode, reboot DUT because usb connector overheat occurs...\n");
			kernel_restart("CC_SAFETY");
		}

		info->battery_temp = info->data.temp_t4_thres + 1;
		if (gcs->cc_safety_level == 0) {
			chr_err("[TYPEC_OTP] Start usb over-temperature protection %s!\n", ret == 0 ? "successfully" : "fail");
			ret = fih_disable_cc(info, true);
			gcs->cc_safety_level = 3;
			printk("FIHBATTLOG::%d\n", SH_BATTLOG_DETECT_USB_HIGH_TEMP);
			pr_notice("[TYPEC_OTP][%s] write battery_log: %d\n", __func__, SH_BATTLOG_DETECT_USB_HIGH_TEMP);
			fih_send_charger_info(thermal_usb_charging, false);
			ktime_now = ktime_get_boottime();
			gcs->cc_safety_ts64_otp_enable = ktime_to_timespec64(ktime_now);
		}
	}
	else if (TTemperature >= TYPEC_OTP_CANCEL_THRESHOLD && TTemperature < TYPEC_OTP_ALERT_THRESHOLD) {
		if (gcs->cc_safety_level == 3)
			ret = fih_disable_cc(info, true);
		else
			ret = fih_disable_cc(info, false);
		chr_err("[TYPEC_OTP][45-55] Keep early status to %s CC %s\n", gcs->cc_safety_level == 3 ? "disable" : "enable", ret == 0 ? "successfully" : "fail");
	}
	else {
		if (gcs->cc_safety_level == 3) {
			ret = fih_disable_cc(info, false);
			gcs->cc_safety_level = 0;
			printk("FIHBATTLOG::%d\n", SH_BATTLOG_RELEASE_USB_HIGH_TEMP);
			pr_notice("[TYPEC_OTP][%s] write battery_log: %d\n", __func__, SH_BATTLOG_RELEASE_USB_HIGH_TEMP);
			fih_send_charger_info(thermal_usb_charging, true);
			chr_err("[TYPEC_OTP] Stop usb over-temperature protection %s!\n", ret == 0 ? "successfully" : "fail");
		}
	}

	print_cc_safety_state(gcs, "mtk_charger");
	chr_err("[TYPEC_OTP] TS Temp: %05d | info->battery_temp: %d\n", TTemperature, info->battery_temp);
/******** [SX3-696][REQ][Charger][SW-PMIC-0043]Disable charger when USB TypeC connector becomes too hot ---- END   ********/
...
static DEVICE_ATTR(cc_safety_level, 0644, show_cc_safety_level, NULL);


static ssize_t show_cc_safety_level_test_enable(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", gcs->cc_safety_level_test_enable);
}

static ssize_t store_cc_safety_level_test_enable(struct device *dev,
		struct device_attribute *attr, const char *buf,  size_t size)
{
	...
	if (buf != NULL && size != 0) {
		if (kstrtouint(buf, 10, &reg) == 0) {
			gcs->cc_safety_level_test_enable = reg;
			pr_notice("[TYPEC_OTP][%s] -------- Set cc_safety_level_test_enable: %d\n", __func__, gcs->cc_safety_level_test_enable);
			if (gcs->cc_safety_level_test_enable == 0 && gcs->cc_safety_level == 3) {
				// Enabled CC immediately if finished test and cc_safety_level is enabled.
				// Otherwise we need to wait 1 min even set cc_safety_level_test_enable to 0.
				gcs->cc_safety_level = 0;
				printk("FIHBATTLOG::%d\n", SH_BATTLOG_RELEASE_USB_HIGH_TEMP);
				pr_notice("[TYPEC_OTP][%s] write battery_log: %d\n", __func__, SH_BATTLOG_RELEASE_USB_HIGH_TEMP);
				fih_send_charger_info(thermal_usb_charging, true);
				pinfo = dev->driver_data;
				ret = fih_disable_cc(pinfo, false);
				pr_notice("[TYPEC_OTP][%s] -------- gcs->cc_safety_level is %s -> enabled cc %s\n", __func__, gcs->cc_safety_level == 0 ? "disabled" : "enabled", ret == 0 ? "successfully" : "fail");
			}
		}
		else
			pr_err("[TYPEC_OTP][%s] read buffer to uint fail!\n", __func__);
	}
	else
		pr_err("[TYPEC_OTP][%s] buf is NULL or size is zero!\n", __func__);

	return size;
}
static DEVICE_ATTR(cc_safety_level_test_enable, 0644, show_cc_safety_level_test_enable, store_cc_safety_level_test_enable);

static ssize_t show_cc_safety_level_test_val(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	pr_info("[TYPEC_OTP] -------- Get cc_safety_level_test_val: %d\n", gcs->cc_safety_level_test_val);
	return sprintf(buf, "%d\n", gcs->cc_safety_level_test_val);
}

static ssize_t store_cc_safety_level_test_val(struct device *dev,
		struct device_attribute *attr, const char *buf,  size_t size)
{
	unsigned int temperature = 0;

	if (buf != NULL && size != 0) {
		if (kstrtouint(buf, 10, &temperature) == 0) {
			gcs->cc_safety_level_test_val = temperature * 1000;
			pr_info("[TYPEC_OTP] -------- Set cc_safety_level_test_val: %d\n", gcs->cc_safety_level_test_val);
		}
		else
			pr_err("[TYPEC_OTP] read buffer to uint fail!\n");
	}
	else
		pr_err("[TYPEC_OTP] buf is NULL or size is zero!\n");

	return size;
}
static DEVICE_ATTR(cc_safety_level_test_val, 0644, show_cc_safety_level_test_val, store_cc_safety_level_test_val);

// [DEBUG NODE] to test cc disable and enable features
static ssize_t store_cc_forcibly_enable(struct device *dev,
		struct device_attribute *attr, const char *buf,  size_t size)
{
	unsigned int reg = 0;
	int ret = 0;
	...
	if (buf != NULL && size != 0) {
		if (kstrtouint(buf, 10, &reg) == 0) {
			...
			if (reg == 1)
				...
				ret = fih_disable_cc(pinfo, true);
			pr_info("[TYPEC_OTP] -------- %s CC forcibly %s!\n", reg == 1 ? "enables" : "disables", ret == 0 ? "succeed" : "fail");
		}
		else
			pr_err("[TYPEC_OTP] read buffer to uint fail!\n");
	}
...
static DEVICE_ATTR(cc_forcibly_enable, 0644, NULL, store_cc_forcibly_enable);

// [DEBUG NODE] to test reboot DUT from kernel
static ssize_t store_reboot_forcibly(struct device *dev,
		struct device_attribute *attr, const char *buf,  size_t size)
{
	unsigned int reg = 0;
	//int ret;
	pr_debug("[TYPEC_OTP] buf is %s and size is %zu\n", buf, size);
	if (buf != NULL && size != 0) {
		if (kstrtouint(buf, 10, &reg) == 0) {
			if (reg == 1) {
				pr_info("[TYPEC_OTP] -------- to reboot DUT...\n");
				kernel_restart("CC_SAFETY");
			}
			else
				pr_warn("[TYPEC_OTP] -------- to reboot fail due to unknown argument (%d)!\n", reg);
		}
		else
			pr_err("[TYPEC_OTP] read buffer to uint fail!\n");
	}
...
static DEVICE_ATTR(reboot_forcibly, 0644, NULL, store_reboot_forcibly);
/**** [SX3-696][REQ][Charger][SW-PMIC-0043]Disable charger when USB TypeC connector becomes too hot ---- END   ****/

/**** [SX3-746][SX3][Thermal]Implement thermal node for item4_charging and item7_USB_FPC ---- START ****/
#define BTS_RAP_PULL_UP_R               26100	/* 100K, pull up resister */
#define BTS_RAP_PULL_UP_VOLTAGE         1800	/* 1.800V ,pull up avg voltage */
#define VREF_TS_PULL_UP_VOLTAGE_UBOUND  1799	/* 1.799V ,pull up max voltage */
...
static __s32 mtkts_bts_thermistor_conver_temp(__s32 Res)
{
	...
	long numerator = 0, numerator_1000 = 0, denominator = 0;
...
#else
		// Original mtk formula will result in postive value if input voltage > 1728 mV (voltage of -10 degree)
		//TAP_Value = (((Res - RES2) * TMP1) + ((RES1 - Res) * TMP2)) * 1000
		//						/ (RES1 - RES2);
		numerator = (((Res - RES2) * TMP1) + ((RES1 - Res) * TMP2));
		numerator_1000 = numerator * 1000;
		denominator = (RES1 - RES2);
		TAP_Value = (__s32)(numerator_1000/denominator);
#endif
	}
...
/* from misc\mediatek\thermal\common\thermal_zones\mtk_ts_bts.c */
/* static __s32 mtk_ts_bts_volt_to_temp(__u32 dwVolt) */
static __s32 mtk_ts_bts_volt_to_temp(__u32 dwVolt)
{
	__s32 TRes, numerator = 0, denominator = 0;

	numerator = dwVolt * g_RAP_pull_up_R;
	denominator = g_RAP_pull_up_voltage - dwVolt;
	TRes = numerator / denominator;

	return TRes;
}

static u32 get_ts_volt_mV(struct device *dev)
{
	...
	u32 val;
	if (pinfo == NULL) {
		pr_err("[%s]: [TYPEC_OTP] charger manager is null!\n", __func__);
		return -ENODEV;
	}

	if (pinfo->chg1_dev == NULL) {
		pr_err("[%s]: [TYPEC_OTP] charger device is null!\n", __func__);
		return -ENODEV;
	}

	ret = charger_dev_get_ts(pinfo->chg1_dev, &val);
	if (ret < 0)
		pr_err("[%s]: [TYPEC_OTP] charger_dev_get_ts() fail! ret = %d\n", __func__, ret);

	// convert uV to mV
	val /= 1000;

	/**** [SX3U-2583][FTM][BB1] Charging_Test fail by MT6630 stop due to USB OTP ---- START ****/
	if (val > VREF_TS_PULL_UP_VOLTAGE_UBOUND) {
		pr_info("[TYPEC_OTP] -------- %lu is over than %d. Convert it to %d)\n", (unsigned long)val, VREF_TS_PULL_UP_VOLTAGE_UBOUND, VREF_TS_PULL_UP_VOLTAGE_UBOUND);
		val = VREF_TS_PULL_UP_VOLTAGE_UBOUND;
	}
	/**** [SX3U-2583][FTM][BB1] Charging_Test fail by MT6630 stop due to USB OTP ---- END   ****/

	return val;
}

static ssize_t show_TS_VOLT(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	u32 val = get_ts_volt_mV(dev);
	pr_info("[TYPEC_OTP] -------- Get ts_voltage = %lu (mV)\n", (unsigned long)val);
	return sprintf(buf, "%lu\n", (unsigned long)val);
}
static DEVICE_ATTR(TS_VOLT, 0444, show_TS_VOLT, NULL);

static ssize_t show_TS_REGI(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	u32 TVoltage;
	__s32 TRegister;
	...
	// get ts voltage
	TVoltage = get_ts_volt_mV(dev);
	pr_info("[TYPEC_OTP] -------- Get ts_voltage = %lu (mV)\n", (unsigned long)TVoltage);
	// convert voltage to register
	TRegister = mtk_ts_bts_volt_to_temp((__u32)TVoltage);
	pr_info("[TYPEC_OTP] -------- Get ts_register = %ld (kOhm)\n", (long)TRegister);

	return sprintf(buf, "%ld\n", (long)TRegister);
}

static DEVICE_ATTR(TS_REGI, 0444, show_TS_REGI, NULL);

static __s32 get_ts_temp(struct device *dev)
{
	u32 TVoltage;
	__s32 TRegister, TTemperature = -200;
	int i;

	for (i = 1; i <= 3; i++) {
		pr_info("[TYPEC_OTP] READ TS_TEMP (%02d) ================ \n", i);
		// get ts voltage
		TVoltage = get_ts_volt_mV(dev);
		pr_info("[TYPEC_OTP] -------- Get ts_voltage = %lu (mV)\n", (unsigned long)TVoltage);
		// convert voltage to register
		TRegister = mtk_ts_bts_volt_to_temp((__u32)TVoltage);
		pr_info("[TYPEC_OTP] -------- Get ts_register = %d (kOhm)\n", (int)TRegister);
		// convert register to temperature
		TTemperature = mtkts_bts_thermistor_conver_temp(TRegister);
		pr_info("[TYPEC_OTP] -------- Get ts_temperature = %05d (degree)\n", TTemperature);

		if (TTemperature < TYPEC_OTP_ALERT_THRESHOLD) {
			break;
		}
		else {
			pr_warn("[TYPEC_OTP] usb_connector temperature %05d >= %d ----> try again\n", TTemperature, TYPEC_OTP_ALERT_THRESHOLD);
			msleep(20);
		}
	}
...
static ssize_t show_TS_TEMP(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	__s32 TTemperature = get_ts_temp(dev);
	return sprintf(buf, "%05d\n", TTemperature);
}

static DEVICE_ATTR(TS_TEMP, 0444, show_TS_TEMP, NULL);
		...
		struct device_attribute *attr, char *buf)
{
	...
	pr_info("[TYPEC_OTP] -------- Get ts_Information = %lu (mV), %ld (KOhm), %05d (degree)\n", (unsigned long)TVoltage, (long)TRegister, TTemperature);

	return sprintf(buf, "%lu,%ld,%05d\n", (unsigned long)TVoltage, (long)TRegister, TTemperature);
}

static DEVICE_ATTR(TS_INFO, 0444, show_TS_INFO, NULL);
/**** [SX3-746][SX3][Thermal]Implement thermal node for item4_charging and item7_USB_FPC ---- END   ****/
...
static int persist_power_path_enable_show(struct seq_file *m, void *data)
{
	struct mtk_charger *pinfo = m->private;
	bool power_path_en = true;
...
static int persist_power_path_enable_open(struct inode *node, struct file *file)
{
	return single_open(file, persist_power_path_enable_show, pde_data(node));
}

static ssize_t persist_power_path_enable_write(struct file *file,
		const char *buffer, size_t count, loff_t *data)
{
	int len = 0, ret = 0;
	char desc[32] = {0};
	...
	if (ret == 0) {
		/* SX4V-285 */
		info->persist_power_path_enable = enable;
		pr_info("[[%s:%d]] set persist_power_path_enable: %d\n", __func__, __LINE__, info->persist_power_path_enable);
		/* SX4V-285 */
		charger_dev_enable_powerpath(info->chg1_dev, enable);
		chr_info("%s: enable power path = %d\n", __func__, enable);
		return count;
	...
	chr_err("bad argument, echo [enable] > persist_power_path_enable\n");
	return count;
}

static const struct proc_ops persist_power_path_enable_fops = {
	.proc_open = persist_power_path_enable_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
	.proc_write = persist_power_path_enable_write,
};
...
/* FIH file node */
static ssize_t bat_vol_decrease_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	
	return sprintf(buf, "bat_vol_decrease = %d\n", bat_vol_decrease);
}

ssize_t bat_vol_decrease_store(struct device *dev,
		struct device_attribute *attr, const char *buf,  size_t size)
{
	int scale;
...
static DEVICE_ATTR_RW(bat_vol_decrease);

static ssize_t total_time_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
ssize_t total_time_store(struct device *dev,
		struct device_attribute *attr, const char *buf,  size_t size)
{
	int scale;
...
static DEVICE_ATTR_RW(total_time);

static ssize_t total_coulomb_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%ld\n", damage_info->total_coulomb);
}

ssize_t total_coulomb_store(struct device *dev,
		struct device_attribute *attr, const char *buf,  size_t size)
{
	int scale;
	...
	if (kstrtoint(buf, 10, &scale) == 0) 
	{ 
		damage_info->total_coulomb = scale;
		chr_err("[%s] total_coulomb = %ld  \n", __func__,damage_info->total_coulomb); 
...
static DEVICE_ATTR_RW(total_coulomb);

static ssize_t battery_deterioration_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	battery_deterioration_calculate();
	return sprintf(buf, "%d\n", battery_deterioration);
}

ssize_t battery_deterioration_store(struct device *dev,
		struct device_attribute *attr, const char *buf,  size_t size)
{
	int scale;
	...
	if (kstrtoint(buf, 10, &scale) == 0) 
	{ 
		battery_deterioration_set_scale = scale;
		chr_err("[%s] battery_deterioration_set_scale = %d ,set 200 will get default value \n", __func__,scale); 
...
static DEVICE_ATTR_RW(battery_deterioration);

static ssize_t bat_max_capacity_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
#if defined(CONFIG_FIH_SX4)
    int max_capacity = 972000; /* 20000%(4860*200=972000)(mAh) */
#elif defined(CONFIG_FIH_SX3)
    int max_capacity = 724000; /* 20000%(3620*200=724000)(mAh) */
#endif
	chr_err("[%s]: %d (mAh) \n", __func__,max_capacity);

	return sprintf(buf, "%d\n",max_capacity);
}

static DEVICE_ATTR_RO(bat_max_capacity);


static ssize_t oem_enable_charging_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;
...
static ssize_t oem_enable_charging_store(struct device *dev, struct device_attribute *attr, const char *buf,
				 size_t size)
{
	struct mtk_charger *pinfo = dev->driver_data;
	int current_unlimited = 0;
	...
	if (sscanf(desc, "%d %d", &current_unlimited, &cmd_discharging) == 2) {
		pinfo->usb_unlimited = current_unlimited;
		if (cmd_discharging == 1) {
			pinfo->cmd_discharging = true;
			charger_dev_enable(pinfo->chg1_dev, false);
			pinfo->direct_charging_en = true;
		} else if (cmd_discharging == 0) {
			pinfo->cmd_discharging = false;
			charger_dev_enable(pinfo->chg1_dev, true);
			pinfo->direct_charging_en = false;		
		}
		fih_send_charger_info(direct_charging,pinfo->direct_charging_en);

		chr_err("%s current_unlimited=%d, cmd_discharging=%d\n",
			__func__, current_unlimited, cmd_discharging);
		chr_err("%s directcharging pinfo->cmd_discharging = %d \n",
...
static DEVICE_ATTR_RW(oem_enable_charging);
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret=power_supply_get_property(finfo->chg_psy,
...
static ssize_t Battery_Temp_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", battery_manager_info(battery_temp));
}
...
static ssize_t Current_Now_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", battery_manager_info(battery_current));
}
...
static ssize_t ADC_IBAT_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int ret;
	union power_supply_propval online = {0};
	int ibat = (mt6360_info(mt6360_ibat)/1000); 
	ret = power_supply_get_property(finfo->chg_psy,
			POWER_SUPPLY_PROP_ONLINE, &online);
	if (ret < 0)
			chr_err("%s: %d\n", __func__, ret);
	...
	if(online.intval == 0){
		ibat = 0;
		chr_err("do not insert USB,don't show ibat_adc, set ibat_adc = 0");
	}
	pr_debug("[%s]: %d\n", __func__, ibat);
...
static DEVICE_ATTR_RO(ADC_IBAT);

static ssize_t ADC_Charger_Current_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int ret;
	union power_supply_propval online = {0};
	int ibus = (mt6360_info(mt6360_ibus)/1000); 
	ret = power_supply_get_property(finfo->chg_psy,
			POWER_SUPPLY_PROP_ONLINE, &online);
	if (ret < 0)
			chr_err("%s: %d\n", __func__, ret);
	if(online.intval == 0){
		ibus = 0;
		chr_err("do not insert USB, IBUS = 0");
	}
...
static DEVICE_ATTR_RO(ADC_Charger_Current);

static ssize_t Age_Percentage_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "65535\n");
}
static DEVICE_ATTR_RO(Age_Percentage);

static ssize_t Fuelgauge_Capacity_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int soc = show_soc();
	return sprintf(buf, "%d\n",soc);
}
static DEVICE_ATTR_RO(Fuelgauge_Capacity);

static ssize_t Debug_Code_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	/**************** 
	[Error information description]
	jeita : If the battery temperature is over 55°C or below 0°C, the bit will be set to 1.
	Thermal 12: If thermal mitigation level 12 is enabled, the bit will be set to 1.
	Thermal 13: If thermal mitigation level 13 is enabled, the bit will be set to 1.
	USB Thermal: If the USB thermal temperature exceeds the limit, the bit will be set to 1.
	Safety Timeout: If the charging time exceeds the safety limit, the bit will be set to 1.
	VBUS OVP: If VBUS exceeds the overvoltage limit, the bit will be set to 1.
	DPDM OVP: If USB Data+ (DP) & Data- (DM) overvoltage protection is enabled, the bit will be set to 1.
	****************/
	
	/* for debug
	bool jeita = !fih_get_charger_info(jeita_charging);
	bool thermal12 = !fih_get_charger_info(thermal12_charging);
	bool thermal13 = !fih_get_charger_info(thermal13_charging);
	bool usb_thermal = !fih_get_charger_info(thermal_usb_charging);
	bool safety_timeout = finfo->safety_timeout;
	bool VBUS_OVP = finfo->vbusov_stat;
	bool DPDM_OVP = finfo->dpdmov_stat;
	chr_err("[Debug_Code] jeita = %d ,fih_get_charger_info(jeita_charging) = %d",jeita,fih_get_charger_info(jeita_charging));
	chr_err("[Debug_Code] thermal12 = %d ,fih_get_charger_info(thermal12_charging) = %d",thermal12,fih_get_charger_info(thermal12_charging));
	chr_err("[Debug_Code] thermal13 = %d ,fih_get_charger_info(thermal13_charging) = %d",thermal13,fih_get_charger_info(thermal13_charging));
	chr_err("[Debug_Code] usb_thermal = %d ,fih_get_charger_info(thermal_usb_charging) = %d",usb_thermal,fih_get_charger_info(thermal_usb_charging));
	chr_err("[Debug_Code] safety_timeout = %d ,finfo->safety_timeout = %d",safety_timeout,finfo->safety_timeout);
	chr_err("[Debug_Code] VBUS_OVP = %d ,finfo->vbusov_stat= %d",VBUS_OVP,finfo->vbusov_stat);
	chr_err("[Debug_Code] DPDM_OVP = %d ,finfo->dpdmov_stat= %d",DPDM_OVP,finfo->dpdmov_stat);
	*/

	int sum = 0;
	sum |= (!fih_get_charger_info(jeita_charging)) << 0;   
	sum |= (!fih_get_charger_info(thermal12_charging)) << 1; 
	sum |= (!fih_get_charger_info(thermal13_charging)) << 2; 
	sum |= (!fih_get_charger_info(thermal_usb_charging)) << 3; 
	sum |= (finfo->safety_timeout) << 4;              
	sum |= (finfo->vbusov_stat) << 5;                 
	sum |= (finfo->dpdmov_stat) << 6;                 

	chr_err("[Debug_Code] sum = %d ,  DPDM_OVP = %d , VBUS_OVP = %d , safety_timeout = %d , usb_thermal = %d , thermal 13 = %d ,thermal 12 = %d , jeita = %d \n"
			,sum,finfo->dpdmov_stat,finfo->vbusov_stat,finfo->safety_timeout,
			!fih_get_charger_info(thermal_usb_charging),!fih_get_charger_info(thermal13_charging),!fih_get_charger_info(thermal12_charging),!fih_get_charger_info(jeita_charging));

	return sprintf(buf, "%d\n",sum);
}
static DEVICE_ATTR_RO(Debug_Code);
...
int set_tcpc_to_charger(struct tcpc_device *tcpc)
{
	struct mtk_charger *pinfo = finfo;
	if (pinfo == NULL) {
		chr_err("[TYPEC_OTP][%s] global mtk_charger is null! ret: %d", __func__, -ENODEV);
		return -ENODEV;
	}

	// phone has only one typec port
	chr_err("[TYPEC_OTP][%s] configure mtk_charger typec %s", __func__, tcpc == NULL ? "null" : "instance");
	pinfo->tcpc = gcs->tcpc = tcpc;

	return 0;
}
EXPORT_SYMBOL(set_tcpc_to_charger);

int fih_disable_cc(struct mtk_charger *pinfo, bool disable) {
	int ret = 0;
	if (pinfo->tcpc) {
		ret = tcpm_typec_disable_function(pinfo->tcpc, disable);
		chr_err("[TYPEC_OTP] %s cc %s", disable ? "disable" : "enable", ret == 0 ? "successfully" : "fail");
	} else
		chr_err("[TYPEC_OTP] charger typec object is null!");
...
stop_charging:
	...
	fih_send_charger_info(jeita_charging,info->sw_jeita.charging);
	chr_err("%s directcharging info->cmd_discharging = %d \n",__func__, info->cmd_discharging);	
...
bool mtk_charger_status_info(u32 event){
	bool val = 0;
	chr_err("[%s] %d  \n",__func__,__LINE__);
	switch (event) {
	case jeita_charging:
		val = sharp_charging_en.jeita_charging;
		chr_err("[%s] %d  get jeita_charging %d \n",__func__,__LINE__,sharp_charging_en.jeita_charging);
		break;
	case power_path_charging:
		val = sharp_charging_en.power_path_charging;
		chr_err("[%s] %d  get power_path_charging %d\n",__func__,__LINE__,sharp_charging_en.power_path_charging);
		break;
	case direct_charging:
		val = sharp_charging_en.direct_charging;
		chr_err("[%s] %d  get direct_charging %d\n",__func__,__LINE__,sharp_charging_en.direct_charging);
		break;
	case charging_online:
		val = sharp_charging_en.charging_online;
		chr_err("[%s] %d  get charging_online %d\n",__func__,__LINE__,sharp_charging_en.charging_online);
		break;
	default:
		break;
	}
...
static int mtk_charger_plug_out(struct mtk_charger *info)
{
	...
	if(info->persist_power_path_enable != 0) {
	pr_info("[[%s:%d]] set persist_power_path_enable: %d\n", __func__, __LINE__, info->persist_power_path_enable);
...
static void Battery_aging_observation(struct mtk_charger *info)
{
	ktime_t ktime_now;
	struct timespec64 time_now;
	int temp = get_battery_temp(true); /*battery temperature*/
	int set_capacity = 90;/*90 percentage of capacity*/
	int set_temp = 35 ;/*35 deg*/
	int real_capacity = show_soc(); /* real capacity, not ui soc*/


	damage_info = &info->damage_info;

	ktime_now = ktime_get_boottime();
	time_now = ktime_to_timespec64(ktime_now);
	chr_err("%s: now: %lld now_coulomb =%d  ,tmp: %d deg ,real_capacity= %d , stat =%d\n", __func__, time_now.tv_sec, get_battery_columb()/10,temp,real_capacity,damage_info->stat);
	

	if(temp >set_temp && real_capacity>set_capacity && damage_info->stat == 1){
		chr_err("[%s] battery is damaged due to temperature > %ddeg and real capacity > 90 \n", __func__,set_temp);


	}else if(temp >set_temp && real_capacity>set_capacity){
		chr_err("[%s] battery damage start ----------------------- \n", __func__);
		/*save start time and coulomb*/
		damage_info->stat = 1;
		damage_info->start_time = time_now.tv_sec;
		damage_info->start_coulomb = get_battery_columb()/10;
		chr_err("[%s] start_time = %ld sec , start_coulomb = %ld mAh \n", __func__,damage_info->start_time,damage_info->start_coulomb);

	}else if(damage_info->stat == 1){
		chr_err("[%s] battery damage end ----------------------- \n", __func__);
		/*save end time and coulomb*/
		damage_info->stat = 0;
		damage_info->end_time = time_now.tv_sec;
		damage_info->end_coulomb = get_battery_columb()/10;
		chr_err("[%s] end_time = %ld sec , end_coulomb = %ld mAh \n", __func__,damage_info->end_time,damage_info->end_coulomb);

		/*calculate total time and coulomb*/
		damage_info->total_time = damage_info->end_time - damage_info->start_time;
		damage_info->total_coulomb = damage_info->end_coulomb - damage_info->start_coulomb;
		chr_err("[%s] total_time = %ld sec , total_coulomb = %ld mAh \n", __func__,damage_info->total_time,damage_info->total_coulomb);
		
		/*ensure total_coulomb always positive*/
	...
	if(bat_vol_decrease == 1){
		...
		chr_err("[%s] battery has already aged ,bat_vol_decrease = %d ,battery max voltage set to = %d mV\n", __func__,bat_vol_decrease,info->data.battery_cv/1000);
	}
...
static void battery_deterioration_calculate(void)
{
	...
	int full_cap = battery_full_capacity;
	int cal;
	
	cal = (now_cap*100)/full_cap;

	chr_err("[%s] now_cap = %d mAh, deterioration_persent = %d, full_cap = %d\n", __func__,now_cap,cal,full_cap);
...
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		Battery_aging_observation(info);
		...
		ret=power_supply_get_property(info->chg_psy,
			POWER_SUPPLY_PROP_ONLINE, &online);
		fih_send_charger_info(charging_online,online.intval);
		spin_lock_irqsave(&info->slock, flags);
		if(online.intval == 0){ //ac charger wakelock do not release frome her
...
/**** [SX3-696][REQ][Charger][SW-PMIC-0043]Disable charger when USB TypeC connector becomes too hot ---- START ****/
	ret = device_create_file(&(pdev->dev), &dev_attr_cc_safety_level);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_cc_safety_level_test_enable);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_cc_safety_level_test_val);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_cc_forcibly_enable);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_reboot_forcibly);
	if (ret)
		goto _out;
/**** [SX3-696][REQ][Charger][SW-PMIC-0043]Disable charger when USB TypeC connector becomes too hot ---- END   ****/

/**** [SX3-746][SX3][Thermal]Implement thermal node for item4_charging and item7_USB_FPC ---- START ****/
	ret = device_create_file(&(pdev->dev), &dev_attr_TS_VOLT);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_TS_REGI);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_TS_TEMP);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_TS_INFO);
	if (ret)
		goto _out;
/**** [SX3-746][SX3][Thermal]Implement thermal node for item4_charging and item7_USB_FPC ---- END   ****/
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_bat_vol_decrease);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_total_time);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_total_coulomb);
	if (ret)
		goto _out;
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_bat_max_capacity);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_oem_enable_charging);
	if (ret)
		goto _out;
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_Current_Now);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_ADC_IBAT);
	if (ret)
		goto _out;
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_Age_Percentage);
	if (ret)
		goto _out;
	
	ret = device_create_file(&(pdev->dev), &dev_attr_Fuelgauge_Capacity);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_Debug_Code);
	if (ret)
		goto _out;
	...
	/* SX4V-285 */
	entry = proc_create_data("persist_power_path_enable", 0644, battery_dir,
			&persist_power_path_enable_fops, info);
	if (!entry) {
		ret = -ENODEV;
		pr_err("[[%s]]: fail to create persist_power_path_enable\n", __func__);
		goto fail_procfs;
	}
	/* SX4V-285 */
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	/* SX4V-297 */
	bool thermal_charging = true;
	bool thermal_input_charging = true;
	...
	/* SX4V-297 */
		chr_err("[%s] %d thermal POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX = %d \n",__func__,__LINE__,val->intval);
		if(val->intval ==1000||val->intval ==0){
			thermal_charging = false;
			chr_err("[%s] %d thermal_charging  = %d \n",__func__,__LINE__,thermal_charging);
		}else{
			thermal_charging = true;
			chr_err("[%s] %d thermal_charging  = %d \n",__func__,__LINE__,thermal_charging);
		}
		fih_send_charger_info(thermal12_charging,thermal_charging);
		
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		val->intval =
			info->chg_data[idx].thermal_input_current_limit;
		chr_err("[%s] %d POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT  thermal_input_current_limit= %d \n",__func__,__LINE__,val->intval);
		if(val->intval ==0){
			thermal_input_charging = false;
			chr_err("[%s] %d thermal_input_charging  = %d \n",__func__,__LINE__,thermal_input_charging);
		}else{
			thermal_input_charging = true;
			chr_err("[%s] %d thermal_input_charging  = %d \n",__func__,__LINE__,thermal_input_charging);
		}
		fih_send_charger_info(thermal13_charging,thermal_input_charging);
	...
	int idx, bool en)
{
	...
	fih_send_charger_info(power_path_charging,is_en);
			...
			unsigned long evt, void *val)
{
	...
	case MTK_SINK_VBUS:
		if (pinfo->en_cts_mode) {
			...
			if ((*(int *)val) < 100) {
				if (pinfo->power_path_en && pinfo->persist_power_path_enable != 0) {
					pr_info("[[%s:%d]] set persist_power_path_enable: %d\n", __func__, __LINE__, pinfo->persist_power_path_enable);
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	damage_info = kcalloc(1, sizeof(struct damage_info_data), GFP_KERNEL);	
	if (damage_info == NULL) {
		chr_err("Failed to allocate memory for damage_info.\n");
	}
	...
	/**** [SX3-696] initite cc_safety structure: START ****/
	chr_err("[TYPEC_OTP] Initiate global cc_safety object\n");
	gcs = kcalloc(1, sizeof(struct S_cc_safety), GFP_KERNEL);
	if (gcs == NULL)
		chr_err("[TYPEC_OTP] Failed to allocate memory for S_cc_safety!\n");
	else {
		gcs->cc_safety_level = 0;
		gcs->cc_safety_level_test_enable = 0;
		gcs->cc_safety_level_test_val = 25000;
		gcs->tcpc = info->tcpc;
	ktime_now = ktime_get_boottime();
		gcs->cc_safety_ts64_otp_enable = ktime_to_timespec64(ktime_now);
		print_cc_safety_state(gcs, "mtk_charger");
		set_cc_safety_to_battery(gcs);
	}
	/**** [SX3-696] initite cc_safety structure: END   ****/

	info->persist_power_path_enable = 1;
	pr_info("[[%s:%d]] set persist_power_path_enable: %d\n", __func__, __LINE__, info->persist_power_path_enable);
