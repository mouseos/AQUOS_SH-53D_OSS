...
/* variable */
int set_temperature = original_temperature;
struct mtk_battery *fih_gm;
struct mtk_battery *gmb;
struct S_cc_safety *gcs;

static struct customized_charging_status_source sharp_charging_en = {
	.jeita_charging = true,
	.power_path_charging = true,
	.direct_charging = false,
	.charging_online = true,
	.thermal12_charging = true,
	.thermal13_charging = true,
	.thermal_usb_charging = true
};
...
int force_get_tbat(struct mtk_battery *gm, bool update)
{
	...
	bm_debug(gm,"[%s] set_temperature = %d (default = -123) \n", __func__,set_temperature);
...
/* SX4V-282 */
int get_battery_temp(bool update){
	return force_get_tbat(fih_gm, true);
}
EXPORT_SYMBOL(get_battery_temp);

int get_battery_columb(void){
	int val;
	gauge_get_property(fih_gm, GAUGE_PROP_COULOMB, &val);
	return val;
}
EXPORT_SYMBOL(get_battery_columb);

void set_cc_safety_to_battery(struct S_cc_safety* cs) {
	bm_err(gmb,"[TYPEC_OTP][%s] set charger cs to battery\n",__func__);
	gcs = cs;
	print_cc_safety_state(NULL, "mtk_charger");
}
EXPORT_SYMBOL(set_cc_safety_to_battery);

struct S_cc_safety* get_cc_safety_from_battery(void) {
	bm_err(gmb,"[TYPEC_OTP][%s] get cs from battery \n",__func__);
	return gcs;
}
EXPORT_SYMBOL(get_cc_safety_from_battery);

void print_cc_safety_state(struct S_cc_safety* cs, char* mod_name) {
	if (cs) {
		pr_notice("[TYPEC_OTP][%s] cc_safety_level: %d | typec: %s | test_mode: %d | dummy temp: %d | cc_safety_ts64_otp_enable.tv_sec: %lld | cc_safety_ts64_otp_enable.tv_nsec: %ld\n",
					mod_name,
					cs->cc_safety_level, cs->tcpc == NULL ? "null" : "object",
					cs->cc_safety_level_test_enable,
					cs->cc_safety_level_test_val,
					cs->cc_safety_ts64_otp_enable.tv_sec,
					cs->cc_safety_ts64_otp_enable.tv_nsec);
	}
	else {
		if (gcs)
			pr_notice("[TYPEC_OTP][mtk_battery] cc_safety_level: %d | typec: %s | test_mode: %d | dummy temp: %d | cc_safety_ts64_otp_enable.tv_sec: %lld | cc_safety_ts64_otp_enable.tv_nsec: %ld\n",
						gcs->cc_safety_level,
						gcs->tcpc == NULL ? "null" : "object",
						gcs->cc_safety_level_test_enable,
						gcs->cc_safety_level_test_val,
						gcs->cc_safety_ts64_otp_enable.tv_sec,
						gcs->cc_safety_ts64_otp_enable.tv_nsec);
		else
			pr_err("[TYPEC_OTP] both %s and mtk_battery cc_safety instances are null! cannot print its state!", mod_name);
	}
}
EXPORT_SYMBOL(print_cc_safety_state);

/* SX4V-297 */
void fih_send_charger_info(u32 event,bool en){
	bm_debug(gmb,"[%s] %d  \n",__func__,__LINE__);
	switch (event) {
	case jeita_charging:
		sharp_charging_en.jeita_charging = en;
		bm_err(gmb,"[%s] %d  send jeita_charging %d \n",__func__,__LINE__,sharp_charging_en.jeita_charging);
		break;
	case power_path_charging:
		sharp_charging_en.power_path_charging = en;
		bm_err(gmb,"[%s] %d  send power_path_charging %d\n",__func__,__LINE__,sharp_charging_en.power_path_charging);
		break;
	case direct_charging:
		sharp_charging_en.direct_charging = en;
		bm_err(gmb,"[%s] %d  send direct_charging %d\n",__func__,__LINE__,sharp_charging_en.direct_charging);
		break;
	case charging_online:
		sharp_charging_en.charging_online = en;
		bm_err(gmb,"[%s] %d  send charging_online %d\n",__func__,__LINE__,sharp_charging_en.charging_online);
		break;
	case thermal12_charging:
		sharp_charging_en.thermal12_charging = en;
		bm_err(gmb,"[%s] %d  send thermal12_charging %d\n",__func__,__LINE__,sharp_charging_en.thermal12_charging);
		break;
	case thermal13_charging:
		sharp_charging_en.thermal13_charging = en;
		bm_err(gmb,"[%s] %d  send thermal13_charging %d\n",__func__,__LINE__,sharp_charging_en.thermal13_charging);
		break;
	case thermal_usb_charging:
		sharp_charging_en.thermal_usb_charging = en;
		bm_err(gmb,"[%s] %d  send thermal_usb_charging %d\n",__func__,__LINE__,sharp_charging_en.thermal_usb_charging);
		break;
	default:
		break;
	}
}
EXPORT_SYMBOL(fih_send_charger_info);


bool fih_get_charger_info(u32 event){
	bool val = 0;
	bm_debug(gmb,"[%s] %d  \n",__func__,__LINE__);
	switch (event) {
	case jeita_charging:
		val = sharp_charging_en.jeita_charging;
		bm_err(gmb,"[%s] %d  get jeita_charging %d \n",__func__,__LINE__,sharp_charging_en.jeita_charging);
		break;
	case power_path_charging:
		val = sharp_charging_en.power_path_charging;
		bm_err(gmb,"[%s] %d  get power_path_charging %d\n",__func__,__LINE__,sharp_charging_en.power_path_charging);
		break;
	case direct_charging:
		val = sharp_charging_en.direct_charging;
		bm_err(gmb,"[%s] %d  get direct_charging %d\n",__func__,__LINE__,sharp_charging_en.direct_charging);
		break;
	case charging_online:
		val = sharp_charging_en.charging_online;
		bm_err(gmb,"[%s] %d  get charging_online %d\n",__func__,__LINE__,sharp_charging_en.charging_online);
		break;
	case thermal12_charging:
		val = sharp_charging_en.thermal12_charging;
		bm_err(gmb,"[%s] %d  get thermal12_charging %d\n",__func__,__LINE__,sharp_charging_en.thermal12_charging);
		break;
	case thermal13_charging:
		val = sharp_charging_en.thermal13_charging;
		bm_err(gmb,"[%s] %d  get thermal13_charging %d\n",__func__,__LINE__,sharp_charging_en.thermal13_charging);
		break;
	case thermal_usb_charging:
		val = sharp_charging_en.thermal_usb_charging;
		bm_err(gmb,"[%s] %d  send thermal_usb_charging %d\n",__func__,__LINE__,sharp_charging_en.thermal_usb_charging);
		break;
	default:
		break;
...
EXPORT_SYMBOL(fih_get_charger_info);
