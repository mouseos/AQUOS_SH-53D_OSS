...
/* variable */
static int read_mt6360_ibus_node_uA ;
static int read_mt6360_ibat_node_uA ;
static int PD_enable =0;
static int DCP_cur_max = 0;
static struct platform_device *f_pdev = NULL;
...
/* MT6360_PMU_CHG_CTRL11 : 0x31B  SX3-3979 */
#define MT6360_MASK_VRECH	(0x03)
#define MT6360_SHFT_VRECH	(0)
#define MT6360_VRECH_MAXVAL	(0x03)
...
out:
	...
	ret = regmap_update_bits(mci->regmap,
				 ...
				 MT6360_MASK_VRECH, 0x2);// Charging re-charge voltage threshold with VOREG: 200mV
vsys_wkard_fail:
...
static int mt6360_dump_registers(struct charger_device *chg_dev)
{
	...
	dev_info(mci->dev,
		...
		read_mt6360_ibat_node_uA = adc_vals[MT6360_ADC_IBAT];
...
int mt6360_info(u32 event){
	int val = 0;
	switch (event) {
	case mt6360_ibus:
		val = read_mt6360_ibus_node_uA;
		break;
	case mt6360_ibat:
		val = read_mt6360_ibat_node_uA;
		break;
	default:
		break;
	}
...
EXPORT_SYMBOL(mt6360_info);

void send_to_mt6360(u32 event,int value){
	dev_err(&f_pdev->dev,"[%s] \n",__func__);	
	switch (event) {
	case PD_START:
		PD_enable = value;
		dev_err(&f_pdev->dev,"[%s] PD_enable = %d\n",__func__,PD_enable);		
		break;
	case DCP_input_current_max:
		DCP_cur_max = value;
		dev_err(&f_pdev->dev,"[%s] DCP_cur_max = %d\n",__func__,DCP_cur_max);		
		break;
	default:
		break;
	}
}
EXPORT_SYMBOL(send_to_mt6360);
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		if (mci->psy_desc.type == POWER_SUPPLY_TYPE_USB){
			val->intval = 500000;
			dev_info(mci->dev, "%s:USB POWER_SUPPLY_PROP_CURRENT_MAX val->intval = %d \n", __func__,val->intval);
		}
		if (mci->psy_desc.type == POWER_SUPPLY_TYPE_USB_DCP){
			val->intval = DCP_cur_max;
			dev_info(mci->dev, "%s:DCP POWER_SUPPLY_PROP_CURRENT_MAX val->intval = %d \n", __func__,val->intval);
		}
		if (PD_enable){
			val->intval = 3000000;
			dev_info(mci->dev, "%s:PD POWER_SUPPLY_PROP_CURRENT_MAX val->intval = %d \n", __func__,val->intval);
		}
		break;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		dev_info(mci->dev,"[%s]vbus_max = %d mV \n",__func__,val->intval/1000);
		break;
	default:
		ret = -ENODATA;
	}
