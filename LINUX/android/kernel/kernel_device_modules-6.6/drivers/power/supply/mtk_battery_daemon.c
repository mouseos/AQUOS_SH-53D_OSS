...
int full_capacity_now;
EXPORT_SYMBOL(full_capacity_now);
int real_capacity;
EXPORT_SYMBOL(real_capacity);
...
/*FIH file node*/
static ssize_t Temperature_Setting_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t Temperature_Setting_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct mtk_battery *gm;
	struct mtk_gauge *gauge;
	...
	{ 
		...
		bm_err(gm, "[%s] temperature_setting = %d \n", __func__,temp); 
...
/*FIH file node*/
static DEVICE_ATTR_RW(Temperature_Setting);
...
static int mtk_battery_setup_files(struct platform_device *pdev)
{
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_Temperature_Setting);
	if (ret)
		goto _out;
...
int sharp_customized_uisoc_sx3_precise_ver(struct mtk_battery *gm,int mtk_uisoc){
    int new_ui_soc;
    if(mtk_uisoc == 10000){
                new_ui_soc = (mtk_uisoc+50)/100 ;
                bm_err(gm,"[mtk_battery_daemon_sx3] sharp_uisoc = %d , mtk_uisoc = %d",new_ui_soc,mtk_uisoc);
            }else if(mtk_uisoc< 9900 &&mtk_uisoc >= 7000){
                new_ui_soc = (805000 + 66*(mtk_uisoc - 7000))/10000;
                bm_err(gm,"[mtk_battery_daemon_sx3] sharp_uisoc = %d , mtk_uisoc = %d",new_ui_soc,mtk_uisoc);
            }else if(mtk_uisoc< 7000 && mtk_uisoc>=4000){
                new_ui_soc = (455000 +115*(mtk_uisoc - 4000))/10000;
                bm_err(gm,"[mtk_battery_daemon_sx3] sharp_uisoc = %d  mtk_uisoc = %d",new_ui_soc,mtk_uisoc);
            }else if(mtk_uisoc< 4000 && mtk_uisoc>=2000){
                new_ui_soc = (205000 + 125*(mtk_uisoc - 2000))/10000;
                bm_err(gm,"[mtk_battery_daemon_sx3] sharp_uisoc = %d , mtk_uisoc = %d",new_ui_soc,mtk_uisoc);
            }else{
                new_ui_soc = (mtk_uisoc+50)/100 ;
                bm_err(gm,"[mtk_battery_daemon_sx3] sharp_uisoc = %d , mtk_uisoc = %d",new_ui_soc,mtk_uisoc);
...
int sharp_customized_uisoc_sx4_precise_ver(struct mtk_battery *gm,int mtk_uisoc){
	int new_ui_soc;
	if(mtk_uisoc == 10000){
				new_ui_soc = (mtk_uisoc+50)/100 ;
				bm_err(gm,"[mtk_battery_core_sx4] sharp_uisoc = %d , mtk_uisoc = %d",new_ui_soc,mtk_uisoc);
			}else if(mtk_uisoc< 9900 &&mtk_uisoc >= 7000){
				new_ui_soc = (805000 + 66*(mtk_uisoc - 7000))/10000;
				bm_err(gm,"[mtk_battery_core_sx4] sharp_uisoc = %d , mtk_uisoc = %d",new_ui_soc,mtk_uisoc);
			}else if(mtk_uisoc< 7000 && mtk_uisoc>=4800){
				new_ui_soc = (505000 +136*(mtk_uisoc - 4800))/10000;
				bm_err(gm,"[mtk_battery_core_sx4] sharp_uisoc = %d  mtk_uisoc = %d",new_ui_soc,mtk_uisoc);
			}else if(mtk_uisoc< 4800 && mtk_uisoc>=2000){
				new_ui_soc = (205000 + 107*(mtk_uisoc - 2000))/10000;
				bm_err(gm,"[mtk_battery_core_sx4] sharp_uisoc = %d , mtk_uisoc = %d",new_ui_soc,mtk_uisoc);
			}else{
				new_ui_soc = (mtk_uisoc+50)/100 ;
				bm_err(gm,"[mtk_battery_core_sx4] sharp_uisoc = %d , mtk_uisoc = %d",new_ui_soc,mtk_uisoc);
...
//	ret_msg->nl_cmd = msg->nl_cmd;
	...
	case FG_DAEMON_CMD_SET_KERNEL_UISOC:
	{
		...
		if (gm->disableGM30 == true)
			...
			bm_err(gm, "Start customize capacity \n");
			gm->ui_soc = sharp_customized_uisoc_sx4_precise_ver(gm,daemon_ui_soc);
			#elif defined(CONFIG_FIH_SX3)
			gm->ui_soc = sharp_customized_uisoc_sx3_precise_ver(gm,daemon_ui_soc);
			#else
	...
	case FG_DAEMON_CMD_SET_QMAX:
	{
		...
		full_capacity_now = gm->algo_qmax;
