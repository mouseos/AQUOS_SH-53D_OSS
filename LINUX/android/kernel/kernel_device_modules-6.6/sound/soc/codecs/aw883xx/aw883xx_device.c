...
static int aw_dev_sysst_check(struct aw_device *aw_dev)
{
	...
	struct aw_eps_en *eps_desc = &aw_dev->eps_en;
	...
	if (noise_gate_desc->reg != AW_REG_NONE) {
		...
		if (reg_val & (~noise_gate_desc->noise_gate_mask))
			check_value &= noise_gate_desc->check_mask;
	}

	if (eps_desc->reg != AW_REG_NONE) {
		aw_dev->ops.aw_reg_read(aw_dev, eps_desc->reg, &reg_val);
		if (reg_val & (~eps_desc->eps_mask))
			check_value &= eps_desc->check_mask;
	}
