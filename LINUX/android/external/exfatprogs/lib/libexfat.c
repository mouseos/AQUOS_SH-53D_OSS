...
/* SHARP_EXTEND [FSI-22-5-1] Set block device offset Start */
	if (!blk_dev_offset) {
		exfat_info("block device offset read failed\n");

		if (EXFAT_MAX_DATA_AREA_SIZE_128GB > blk_dev_size / DEFAULT_SECTOR_SIZE)
			blk_dev_offset = EXFAT_BOUNDARY_UNIT_UP_TO_128GB;
		else if (EXFAT_MAX_DATA_AREA_SIZE_512GB > blk_dev_size / DEFAULT_SECTOR_SIZE)
			blk_dev_offset = EXFAT_BOUNDARY_UNIT_UP_TO_512GB;
		else if (EXFAT_MAX_DATA_AREA_SIZE_2048GB > blk_dev_size / DEFAULT_SECTOR_SIZE)
			blk_dev_offset = EXFAT_BOUNDARY_UNIT_UP_TO_2048GB;
		else
			exfat_err("exceeds block device size(%s)\n", ui->dev_name);
