...
//at sensor driver.c
 ...
 //#if defined(HI556_MIPI_RAW)
 //at sensor driver.c
 extern unsigned int HI556_read_region(unsigned int addr, unsigned char *data, unsigned int size);
 //#endif

 unsigned int Custom_read_region_HI556(struct i2c_client *client, unsigned int addr,
				 unsigned char *data, unsigned int size)
 {
	 pr_debug("[Zong] I2C_addr: 0x%x, size: %d\n", addr, size);
	 if (HI556_read_region(addr, data, size) == 0)
		 return size;
	 else
			      ...
			      u32 addr, u8 *data, u16 i2c_id, u32 size)
{
	...
	while (size_to_read > 0) {
		...
		if (iReadRegI2C(client, &offset, 1, (u8 *)Buff, 1,
			i2c_id + (page << 1)) < 0) {
			pr_info("fail addr=0x%x 0x%x, P=%d, offset=0x%x",
	...
	pr_info("addr =%x size %d data read = %d\n", addr, size, ret);
	return ret;
}
