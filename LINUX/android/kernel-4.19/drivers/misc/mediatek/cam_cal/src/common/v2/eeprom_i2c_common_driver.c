			    ...
			    u8 *a_puBuff)
{
	...
	if (ui4_length > EEPROM_I2C_READ_MSG_LENGTH_MAX) {
		pr_err("error: exceed one transition %d bytes limitation\n",
			 EEPROM_I2C_READ_MSG_LENGTH_MAX);
		return -1;
	}
	else
	{
		pr_info("ui4_length = %d\n", ui4_length);
	}
			     ...
			     unsigned char *pinputdata)
{
	...
	pBuff = pinputdata;
	pr_info("i4ResidueDataLength = %d, u4CurrentOffset = 0x%x, pBuff = 0x%x\n", i4ResidueSize, u4CurrentOffset, pBuff);
	do {
