...
#undef KBUF_DUMP //dump for bring up
			   ...
			   size_t size, loff_t *offset)
{
	...
	*offset += size;
	...
	#ifdef KBUF_DUMP
	int i = 0; //dump kbuffer at kerenl space
	pr_info("B: [kbuf dump]\n");
	for (i = 0; i < size; i++)
	{
		pr_info("offset[0x%4x] value[0x%2x]\n", i, *(kbuf + i));
	}
	pr_info("E: [kbuf dump]\n");
	#endif
	kfree(kbuf);
