...
int read_mt6360_ibus_node_uA ;
bool mt6360_power_path_report_en = true ;
bool force_power_path_en = true;
bool echo_power_path = false;
...
out:
	...
	mt6360_pmu_reg_update_bits(mpci->mpi, MT6360_PMU_CHG_CTRL11, MT6360_MASK_VRECH, 0x2); // Charging re-charge voltage threshold with VOREG: 200mV
#endif
vsys_wkard_fail:
	mutex_unlock(&mpci->ichg_lock);
	return ret;
					    ...
					    bool en)
{
	...
	if(echo_power_path == true){
		if(force_power_path_en != en) {
			dev_dbg(mpci->dev, "%s: en = %d not match, keep %d\n", __func__, en, force_power_path_en);
			return 0;
		}
	}
	dev_dbg(mpci->dev, "%s: en = %d\n", __func__, en);
	mt6360_power_path_report_en = en;
	return mt6360_pmu_reg_update_bits(mpci->mpi, MT6360_PMU_CHG_CTRL1,
					MT6360_MASK_FORCE_SLEEP, en ? 0 : 0xff);
}

void enable_power_path(bool en)
{
	mt6360_enable_power_path(g_mpci->chg_dev,en);
}
...
static void mt6360_set_safety_timer(struct mt6360_pmu_chg_info *mpci)
{
	int chg_ctrl12;

	mt6360_pmu_reg_update_bits(mpci->mpi, MT6360_PMU_CHG_CTRL12,
					  MT6360_MASK_BIT_5,  0xff );
	mt6360_pmu_reg_update_bits(mpci->mpi, MT6360_PMU_CHG_CTRL12,
					  MT6360_MASK_BIT_6,  0xff );
	mt6360_pmu_reg_update_bits(mpci->mpi, MT6360_PMU_CHG_CTRL12,
					  MT6360_MASK_BIT_7,  0xff );

	chg_ctrl12 = mt6360_pmu_reg_read(mpci->mpi, MT6360_PMU_CHG_CTRL12);
	dev_info(mpci->dev, "%s: chg_ctrl12 = 0x%02X\n",__func__, chg_ctrl12);
...
static int mt6360_get_ts(struct charger_device *chg_dev, u32 *ts)
{
	struct mt6360_pmu_chg_info *mpci = charger_get_data(chg_dev);
	int ret;

	ret = mt6360_get_adc(chg_dev, ADC_CHANNEL_TS, ts, ts);
	if (ret < 0) {
		dev_err(mpci->dev, "%s: failed, ret = %d\n", __func__, ret);
		return ret;
	}

	dev_info(mpci->dev, "%s: ts = %d\n", __func__, *ts);
	return 0;
}
...
static int mt6360_dump_registers(struct charger_device *chg_dev)
{
	...
	bool chg_en = false;
	u8 chg_stat1 = 0,chg_ctrl12 = 0, chg_ctrl[2] = {0};
	...
	read_mt6360_ibus_node_uA = adc_vals[MT6360_ADC_IBUS];
	dev_info(mpci->dev, "%s: CHG_EN = %d, CHG_STATUS = %s, CHG_STAT1 = 0x%02X, CHG_CTRL12 = 0x%02X \n",
		 __func__, chg_en, mt6360_chg_status_name[chg_stat], chg_stat1, chg_ctrl12);
...
/**** [SX3-3201] Restore 0x17 register value when plugout usb cable: START ****/
	if (pwr_rdy == 0) {
		ret = mt6360_pmu_reg_write(mpci->mpi,
						 MT6360_PMU_CHG_CTRL7, 0x10);
		dev_info(mpci->dev, "%s: set MT6360_PMU_CHG_CTRL7 (0x%02x) = 0x10 | ret = %d\n", MT6360_PMU_CHG_CTRL7, __func__, ret);
	}
/**** [SX3-3201] Restore 0x17 register value when plugout usb cable: END   ****/
#ifdef CONFIG_MT6360_PMU_CHARGER_TYPE_DETECT
#ifndef CONFIG_TCPC_CLASS
	mutex_lock(&mpci->chgdet_lock);
...
// workaround for mt6768 
	...
	/* Enable TypeC OTP for check EVB version by TS pin */
	dev_info(mpci->dev, "[%s] Enable TypeC OTP for check EVB version by TS pin.\n", __func__);
	ret = mt6360_pmu_reg_set_bits(mpci->mpi, MT6360_PMU_TYPEC_OTP_CTRL,
