...
struct linux_thermal_ctrl_if {
	...
	struct thermal_cooling_device *cl_tethering_off_dev;
};
...
static unsigned int cl_tethering_off_dev_state;
...
/* +mtktspa_cooling_tethering_off_ops+ */
static int wmt_cl_tethering_off_get_max_state(
struct thermal_cooling_device *cool_dev, unsigned long *pv)
{
	*pv = 1;
	wmt_tm_dprintk("[%s] %lu\n", __func__, *pv);
...
static int wmt_cl_tethering_off_get_cur_state(
struct thermal_cooling_device *cool_dev, unsigned long *pv)
{
	*pv = cl_tethering_off_dev_state;
	wmt_tm_dprintk("[%s] %lu\n", __func__, *pv);
	return 0;
}

static int wmt_cl_tethering_off_set_cur_state(
struct thermal_cooling_device *cool_dev, unsigned long v)
{
	char event[20] = {0};
	char *envp[2] = { event, NULL };

	if (cl_tethering_off_dev_state != (unsigned int)v) {
		cl_tethering_off_dev_state = (unsigned int)v;
		sprintf(event, "TETHERING=%d", cl_tethering_off_dev_state?0:1);

		/* send uevent to notify tethering off */
		kobject_uevent_env(&(cool_dev->device.kobj),
			KOBJ_CHANGE, envp);
	}
...
static struct thermal_cooling_device_ops mtktspa_cooling_tethering_off_ops = {
	.get_max_state = wmt_cl_tethering_off_get_max_state,
	.get_cur_state = wmt_cl_tethering_off_get_cur_state,
	.set_cur_state = wmt_cl_tethering_off_set_cur_state,
};
...
static int wmt_tm_thz_cl_register(void)
{
	...
	p_linux_if->cl_tethering_off_dev =
		mtk_thermal_cooling_device_register("mtktswmt-teth-off", NULL,
						&mtktspa_cooling_tethering_off_ops);
...
static int wmt_tm_thz_cl_unregister(void)
{
	...
	if (p_linux_if->cl_tethering_off_dev) {
		mtk_thermal_cooling_device_unregister(p_linux_if->cl_tethering_off_dev);
		p_linux_if->cl_tethering_off_dev = NULL;
	}
