...
static int _cltsd_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "global_ui_warn_shown %d (%d)\n", (global_ui_warn_shown > 0)?1:0, global_ui_warn_shown);
	{
		...
		for (i = 0; i < MAX_NUM_INSTANCE_MTK_COOLER_USHUTDOWN; i++) {
			seq_printf(m, "[%02d] state %d (tz_idx %d)(T %d): [ %s %d %d %d %d %d ] %d %d %d %d\n", i,
...
static const struct file_operations _cltsd_fops = {
	.owner = THIS_MODULE,
	.open = _cltsd_open,
	.read = seq_read,
	.llseek = seq_lseek,
	...
	*((unsigned long *)cdev->devdata) = state;
	...
	switch (cl_ushutdown_timer_state[i].t_state) {
		...
		case TSD_CLEAR_COUNTING:
			...
			break;
		default:
			mtk_cooler_ushutdown_dprintk("%s unknown current tsd_state %d, state %d, prev_state %d\n", __func__, cl_ushutdown_timer_state[i].t_state, state, prev_state);
	}
	return 0;
}
...
static int __init mtk_cooler_ushutdown_init(void)
{
	int err = 0;
	int i;
...
static void __exit mtk_cooler_ushutdown_exit(void)
{
