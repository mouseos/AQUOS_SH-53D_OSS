...
/* SHARP EXTENDED Mod-Start CUST-ID:0006 */
#if 0
...
#define P61_SPI_CLOCK	  20000000L;
#endif
/* SHARP EXTENDED Mod-End CUST-ID:0006 */
...
#endif
/* SHARP EXTENDED Mod-End CUST-ID:0274 */
...
/* SHARP EXTENDED Add-Start CUST-ID:0011 */
	unsigned char *tx_buffer;
	unsigned char *rx_buffer;
/* SHARP EXTENDED Add-End CUST-ID:0011 */
...
/* SHARP EXTENDED Mod-Start CUST-ID:0274 */
#ifdef CONFIG_ANDROID_ENGINEERING
static unsigned char debug_spi_log = 0;
module_param(debug_spi_log, byte, 0644);

static void p61_write_log(const uint8_t *data, uint16_t size)
{
    int len1,len2,ofs;
    char work1[256];
    char work2[8];
    ...
    printk(KERN_INFO "[nfc-spi]write size=%d\n", size);
...
static void p61_read_log(uint8_t index, uint8_t *data, uint16_t size)
{
    int len1,len2,ofs;
    char work1[512];
    char work2[8];
    ...
    printk(KERN_INFO "[nfc-spi]read index=%d, size=%d\n", index, size);
    for( len1=0 ; len1<(size/16) ; len1++ ){
        printk(KERN_INFO "%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
               *(data+len1*16+0),
               *(data+len1*16+1),
			  ...
			  unsigned long arg)
{
	int ret = 0;
	struct p61_dev *p61_dev = NULL;
/* SHARP EXTENDED ADD-Start CUST-ID:0005 */
#ifdef P61_HARD_RESET
/* SHARP EXTENDED ADD-End CUST-ID:0005 */
	unsigned char buf[100];
/* SHARP EXTENDED ADD-Start CUST-ID:0005 */
#endif /* P61_HARD_RESET */
/* SHARP EXTENDED ADD-End CUST-ID:0005 */
...
/* SHARP EXTENDED ADD-Start CUST-ID:0274 */
#ifdef CONFIG_ANDROID_ENGINEERING
			p61_read_log(1, buf, sizeof(buf));
#endif
...
/* SHARP EXTENDED Mod-Start CUST-ID:0011 */
/*  unsigned char tx_buffer[MAX_BUFFER_SIZE]; */
	unsigned char *tx_buffer;
/* SHARP EXTENDED Mod-End CUST-ID:0011 */
...
/* SHARP EXTENDED ADD-Start CUST-ID:0274 */
#ifdef CONFIG_ANDROID_ENGINEERING
	p61_write_log(&tx_buffer[0], count);
#endif
/* SHARP EXTENDED ADD-End CUST-ID:0274 */
...
/*  unsigned char rx_buffer[MAX_BUFFER_SIZE]; */
	unsigned char *rx_buffer = p61_dev->rx_buffer;
/* SHARP EXTENDED Mod-End CUST-ID:0011 */
...
/* SHARP EXTENDED ADD-Start CUST-ID:0274 */
#ifdef CONFIG_ANDROID_ENGINEERING
		p61_read_log(2, &rx_buffer[0], count);
#endif
/* SHARP EXTENDED ADD-End CUST-ID:0274 */
		}
	} else {
...
/* SHARP EXTENDED ADD-Start CUST-ID:0274 */
#ifdef CONFIG_ANDROID_ENGINEERING
		p61_read_log(3, &rx_buffer[0], count);
#endif
/* SHARP EXTENDED ADD-End CUST-ID:0274 */
	}
...
static int p61_probe(struct spi_device *spi)
{
	...
	struct p61_dev *p61_dev = NULL;
/* SHARP EXTENDED DEL-Start CUST-ID:0006 */
//	unsigned int max_speed_hz;
/* SHARP EXTENDED DEL-End CUST-ID:0006 */
...
/* SHARP EXTENDED Add-Start CUST-ID:0011 */
	p61_dev->tx_buffer = NULL;
	p61_dev->rx_buffer = NULL;

	p61_dev->tx_buffer = kzalloc(MAX_BUFFER_SIZE, GFP_DMA | GFP_KERNEL);
	if (!p61_dev->tx_buffer) {
		ret = -ENOMEM;
		P61_ERR_MSG("%s :tx_buffer failed to allocate memory\n", __func__);
		goto err_exit0;
	}
	p61_dev->rx_buffer = kzalloc(MAX_BUFFER_SIZE, GFP_DMA | GFP_KERNEL);
	if (!p61_dev->rx_buffer) {
		ret = -ENOMEM;
		P61_ERR_MSG("%s :rx_buffer failed to allocate memory\n", __func__);
		goto err_exit0;
	}
/* SHARP EXTENDED Add-End CUST-ID:0011 */
...
/* SHARP EXTENDED Add-Start CUST-ID:0011 */
	if(p61_dev->tx_buffer != NULL)
		kfree(p61_dev->tx_buffer);
	if(p61_dev->rx_buffer != NULL)
		kfree(p61_dev->rx_buffer);
/* SHARP EXTENDED Add-End CUST-ID:0011 */
	if (p61_dev != NULL)
		kfree(p61_dev);
err_exit:
