...
static int fih_info_property(struct platform_device *pdev)
{
	...
	if (!p_chr) {
		pr_err("%s:%d, devmodel not specified\n", __func__, __LINE__);
	} else {
	...
	if (!p_chr) {
		pr_err("%s:%d, baseband not specified\n", __func__, __LINE__);
	} else {
	...
	if (!p_chr) {
		pr_err("%s:%d, bandinfo not specified\n", __func__, __LINE__);
	} else {
	...
	if (!p_chr) {
		pr_err("%s:%d, hwmodel not specified\n", __func__, __LINE__);
	} else {
