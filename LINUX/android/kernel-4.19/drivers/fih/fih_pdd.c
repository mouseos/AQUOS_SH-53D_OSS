...
#define FIH_PROC_PATH  "ckdt"  /* productdate */ /* MFG Date */

static char productdate[48];  /* "yyyymmdd" */

static int fih_pdd_proc_read_info(struct seq_file *m, void *v)
{
	seq_printf(m, "%s\n", productdate);
	return 0;
}

static int fih_pdd_proc_open_info(struct inode *inode, struct file *file)
{
	return single_open(file, fih_pdd_proc_read_info, NULL);
}

static const struct file_operations fih_pdd_fops_info = {
	.open    = fih_pdd_proc_open_info,
	.read    = seq_read,
	.llseek  = seq_lseek,
...
static int fih_pdd_property(struct platform_device *pdev)
{
	...
	if (!p_chr) {
		pr_info("%s:%d, productdate not specified\n", __func__, __LINE__);
	} else {
...
static int fih_pdd_probe(struct platform_device *pdev)
{
	...
	rc = fih_pdd_property(pdev);
	if (rc) {
		pr_err("%s Unable to set property\n", __func__);
		return rc;
	}

	if (proc_create(FIH_PROC_PATH, 0, NULL, &fih_pdd_fops_info) == NULL) {
...
MODULE_DEVICE_TABLE(of, fih_pdd_dt_match);
...
static int __init fih_pdd_init(void)
{
	...
	ret = platform_driver_register(&fih_pdd_driver);
	if (ret) {
		pr_err("%s: failed!\n", __func__);
		return ret;
	}
