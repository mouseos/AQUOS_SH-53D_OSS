...
static void jeita_current_limit(struct charger_manager *info)
{
...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (is_typec_adapter(info)) {
		...
		if( info->data.jeita_bat_current >=info->data.ac_charger_current)
		{
			pdata->charging_current_limit =
					info->data.ac_charger_current;
			chr_err("[current_limit] %s %d: jeita_bat_current = %d ,ac_charger_current =%d \n", __func__,__LINE__,info->data.jeita_bat_current,info->data.ac_charger_current);
			chr_err("[current_limit] %s %d: set IBAT current limit = %d\n", __func__,__LINE__,pdata->charging_current_limit);
		}else{
			pdata->charging_current_limit = info->data.jeita_bat_current ;
			chr_err("[current_limit] %s %d: jeita_bat_current = %d ,ac_charger_current =%d \n", __func__,__LINE__,info->data.jeita_bat_current,info->data.ac_charger_current);
			chr_err("[current_limit] %s %d: set IBAT current limit = %d\n", __func__,__LINE__,pdata->charging_current_limit);
		}
		/*jeita has higher priority end*/
		...
		chr_err("[current_limit] %s %d: set IBUS current limit = %d\n", __func__,__LINE__,pdata->input_current_limit);
		/*jeita has higher priority start*/
		if( info->data.jeita_bat_current >=info->data.charging_host_charger_current)
			{
			pdata->charging_current_limit =
					info->data.charging_host_charger_current;
			chr_err("[current_limit] %s %d: jeita_bat_current = %d ,charging_host_charger_current =%d \n", __func__,__LINE__,info->data.jeita_bat_current,info->data.charging_host_charger_current);
			chr_err("[current_limit] %s %d: set IBAT current limit = %d\n", __func__,__LINE__,pdata->charging_current_limit);
		}else{
			pdata->charging_current_limit = info->data.jeita_bat_current ;
			chr_err("[current_limit] %s %d: jeita_bat_current = %d ,charging_host_charger_current =%d \n", __func__,__LINE__,info->data.jeita_bat_current,info->data.charging_host_charger_current);
						...
						bool en)
{
	...
	if (en) {
		...
		/*SX3-300 Customize charging state report*/
		if(info->sw_jeita.charging == false){
			charger_manager_notifier(info, CHARGER_NOTIFY_NORMAL);
			chr_err("%s: en:%d %s . sw_jeita.charging == false, set /sys/class/power_supply/battery/status charging state\n", __func__, en, info->algorithm_name);
		}

		charger_dev_get_input_current(info->chg1_dev, &oldmA);
		if(info->direct_charging_en == true && oldmA > 0){
			charger_manager_notifier(info, CHARGER_NOTIFY_NORMAL);
			chr_err("%s: en:%d %s . direct_charging_en == true, current: %d\n", __func__, en, info->algorithm_name, oldmA);
		} else if(info->direct_charging_en == true && oldmA == 0) {
			charger_manager_notifier(info, CHARGER_NOTIFY_STOP_CHARGING);
			chr_err("%s: en:%d %s . direct_charging_en == true, no current\n", __func__, en, info->algorithm_name);
		}
		if(info->power_path_en == false ){
			charger_manager_notifier(info, CHARGER_NOTIFY_STOP_CHARGING);
			chr_err("%s: en:%d %s . power_path_en == false, set /sys/class/power_supply/battery/status discharging state\n", __func__, en, info->algorithm_name);
		}
	}
...
static int select_pdc_charging_current_limit(struct charger_manager *info)
{
	...
	jeita_current_limit(info);
	if( info->data.jeita_bat_current >=info->data.pd_charger_current)
	{
		pdata->charging_current_limit =
			info->data.pd_charger_current;
		chr_err("[current_limit] %s %d: jeita_bat_current = %d ,pd_charger_current =%d \n", __func__,__LINE__,info->data.jeita_bat_current,info->data.pd_charger_current);
		chr_err("[current_limit] %s %d: set IBAT current limit = %d\n", __func__,__LINE__,pdata->charging_current_limit);
	}
	else
	{
