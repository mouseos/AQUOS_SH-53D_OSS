...
//#define BATTERY_DAMAGE_FILE_PATH		"/durable/shpwr/cl_info"
...
extern bool mt6360_power_path_report_en;
...
static int cc_safety_level_val = 0;
...
/**** [SX3-696][REQ][Charger][SW-PMIC-0043]Disable charger when USB TypeC connector becomes too hot ---- START ****/
	...
	chr_debug("[TYPEC_OTP] boot_mode: %d\n", boot_mode);

	TTemperature = get_ts_temp(dev);
	if (cc_safety_level_test_enable) {
		if (cc_safety_level_test_val >= TYPEC_OTP_TEMP_MIN && cc_safety_level_test_val <= TYPEC_OTP_TEMP_MAX) {
			chr_err("[TYPEC_OTP] TypeC temperature is changed forcedly from %d to %d\n", TTemperature, cc_safety_level_test_val);
			TTemperature = cc_safety_level_test_val;
		} else
	...
	if (TTemperature >= TYPEC_OTP_ALERT_THRESHOLD) {
		...
		if (cc_safety_level_val == 0) {
			chr_info("[TYPEC_OTP] Start over-temperature protection...\n");
			ret = mt_disable_cc(true);
			cc_safety_level_val = 3;
		...
		if (cc_safety_level_val == 3)
			ret = mt_disable_cc(true);
		else
			ret = mt_disable_cc(false);
		chr_info("[TYPEC_OTP][45-55] Keep early status to %s CC ...\n", cc_safety_level_val == 3 ? "disable" : "enable");
	}
	else {
		if (cc_safety_level_val == 3) {
			chr_info("[TYPEC_OTP] Stop over-temperature protection...\n");
			ret = mt_disable_cc(false);
			cc_safety_level_val = 0;
		}
	}

	chr_info("[TYPEC_OTP] TS Temp: %05d | info->battery_temp: %d | cc_safety_level: %d | cc_safety_level_test_enable: %d\n",
				TTemperature, info->battery_temp, cc_safety_level_val, cc_safety_level_test_enable);
...
static void charger_check_status(struct charger_manager *info)
{
	...
	info->power_path_en = mt6360_power_path_report_en ;
	...
	*/
	...
	if (info->sc.disable_charger == true)
		charging = false;
	if(info->power_path_en == false)
		charging = false;
	/*
	if(info->direct_charging_en == false){
		charging = false;
	}
	*/
...
/*
static int file_read(const char* filename,int offset, void *data, int data_size)
{
	struct file *file;
	loff_t pos;
	int rc = 0;
	...
	file = filp_open(filename, O_CREAT | O_WRONLY  , 0666);
	if (IS_ERR(file)) {
		rc = PTR_ERR(file);
		pr_err("filp_open 01 %s error %ld", filename, rc);
		goto error;
	}

	pos = offset;
	rc = kernel_read(file, data, data_size, &pos);
	if (rc < 0) {
		pr_err("kernel_read %s error %d\n", filename, rc);
		goto close;
	}
...
static int file_write(const char* filename,int offset, void *data, int data_size)
{
	struct file *file;
	loff_t pos;
	int rc = 0;

	file = filp_open(filename, O_WRONLY | O_NONBLOCK, 0);
	if (IS_ERR(file)) {
		rc = PTR_ERR(file);
		pr_err("filp_open 02 %s error %ld", filename, rc);
		goto error;
	}

	pos = offset;
	rc = kernel_write(file, data, data_size, &pos);
	if (rc < 0) {
		pr_err("kernel_write %s error %d\n", filename, rc);
		goto close;
	}
...
static void battery_damage_decrease(struct charger_manager *info)
{
	...
	chr_err("%s: now: %ld now_coulomb =%d  ,tmp: %d deg ,real_capacity= %d , stat =%d\n", __func__, time_now.tv_sec, gauge_get_coulomb()/10,temp,real_capacity/100,damage_info->stat);
	

	if(temp >set_temp && real_capacity>set_capacity && damage_info->stat == 1){
		chr_err("[%s] battery is damaged due to temperature > 40deg and real capacity > 90 \n", __func__);
		...
		damage_info->start_coulomb = gauge_get_coulomb()/10;
		chr_err("[%s] start_time = %d sec , start_coulomb = %d mAh \n", __func__,damage_info->start_time,damage_info->start_coulomb);
		...
		damage_info->end_coulomb = gauge_get_coulomb()/10;
		chr_err("[%s] end_time = %d sec , end_coulomb = %d mAh \n", __func__,damage_info->end_time,damage_info->end_coulomb);
...
static ssize_t show_ADC_Charger_Current(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int ibus = (read_mt6360_ibus_node_uA/1000); /*mA*/
	pr_debug("[%s]: %d\n", __func__, ibus);
	return sprintf(buf, "%d\n", ibus);
}
		...
		struct device_attribute *attr, const char *buf,  size_t size)
{
	...
	if (kstrtoint(buf, 10, &capacity) == 0) 
	{ 
		direct_charging_set_capacity = capacity;
		pr_err("[%s] capacity_setting = %d \n", __func__,capacity); 
...
ssize_t direct_charging_show(struct device *dev, struct device_attribute *attr, char *buf)
{

    return sprintf(buf, "direct_charging_set_capacity =%d \n",direct_charging_set_capacity);
}
...
static ssize_t show_cc_safety_level_test_enable(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	pr_info("[TYPEC_OTP] -------- Get cc_safety_level_test_enable: %lu (mV)\n", cc_safety_level_test_enable);
	return sprintf(buf, "%d\n", cc_safety_level_test_enable);
}
		...
		struct device_attribute *attr, const char *buf,  size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		cc_safety_level_test_enable = reg;
		pr_info("[TYPEC_OTP] -------- Set cc_safety_level_test_enable: %d\n", cc_safety_level_test_enable);
		if (cc_safety_level_test_enable == 0 && cc_safety_level_val == 3) {
			...
			pr_info("[TYPEC_OTP] -------- Disable test => recovery USB\n", cc_safety_level_test_enable);
			cc_safety_level_val = 0;
			ret = mt_disable_cc(false);
		}
	}
		...
		struct device_attribute *attr, const char *buf,  size_t size)
{
	...
	if (buf != NULL && size != 0) {
		ret = kstrtouint(buf, 10, &reg);
		cc_safety_level_test_val = reg * 1000;
		...
		struct device_attribute *attr, const char *buf,  size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		if (reg == 1)
			...
			ret = mt_disable_cc(true);
		pr_info("[TYPEC_OTP] -------- %s CC forcibly (%u): %s by 'cc_forcibly_enable'!\n", reg == 1 ? "enables" : "disables", reg, ret == 0 ? "succeed" : "fail");
	}
	else
		pr_err("[TYPEC_OTP] buf is NULL or size is zero!\n");
...
void handle_cc_recovery(struct device *dev)
{
	...
	if (cc_safety_level_val == 3) {
		ts_delta = timespec_sub(ts_now, g_ts_otp_enable);
		chr_info("[TYPEC_OTP] -------- cc_safety_level_val: 3 | ts_delta.tv_sec: %d sec\n", ts_delta.tv_sec);
		if (ts_delta.tv_sec >= CC_RECOVERY_TIME_INTERVAL_SEC) {
			chr_info("[TYPEC_OTP] ts_delta.tv_sec (%d) >= %d ----> recover CC => reset cc_safety_level => set testing temp to 25 degree", ts_delta.tv_sec, CC_RECOVERY_TIME_INTERVAL_SEC);
			cc_safety_level_val = 0;
			ret = mt_disable_cc(false);
			// reset test arguments
			cc_safety_level_test_val = 25000;
			cc_safety_level_test_enable = 0;
		}
	}
}
/**** [SX3-696][REQ][Charger][SW-PMIC-0043]Disable charger when USB TypeC connector becomes too hot ---- END   ****/

/**** [SX3-3910] Export cc_safety_level for battery module usage: START ****/
int read_cc_safety_level(void)
{
	return cc_safety_level_val;
}
/**** [SX3-3910] Export cc_safety_level for battery module usage: END   ****/

static ssize_t show_battery_deterioration(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
ssize_t store_battery_deterioration(struct device *dev,
		struct device_attribute *attr, const char *buf,  size_t size)
{
	int scale;
	...
	{ 
		...
		pr_err("[%s] battery_deterioration_set_scale = %d ,set 200 will get default value \n", __func__,scale); 
...
static DEVICE_ATTR(battery_deterioration, 0644, show_battery_deterioration, store_battery_deterioration);



static ssize_t show_bat_vol_decrease(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
ssize_t store_bat_vol_decrease(struct device *dev,
		struct device_attribute *attr, const char *buf,  size_t size)
{
	int scale;
...
static DEVICE_ATTR(bat_vol_decrease, 0644, show_bat_vol_decrease, store_bat_vol_decrease);
...
ssize_t store_total_coulomb(struct device *dev,
		struct device_attribute *attr, const char *buf,  size_t size)
{
	int scale;
...
static DEVICE_ATTR(total_coulomb, 0644, show_total_coulomb, store_total_coulomb);
...
#if 1 /*CONFIG_BATTERY_FLC FTM script start*/
static ssize_t oem_show_Input_Current(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct charger_manager *pinfo = dev->driver_data;
...
static ssize_t oem_store_Input_Current(struct device *dev, struct device_attribute *attr, const char *buf,
				 size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	unsigned int reg = 0;
	...
	if (kstrtoint(buf, 10, &reg) == 0) {
		if (reg <= 0){
			pinfo->chg1_data.thermal_input_current_limit = 0;
			pinfo->data.ac_charger_input_current= 0;
			printk("%s reg=%d buff %s \n", __func__,reg ,buf);
		}
		else {
			pinfo->chg1_data.thermal_input_current_limit = reg;
			printk("%s reg=%d buff %s \n", __func__,reg ,buf);
...
static DEVICE_ATTR(oem_Input_Current, 0664, oem_show_Input_Current, oem_store_Input_Current);

#endif /*CONFIG_BATTERY_FLC FTM script END*/
	...
	if (sscanf(desc, "%d %d", &current_unlimited, &cmd_discharging) == 2) {
		...
		if (cmd_discharging == 1) {
			...
			charger_manager_notifier(pinfo,
						CHARGER_NOTIFY_NORMAL); *//* keep show charging when direct charging happened */
...
static DEVICE_ATTR(oem_enable_charging, 0664, oem_enable_charging_show, oem_enable_charging_store);
		...
		const char *buffer, size_t count, loff_t *data)
{
	...
	if (ret == 0) {
		force_power_path_en = enable;
		echo_power_path = true;
...
#if 1 /*CONFIG_BATTERY_FLC FTM script*/
	ret = device_create_file(&(pdev->dev), &dev_attr_oem_Input_Current);
	if (ret)
		goto _out;
#endif /*CONFIG_BATTERY_FLC FTM script END*/
...
void sc_select_charging_current(struct charger_manager *info, struct charger_data *pdata)
{
	...
	if (pdata->thermal_charging_current_limit != -1) {
		if (pdata->thermal_charging_current_limit <
			...
			pdata->charging_current_limit =
					pdata->thermal_charging_current_limit;
			chr_err("[current_limit] %s %d: set IBAT current limit = %d\n", __func__,__LINE__,pdata->charging_current_limit);
			}
