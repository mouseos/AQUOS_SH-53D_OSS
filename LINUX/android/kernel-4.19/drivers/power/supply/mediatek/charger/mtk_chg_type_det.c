	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TYPEC_CC_ORIENTATION:
		if (mtk_chg == NULL) {
			printk(KERN_WARNING "[%s] mtk_chg == NULL\n", __func__);
			val->intval = TYPEC_CC_UNKNOWN;
		}
		else if (mtk_chg->cti == NULL) {
			printk(KERN_WARNING "[%s] mtk_chg->cti == NULL\n", __func__);
			val->intval = TYPEC_CC_UNKNOWN;
		}
		else if (mtk_chg->cti->tcpc == NULL) {
			printk(KERN_WARNING "[%s] mtk_chg->cti->tcpc_dev == NULL\n", __func__);
			val->intval = TYPEC_CC_UNKNOWN;
		}
		else {
			retcc = tcpm_inquire_remote_cc(mtk_chg->cti->tcpc, &ucc1, &ucc2, false);
			printk(KERN_WARNING "[%s] ret: %d | cc1: %u | cc2: %u\n", __func__, retcc, ucc1, ucc2);
			if (ucc1 == 0 && ucc2 == 5)
				val->intval = TYPEC_CC_OBVERSE;
			else if (ucc1 == 5 && ucc2 == 0)
				val->intval = TYPEC_CC_REVERSE;
			else if ((ucc1 == 0 && ucc2 == 0) || (ucc1 == 15 && ucc2 == 15))
				val->intval = TYPEC_CC_NONE;
			else {
				printk(KERN_ERR "[%s] Unknown CC state! ret: %d | cc1: %u | cc2: %u\n", __func__, retcc, ucc1, ucc2);
				val->intval = TYPEC_CC_UNKNOWN;
			}
		}
		break;
...
int mt_disable_cc(bool disable) {
	int ret = 0;
	struct power_supply *usb_psy = power_supply_get_by_name("usb");
	struct mt_charger *mtk_chg = power_supply_get_drvdata(usb_psy);
	ret = tcpm_typec_disable_function(mtk_chg->cti->tcpc, disable);
	printk(KERN_WARNING "[TYPEC_OTP] %s CC %s!\n", disable == 1 ? "Disable" : "Enable", ret == 0 ? "succeed" : "fail");
