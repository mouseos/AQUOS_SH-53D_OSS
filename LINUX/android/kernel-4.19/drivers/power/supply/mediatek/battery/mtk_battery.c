...
static int set_temperature = original_temperature;

/**** [SX3-2669] global variants: START ****/
// BATTERY REPORT
static struct timespec g_ts_last_batt_report_chg;
static struct timespec g_ts_last_batt_report_norm;
...
static struct timespec g_ts_last_ui_setting_charging,g_ts_last_ui_setting_discharging;
static int g_chg_ui_setting_interval_sec = 60;

/* [SX3-3649] sharp requirement when current below 200mA show 100% end*/

#ifdef CONFIG_FIH_BATTERY
static struct timespec g_ts_charging_start;
static bool g_is_charging_on = false;
#endif
...
#ifdef CONFIG_SHR_BATTERY
	POWER_SUPPLY_PROP_FUELGAUGE_CAPACITY,
	POWER_SUPPLY_PROP_AGE_PERCENTAGE,
#endif
};
...
static void write_BATTERY_REPORT_to_battlog(int batt_report_status) {
	...
	if (batt_report_status == g_last_batt_report_status) {
		ts_delta = timespec_sub(ts_now, g_ts_last_batt_report_chg);
		if (g_last_batt_report_status == POWER_SUPPLY_STATUS_CHARGING) {
			if (ts_delta.tv_sec >= g_batt_report_time_interval_sec_chg)
				write_battlog(SH_BATTLOG_BATT_REPORT_CHG);
			else
				return;
		} else {
			if (ts_delta.tv_sec >= g_batt_report_time_interval_sec_norm)
	...
	g_ts_last_batt_report_chg = ts_now;
...
static void write_FGIC_LEVEL_to_battlog(int batt_capacity){
	if(batt_capacity % 10 == 0) {
		...
		if(batt_capacity == 100)
			...
			bm_err("[%s] Invalid battery capacity: %d!\n", __func__, batt_capacity);
			return;
		}
...
int repare_skip_cap(int ui_capacity){
	...
	ui_capacity==58||ui_capacity==63||ui_capacity==68||ui_capacity==72||ui_capacity==77)&&is_bat_charging ==1 ){
		...
		if(ui_mapping_increase_count>=11){
			ui_capacity+=1;
			bm_err("[repare_skip_cap] modify_ui = %d",ui_capacity);
		}
	}else{
...
static int Uisoc_100persent_settiing(int ui_capacity)
{
	int fgcurrent = 0;
	bool b_ischarging = 0;
	struct timespec ts_now_charging, ts_delta_ui_charging,ts_now_discharging, ts_delta_ui_discharging;
	...
	get_monotonic_boottime(&ts_now_charging);
	get_monotonic_boottime(&ts_now_discharging);
	if(fgcurrent<2030 && fgcurrent >0 && ui_capacity>94 ){
	/*
	fix the persent change too fast
	*/
		bm_err("[terminal_current_setting]%s %d capacity = %d %%  ",__func__,__LINE__,ui_capacity);
		if(cus_cap_variant.current_below_200ma_count == 0){
			...
			bm_err("[terminal_current_setting]%s %d init pre_capacity=%d %% capacity = %d %%  ",__func__,__LINE__,cus_cap_variant.pre_capacity ,ui_capacity);
			bm_err("[terminal_current_setting]%s %d  ts_now_charging.tv_sec = %d, init g_ts_last_ui_setting_charging.tv_sec = %d ,ts_delta_ui_charging.tv_sec = %d"
		,__func__,__LINE__,ts_now_charging.tv_sec, g_ts_last_ui_setting_charging.tv_sec,ts_delta_ui_charging.tv_sec);
		}

		ts_delta_ui_charging = timespec_sub(ts_now_charging, g_ts_last_ui_setting_charging);
		bm_err("[terminal_current_setting]%s %d capacity = %d %%  ts_now_charging.tv_sec = %d, g_ts_last_ui_setting_charging.tv_sec = %d ,ts_delta_ui_charging.tv_sec = %d"
		...
		if(ts_delta_ui_charging.tv_sec >= g_chg_ui_setting_interval_sec){
			if(ui_capacity < 100){
				if((cus_cap_variant.pre_capacity+1)==ui_capacity){
					bm_err("[terminal_current_setting]%s %d capacity = %d %%  do not add capacity by function ",__func__,__LINE__,ui_capacity);
				}else{
					ui_capacity = cus_cap_variant.pre_capacity +1;
					bm_err("[terminal_current_setting]%s %d capacity modeify = %d %%  ",__func__,__LINE__,ui_capacity);
				}
				if(ui_capacity >=100 )
					ui_capacity = 100;
			...
			bm_err("[terminal_current_setting]%s %d pre_capacity = %d %%, capacity = %d %% ,current = %d mA ",__func__,__LINE__,cus_cap_variant.pre_capacity,ui_capacity,fgcurrent/10);
		}else {
			/*Fix the issue of the battery level not being maintained within one minute.*/
			bm_err("[terminal_current_setting]%s %d Within a minute,do not change capacity,return pre_capacity=%d %% ,original capacity = %d %%  ",__func__,__LINE__,cus_cap_variant.pre_capacity,ui_capacity);
			ui_capacity = cus_cap_variant.pre_capacity;
			bm_err("[terminal_current_setting]%s %d Within a minute,do not change capacity,return pre_capacity=%d %% ,modify capacity = %d %%  ",__func__,__LINE__,cus_cap_variant.pre_capacity,ui_capacity);
			
		}
	/*Fix the issue of battery level fluctuation caused by the fluctuation of 203mA.*/
	}else if(b_ischarging == true && cus_cap_variant.pre_capacity > ui_capacity && cus_cap_variant.modified_cap ==true){
		bm_err("[terminal_current_setting]%s %d return pre_capacity=%d %% ,original capacity = %d %%  ",__func__,__LINE__,cus_cap_variant.pre_capacity,ui_capacity);
		ui_capacity = cus_cap_variant.pre_capacity;
		bm_err("[terminal_current_setting]%s %d return pre_capacity=%d %% ,modify capacity = %d %%  ",__func__,__LINE__,cus_cap_variant.pre_capacity,ui_capacity);
	/*when discharge, and the capacity is modified by this function, run below code */
		...
		ts_delta_ui_discharging = timespec_sub(ts_now_discharging, g_ts_last_ui_setting_discharging);
		bm_err("[terminal_current_setting]%s %d capacity = %d %%  ts_now_discharging.tv_sec = %d, g_ts_last_ui_setting_discharging.tv_sec = %d ,ts_delta_ui_discharging.tv_sec = %d"
		...
		if(cus_cap_variant.pre_capacity == ui_capacity){
			bm_err("[terminal_current_setting]%s %d discharge capacity = %d %%  leave loop",__func__,__LINE__,ui_capacity);
	...
	union power_supply_propval *val)
{
	int ret = 0;
	int fgcurrent = 0;
	int cc_safety_level = read_cc_safety_level();
	bool b_ischarging = 0;
	...
	case POWER_SUPPLY_PROP_STATUS:
		/**** [SX3-3910] Set not charging if USB OTP enabled: START ****/
		if (cc_safety_level == 3)
			data->BAT_STATUS = POWER_SUPPLY_STATUS_NOT_CHARGING;
		/**** [SX3-3910] Set not charging if USB OTP enabled: END   ****/
	...
	case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
		...
		if ((ret == POWER_SUPPLY_CAPACITY_LEVEL_FULL) ||
			...
			ts_delta = timespec_sub(ts_now, g_ts_charging_start);
			if(g_is_charging_on == true && ts_delta.tv_sec >= 30) {
				gauge_get_current(&fgcurrent);
				//fgcurrent = gauge_get_average_current(&valid);
			} else {
				fgcurrent = 0;
			}
#else
...
#endif
#ifdef CONFIG_SHR_BATTERY
	case POWER_SUPPLY_PROP_FUELGAUGE_CAPACITY:
		val->intval = gm.soc;
		break;
	case POWER_SUPPLY_PROP_AGE_PERCENTAGE:
		val->intval = 65535;
		break;
#endif
	...
	write_BATTERY_REPORT_to_battlog(data->BAT_STATUS);
	write_CHG_STATUS_to_battlog(data->BAT_STATUS);
	return ret;
}
...
int force_get_tbat(bool update)
{
	...
	/* set temperature file node */
	bm_debug("[%s] set_temperature = %d\n", __func__,set_temperature);
	if(set_temperature != original_temperature){
		gm.tbat_precise = set_temperature; //temp filenode value setting
		bat_temperature_val = set_temperature/10;
	}
...
static void nl_data_handler(struct sk_buff *skb)
{
	...
	/**** [SX3-696] Handle cc recovery cyclically: START ****/
	handle_cc_recovery(gm.gdev->dev.parent);
	/**** [SX3-696] Handle cc recovery cyclically: END   ****/
...
static ssize_t store_Temperature_Setting( 
	struct device *dev, struct device_attribute *attr, 
						const char *buf, size_t size)
{ 
	int temp;
	...
	{ 
		...
		bm_err("[%s] temperature_setting = %d \n", __func__,temp); 
...
/* START CHARGING */
#ifdef CONFIG_FIH_BATTERY
			get_monotonic_boottime(&g_ts_charging_start);
			g_is_charging_on = true;
#endif
...
/**** [SX3-2669] initiate last timespecs: START ****/
	get_monotonic_boottime(&g_ts_last_batt_report_chg);
	get_monotonic_boottime(&g_ts_last_batt_report_norm);
/**** [SX3-2669] initiate last timespecs: END   ****/
/**** [SX3-3649] initiate last timespecs: START ****/
	get_monotonic_boottime(&g_ts_last_ui_setting_charging);
	get_monotonic_boottime(&g_ts_last_ui_setting_discharging);
/**** [SX3-3649] initiate last timespecs: END   ****/
