...
{
    tpd_gpio_output(stx_gpts.host_if->reset_gpio, 1); //ugrec_tky gpio_direction_output(tpd_rst_gpio_number, 0);
    msleep(10);
    tpd_gpio_output(stx_gpts.host_if->reset_gpio, 0); //ugrec_tky gpio_direction_output(tpd_rst_gpio_number, 1);
    msleep(10);
    tpd_gpio_output(stx_gpts.host_if->reset_gpio, 1); //ugrec_tky gpio_direction_output(tpd_rst_gpio_number, 0);
    msleep(150);
}
...
static int tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    stx_gpts.input_dev = tpd->dev; //hfst add
	...
	if(err < 0)
	{
        ...
        tpd_gpio_output(stx_gpts.host_if->reset_gpio, 0);
        msleep(1);
	...
	for (i = 0; i < STX_KEY_NUM_MAX; i++) {
		input_set_capability(tpd->dev, EV_KEY, sitronix_sensor_key_array[i].code);
	}
#endif
    stx_vendor_ID();
    if(fih_st_tp_chip_type == 0){
                                                ...
                                                char *pre_key_status, char cur_key_status, int key_count)
{
	...
	for(i = 0; i < key_count; i++){
		if(cur_key_status & (1 << i)){
			...
			input_report_key(tpd->dev, key_array[i].code, 1);
			input_sync(tpd->dev);
		}else{
			if(*pre_key_status & (1 << i)){
				STX_INFO("sensor key [%d] up\n", i);
				input_report_key(tpd->dev, key_array[i].code, 0);
				input_sync(tpd->dev);
			}
		}
	}
...
#endif
		input_report_key(tpd->dev, BTN_TOUCH, touchCount> 0);
        input_sync(tpd->dev);
#ifdef SITRONIX_SENSOR_KEY
...
#endif
    msleep(10);
    tpd_gpio_output(stx_gpts.host_if->reset_gpio, 1);
