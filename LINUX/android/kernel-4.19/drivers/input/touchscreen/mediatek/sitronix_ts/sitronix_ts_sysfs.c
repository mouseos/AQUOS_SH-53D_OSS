...
static ssize_t st_inform_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	sret = snprintf(buf, 1000,
					...
					stx_gpts.ts_dev_info.rx_chs, stx_gpts.ts_dev_info.tx_chs,
...
static ssize_t st_glove_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t sret;
	STX_DEBUG("%s", __func__);
	mutex_lock(&stx_gpts.dev_mutex);
	...
	sret = snprintf(buf, 100,
					"Sitronix glove_mode = %s\n", stx_gpts.glove_mode ? "on" : "off");
#else
	sret = snprintf(buf, 100,
					"Don't support ST_GLOVE_SWITCH_MODE mode\n");
#endif
...
static ssize_t st_glove_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	STX_DEBUG("%s", __func__);
	mutex_lock(&stx_gpts.dev_mutex);
#ifdef ST_GLOVE_SWITCH_MODE
...
static ssize_t st_cases_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t sret;
	STX_DEBUG("%s", __func__);
	mutex_lock(&stx_gpts.dev_mutex);
	...
	sret = snprintf(buf, 100,
					"Sitronix cases_mode = %s\n", stx_gpts.cases_mode ? "on" : "off");
#else
	sret = snprintf(buf, 100,
					"Don't support ST_CASES_SWITCH_MODE mode\n");
#endif
...
static ssize_t st_cases_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	STX_DEBUG("%s", __func__);
	mutex_lock(&stx_gpts.dev_mutex);
#ifdef ST_CASES_SWITCH_MODE
...
static ssize_t st_rwreg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (num_chars == 4)
	{
		...
		STX_INFO("%s reg_addr:0x%x reg_data:0x%x", __func__, reg_addr, reg_data);
		/*write register*/
		buffer[0] = reg_addr;
		buffer[1] = reg_data;
...
static ssize_t st_forceUpg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	char spath[40] = {0};
	STX_DEBUG("%s", __func__);
	mutex_lock(&stx_gpts.dev_mutex);
...
static ssize_t st_manual_rst_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	STX_DEBUG("%s", __func__);
...
static ssize_t st_smart_wakeup_switch_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	STX_DEBUG("%s", __func__);
	mutex_lock(&stx_gpts.dev_mutex);
...
static ssize_t st_temporary_set_i2caddr_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	char addr;
	STX_DEBUG("%s", __func__);
...
static DEVICE_ATTR(st_smtwakeup, S_IRUGO | S_IWUSR, NULL, st_smart_wakeup_switch_store);

static DEVICE_ATTR(st_i2caddr_set, S_IRUGO | S_IWUSR, NULL, st_temporary_set_i2caddr_store);
...
int st_create_sysfs(struct i2c_client *client)
{
	...
	if (0 != err)
	{
		STX_ERROR("%s() - ERROR: sysfs_create_group() failed.", __func__);
