...
int st_get_status(struct sitronix_ts_data *ts)
{
	...
	if (ret != 8)
	{
		STX_ERROR(" i2c communcate error in getting status : 0x%x", ret);
		return -1;
	}
...
int st_get_dev_status(struct i2c_client *client, uint8_t *err_code, uint8_t *dev_status)
{
	int ret = 0;
	uint8_t buffer[8];
...
int st_print_version(struct sitronix_ts_data *ts)
{
	...
	if (ret != 4)
	{
		STX_ERROR(" i2c communcate error in getting FW reversion : 0x%x", ret);
		return -1;
	}
	STX_INFO("Touch Panel Firmware Revision %x %x %x %x", buffer[0], buffer[1], buffer[2], buffer[3]);
	snprintf(ts->ts_dev_info.fw_revision, 32, "%x.%x.%x.%x", buffer[0], buffer[1], buffer[2], buffer[3]);
...
static int sitronix_get_resolution(struct sitronix_ts_data *ts)
{
	...
	{
		ts->ts_dev_info.x_res = ((buffer[0] & (X_RES_H_BMSK << X_RES_H_SHFT)) << 4) | buffer[1];
		ts->ts_dev_info.y_res = ((buffer[0] & Y_RES_H_BMSK) << 8) | buffer[2];
		STX_INFO("resolution = %d x %d", ts->ts_dev_info.x_res, ts->ts_dev_info.y_res);
	}
	return 0;
}
...
int st_enter_glove_mode(struct sitronix_ts_data *ts)
{
	...
	if (1) //!ts->glove_mode)
	{
		...
		if (ret < 0)
		{
			STX_ERROR(" i2c communcate error in getting DeviceControl : 0x%x", ret);
			return ret;
		}
		...
		if (ret < 0)
		{
			STX_ERROR(" i2c communcate error in setting DeviceControl : 0x%x", ret);
			return ret;
		}
		ts->glove_mode = true;
	}
