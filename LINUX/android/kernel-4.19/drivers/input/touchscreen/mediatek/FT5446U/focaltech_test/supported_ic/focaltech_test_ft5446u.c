...
static int short_get_ab_ch(u8 *ab_ch, u8 delay, u8 mode, bool is_300, int *r_buf)
{
    ...
    /* set delay to 0 */
    ret = fts_test_write_reg(FACTROY_REG_SHORT_RES_LEVEL, delay);
    if (ret < 0) {
...
static int ft5472_noise_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    int i = 0;
    ...
    if (!thr->noise_min || !thr->noise_max) {
        FTS_TEST_SAVE_ERR("noise_min/max is null\n");
        ret = -EINVAL;
        goto test_err;
    }
	...
	ret = fts_test_read_reg(FACTORY_REG_FRE_LIST, &reg_0a_val);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("read normalize fail,ret=%d\n", ret);
        goto test_err;
    }

	ret = fts_test_read_reg(FACTORY_REG_DATA_SELECT, &reg_06_val);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("read normalize fail,ret=%d\n", ret);
        goto test_err;
    }

	ret = fts_test_read_reg(FACTORY_REG_ORDER_ADDR_H, &reg_1a_val);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("read normalize fail,ret=%d\n", ret);
        goto test_err;
    }

	ret = fts_test_read_reg(FACTORY_REG_ORDER_ADDR_L, &reg_1b_val);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("read normalize fail,ret=%d\n", ret);
        goto test_err;
    }
    ...
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("write FACTORY_REG_FRE_LIST fail,ret=%d\n", ret);
        goto restore_reg;
    }
    ...
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("write FACTORY_REG_DATA_SELECT fail,ret=%d\n", ret);
        goto restore_reg;
    }

	ret = fts_test_write_reg(FACTORY_REG_ORDER_ADDR_H, 0x01);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("write FACTORY_REG_DATA_SELECT fail,ret=%d\n", ret);
        goto restore_reg;
    }

    FTS_TEST_INFO("noise frame num:%d", thr->basic.noise_framenum);
    ret = fts_test_write_reg(0x1c, thr->basic.noise_framenum);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("write 0x1c fail,ret=%d\n", ret);
        goto restore_reg;
    }

	ret = fts_test_write_reg(FACTORY_REG_ORDER_ADDR_L, 0x01);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("write FACTORY_REG_DATA_SELECT fail,ret=%d\n", ret);
        goto restore_reg;
    }
    ...
    /* compare */
    tmp_result = compare_array(noise,
                               thr->noise_min,
                               thr->noise_max,
                               false);
...
restore_reg:
    /* set the origin value */
    ret = fts_test_write_reg(FACTORY_REG_DATA_SELECT, reg_06_val);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("restore FACTORY_REG_DATA_SELECT fail,ret=%d\n", ret);
    }

    ret = fts_test_write_reg(FACTORY_REG_FRE_LIST, reg_0a_val);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("restore FACTORY_REG_FRE_LIST fail,ret=%d\n", ret);
    }

	ret = fts_test_write_reg(FACTORY_REG_ORDER_ADDR_H, reg_1a_val);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("restore FACTORY_REG_ORDER_ADDR_H fail,ret=%d\n", ret);
    }

    ret = fts_test_write_reg(FACTORY_REG_ORDER_ADDR_L, reg_1b_val);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("restore FACTORY_REG_ORDER_ADDR_L fail,ret=%d\n", ret);
    }
test_err:
    /* result */
    if (tmp_result) {
...
static int start_test_ft5472(void)
{
    ...
    if (true == test_item->noise_test) {
        ret = ft5472_noise_test(tdata, &temp_result);
...
static int param_init_ft5472(void)
{
