...
#include "fih/fih_tray.h"  /* FIH: for /proc/AllHWlist/tray_status */
	...
	if (host->mrq) {
		...
		if (host->cmd) {
			...
			if (host->mrq->data->flags & MMC_DATA_WRITE) {
				/* data write timeout */
				printk("FIHBATTLOG::103\n");//SD_ERROR_WRITE_DATA_TIMEOUT
			} else {
				/* data read timeout*/
				printk("FIHBATTLOG::96\n");//SD_ERROR_READ_DATA_TIMEOUT
			}
...
static int msdc_get_cd(struct mmc_host *mmc)
{
	...
	/* FIH, Simon: Add battery log for SD card detected insert */
	...
	fih_tray_msdc(mmc_gpio_get_cd(mmc));  /* FIH: for /proc/AllHWlist/tray_status */
