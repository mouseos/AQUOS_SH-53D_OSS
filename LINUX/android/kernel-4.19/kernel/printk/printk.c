...
// Added for battery log by Todd 20230105[
#define __LOG_BLS_BUF_LEN 1024
DECLARE_WAIT_QUEUE_HEAD(bls_log_wait);
static DEFINE_RAW_SPINLOCK(bls_logbuf_lock);
...
int do_syslog(int type, char __user *buf, int len, int source)
{
	...
	case SYSLOG_ACTION_GET_KERNEL_BUFFER:
		...
		error = wait_event_interruptible(bls_log_wait,
							(log_bls_start - log_bls_end));
		if (error)
			goto out;
		i = 0;
	
		raw_spin_lock_irq(&bls_logbuf_lock);
	
		while (!error&&(log_bls_start != log_bls_end)&&i < len) {
			c = LOG_BLS_BUF(log_bls_start);
			log_bls_start++;
			raw_spin_unlock_irq(&bls_logbuf_lock);
...
asmlinkage __visible int printk(const char *fmt, ...)
{
	...
	// Added for battery log by Todd 20230105[
	if(strstr(fmt,"FIHBATTLOG") != NULL) {
	//if(strstr(fmt,"CH3_DBG") != NULL) {//for test
		int i;
		char printk_blsbuf[512];

		raw_spin_lock_irq(&bls_logbuf_lock);
		r=vscnprintf(printk_blsbuf, sizeof(printk_blsbuf), fmt, args);
		...
		if(waitqueue_active(&bls_log_wait))
			wake_up_interruptible(&bls_log_wait);
	}
	// Added for battery log by Todd 20230105]
	va_end(args);
