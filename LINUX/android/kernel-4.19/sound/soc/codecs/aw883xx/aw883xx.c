...
/* SX3-155 support speaker state notifier API start */
#include <linux/notifier.h>
/* SX3-155 support speaker state notifier API end */
...
/* SX3-155 support speaker state notifier API start */
#define AW_READ_SYSST_RETRIES		5	/* 5 times */
#define AW_READ_SYSST_RETRY_DELAY	5	/* 5 ms */
/* SX3-155 support speaker state notifier API end */
...
int register_aw883xx_shub_api_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&speaker_shub_api_notifier_list, nb);
}
EXPORT_SYMBOL(register_aw883xx_shub_api_notifier);
int unregister_aw883xx_shub_api_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&speaker_shub_api_notifier_list, nb);
}
EXPORT_SYMBOL(unregister_aw883xx_shub_api_notifier);
/* SX3-155 support speaker state notifier API end */
...
/* SX3-155 support speaker state notifier API start */
static int aw883xx_is_pa_enabled(struct aw883xx *aw883xx)
{
	int ret = -1;
	unsigned int cnt = 0;
	uint16_t reg_value = 0;

	while(cnt < AW_READ_SYSST_RETRIES) {
		ret = aw883xx_i2c_read(aw883xx, AW883XX_SYSST_REG, &reg_value);
		if(ret < 0) {
			aw_dev_err(aw883xx->dev, "failed to read REG_SYSST: %d", ret);
			return -EIO;
		} else {
			aw_dev_info(aw883xx->dev, "aw883xx sysst: 0x%04X", reg_value);
			ret = ((reg_value & 0x0311) > 0) ? 1 : 0;
			return ret;
		}

		cnt++;

		msleep(AW_READ_SYSST_RETRY_DELAY);
	}
...
static void aw883xx_start_pa(struct aw883xx *aw883xx)
{
	...
	for (i = 0; i < AW_START_RETRIES; i++) {
		...
		if (ret) {
			...
			/* SX3-155 support speaker state notifier API start */
			if (aw883xx_is_pa_enabled(aw883xx) == 1) {
				// PA enabled.
...
static int aw883xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		mutex_unlock(&aw883xx->lock);
		/* SX3-155 support speaker state notifier API start */
		if (aw883xx_is_pa_enabled(aw883xx) == 0) {
			// PA disabled.
